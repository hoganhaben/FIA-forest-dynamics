...
predict_f.age_first <- function(X, mod.number, data, p.alpha, p.tau) {
  ...
  if (mod.num == 5) {
    ...
    c = X["c"]
    d = X["d"]
    B <- (1 + (data$MEASTIME_t2_first-1990)*p.tau/100) * (1-p.alpha*data$B_L_prop_first) * 
...
## for f(age_last)
predict_f.age_last <- function(X, mod.number, data, p.alpha, p.tau) {
...
sim_f_age_first <- apply(parameters, 1, predict_f.age_first, data = data, mod.number = mod.num,
                         p.alpha = nls.param.df_bal[nls.param.df_bal$Ecoprovince == ecoprov,]$alpha,
                         p.tau = nls.param.df_bal[nls.param.df_bal$Ecoprovince == ecoprov,]$tau,
                         simplify = T)
...
predict_f.year_first <- function(X, mod.number, data, p.alpha, p.tau) {
...
## for f(year_last)
predict_f.year_last <- function(X, mod.number, data, p.alpha, p.tau) {
  ...
  if (mod.num == 5) {
    ...
    c = X["c"]
    d = X["d"]
    B <- (1 + (data$MEASTIME_t2_last-1990)*p.tau/100) * (1-p.alpha*data$B_L_prop_first) *
          (a+b*exp(-((log((data$STDAGE_t2_first/c)))/d)^2))
  } else if (mod.num == 2) {
    A = X["A"]
    k = X["k"]
...
### solve f(year) function at age_first and age_last
...
### take constant times difference in f(age_first) and f(age_last) over deltaT (aka MEASTIME_t2_last - MEASTIME_t2_first)  --  need to transform the numerator to get plots in rows and sim samples down columns -- then can do division 
sim_DeltaB <- t(sim_f_year_last - sim_f_year_first) / (data$MEASTIME_t2_last - data$MEASTIME_t2_first) 
...
predict_f.disturbance_first <- function(X, mod.number, data, p.alpha, p.tau) {
...
## for f(B_L_prop_last)
predict_f.disturbance_last <- function(X, mod.number, data, p.alpha, p.tau) {
  ...
  if (mod.num == 5) {
    ...
    c = X["c"]
    d = X["d"]
    B <- (1 + (data$MEASTIME_t2_first-1990)*p.tau/100) * (1-p.alpha*data$B_L_prop_last) *
          (a+b*exp(-((log((data$STDAGE_t2_first/c)))/d)^2))
  } else if (mod.num == 2) {
    A = X["A"]
    k = X["k"]
...
### solve f(disturbance) function at B_L_prop_first and B_L_prop_last
...
### take difference in f(B_L_prop_first) and f(B_L_prop_last)over deltaT (aka MEASTIME_t2_last - MEASTIME_t2_first)  --  need to transform the numerator to get plots in rows and sim samples down columns -- then can do division 
...
### take constant times difference in f(B_L_prop_first) and f(B_L_prop_last)over deltaT (aka MEASTIME_t2_last - MEASTIME_t2_first)  --  need to transform the numerator to get plots in rows and sim samples down columns -- then can do division 
sim_DeltaB <- t(sim_f_disturbance_last - sim_f_disturbance_first) / (data$MEASTIME_t2_last - data$MEASTIME_t2_first) 
...
### calculate variance from list of DeltaB_age bar samples (by ecoprov)
var.f_year <- unlist(lapply(B_sim_pred_f_year_bar, var, na.rm = T))   ## this is var_f(age)
...
BK_df_year_avg$se <- sqrt(var.f_year[match(BK_df_year_avg$Ecoprovince, names(var.f_age))])
...
BK_df_disturbance_avg$se <- sqrt(var.f_year[match(BK_df_disturbance_avg$Ecoprovince, names(var.f_age))])
...
BK_df_SUM_total_avg$se <- sqrt(var.f_age[match(BK_df_age_avg$Ecoprovince, names(var.f_age))] +
                               var.f_year[match(BK_df_disturbance_avg$Ecoprovince, names(var.f_age))] +
                               var.f_year[match(BK_df_year_avg$Ecoprovince, names(var.f_age))])
...
gg.Delta_B_total_compare <- ggplot(aes(x= value.x, y=value.y, color = Ecoprovince),
                 ...
                 ggrepel::geom_text_repel(aes(label = Ecoprovince),
                  ...
                  ggtitle(label = NULL, subtitle = parse(text = lm_eqn(lm_Delta_B_total_compare)))  + ylim(c(-1,3)) + xlim(c(-1,3))
...
gg.Delta2_alpha <- ggplot(aes(x= alpha_ML, y=value, col = Ecoprovince),
                 ...
                 ggrepel::geom_text_repel(aes(label = Ecoprovince), 
                                          ...
                                          min.segment.length = 10) +
                 ggtitle(label = NULL, subtitle = parse(text = lm_eqn(lm_Delta_B_L1)))  #+ ylim(c(-1.,0.5)) #+ xlim(c(-0.5,1.5))
...
gg_BK2b + coord_cartesian(ylim=c(-2, 2.5))
# dev.off()
```
...
## make the graphic
# tiff(filename = "Fig3_07.10.2023_3_stacker.tiff", width = 13.5, height = 20, units = "cm", res = 600, compression = "lzw")
gg_fig3.Xb
# dev.off()
```
...
```{r}
year_avg_merger3 <- merge(BK_df_year_avg, BK_df_disturbance_avg, by = c("Ecoprovince", "N"), suffixes = c("_disturbance", "_total"))
...
ggpubr::ggarrange(gg.DELTAB_prod_corr1, gg.DELTAB_prod_corr2 + theme(axis.title.y = element_blank()),
                  gg.DELTAB_prod_corr3 + theme(axis.title.y = element_blank()), labels = "AUTO", ncol=3, nrow =1, widths = c(1.03,0.95,1.02))

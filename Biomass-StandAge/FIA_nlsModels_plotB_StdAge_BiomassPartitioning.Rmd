---
title: "FIA NLS Models: Biomass vs. Stand Age (Temporally-Balanced dataset) -- with Biomass Partitioning for the Eastern US"
author: "JAH"
date: "`r Sys.Date()`"
output:
  html_document:
...
# setwd("C:/Users/hogie/Dropbox/FIA_R/NLS_modeling/New_G_nov2022/FINAL_RUN_JULY_2023")    # if working on P-50
...
### model 1
f_1 <- formula(B_plt_t2_MgHa ~ (1 + (MEASTIME_t2-1990)*tau/100) * A*STDAGE_t2 / (k+STDAGE_t2))
## model 2
f_2 <-  formula(B_plt_t2_MgHa ~ (1 + (MEASTIME_t2-1990)*tau/100) * (1-alpha*B_L_prop) * A*STDAGE_t2 / (k+STDAGE_t2))

## model 1 variants
f_1a <- formula(B_plt_t2_MgHa ~ (1 + (MEASTIME_t2-1990)*tau/100) * (p*A+((1-p)*A*STDAGE_t2/(k+STDAGE_t2))))
f_1b <- formula(B_plt_t2_MgHa ~ (1 + (MEASTIME_t2-1990)*tau/100) * (A*STDAGE_t2^s/(k^s+STDAGE_t2^s)))
f_1c <- formula(B_plt_t2_MgHa ~ (1 + (MEASTIME_t2-1990)*tau/100) * (p*A+((1-p)*A*STDAGE_t2^s/(k^s+STDAGE_t2^s))))


## model 2 variants
f_2a <- formula(B_plt_t2_MgHa ~ (1 + (MEASTIME_t2-1990)*tau/100) * (1-alpha*B_L_prop) * (p*A+((1-p)*A*STDAGE_t2/(k+STDAGE_t2))))
f_2b <- formula(B_plt_t2_MgHa ~ (1 + (MEASTIME_t2-1990)*tau/100) * (1-alpha*B_L_prop) * (A*STDAGE_t2^s/(k^s+STDAGE_t2^s)))
f_2c <- formula(B_plt_t2_MgHa ~ (1 + (MEASTIME_t2-1990)*tau/100) * (1-alpha*B_L_prop) * (p*A+((1-p)*A*STDAGE_t2^s/(k^s+STDAGE_t2^s))))

## Log-Normal models
## model 3
f_4 <- formula(B_plt_t2_MgHa ~ (1 + (MEASTIME_t2-1990)*tau/100) * (a+b*exp(-((log(STDAGE_t2/c))/d)^2)))
## model 4
f_5 <-  formula(B_plt_t2_MgHa ~ (1 + (MEASTIME_t2-1990)*tau/100) * (1-alpha*B_L_prop) * (a+b*exp(-((log(STDAGE_t2/c))/d)^2)))
...
# load("C:/Users/hogie/Dropbox/FIA_R/Starter_FIA_data_processing/Growth.Rdata")   # if working on P-50
...
Best_mods.P_bal <- data.frame(
  Ecoprovince = c("211", "212", "221", "222", "223", "231", 
...
try(
  nls_211.1 <- nls(f_1, data = G_211, 
                   start = c(tau=tau.start, A=A.start, k=k.start), 
...
try(
  nls_211.2 <- nls(f_2, data=G_211, 
                   start = c(tau=tau.start, alpha=alpha.start, A=A.start, k=k.start), 
                   weights = G_211$nls_weights.2)
 )
...
tau.fit <- round(coef(get(paste("nls_211.",  Mod.Sel1, sep ="")))["tau"], 1)
    ...
    paste("nls_211.", Mod.Sel1, "a", sep =""), 
      nls(get(paste("f_", Mod.Sel1, "a", sep ="")), 
          ...
          start = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)) {
            c(tau.fit, A.fit, k.fit, p=0.1)
            } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
              c(tau.fit, alpha.fit, A.fit, k.fit, p=0.1)
              } 
                ,
          weights = G_211$nls_weights.2))
 )
  ...
  assign(
    paste("nls_211.", Mod.Sel1, "b", sep =""), 
      nls(get(paste("f_", Mod.Sel1, "b", sep ="")), 
          data = G_211,
          start = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)) {
            c(tau.fit, A.fit, k.fit, s=1.5)
            } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
              c(tau.fit, alpha.fit, A.fit, k.fit, s=1.5)
              } 
                , 
          weights = G_211$nls_weights.2)) 
 )
  ...
  assign(
    paste("nls_211.", Mod.Sel1, "c", sep =""), 
      nls(get(paste("f_", Mod.Sel1, "c", sep ="")), 
          data = G_211,
          start = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)) {
            c(tau.fit, A.fit, k.fit, p=0.1, s=1.5)
            } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
              c(tau.fit, alpha.fit, A.fit, k.fit, p=0.1, s=1.5)
              } 
                , 
          weights = G_211$nls_weights.2))
 )
...
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "211",]$Sel.Mod.2 <- Mod.Sel2
...
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "211",]$tau.variance <- vcov(get(paste("nls_211.",  Mod.Sel2, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "211",]$alpha.variance <- vcov(get(paste("nls_211.",  Mod.Sel2, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "211",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "211",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "211",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "211",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "211",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "211",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "211",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "211",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "211",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "211",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "211",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "211",]$k.97.5 <- intervals_high["k"]
...
try(
  nls_211.5 <- nls(f_5, data=G_211, 
                   ...
                   lower = c(ge=-5, alpha=0, a=0, b=0, c=0, d=0), 
                   upper = c(ge=5, alpha=2, a= 1000, b=1000, c= 5000, d=10),
                   algorithm= "port",
                   weights = G_211$nls_weights.2)
 )

## test
if(exists("nls_211.4") & exists("nls_211.5")){
  print(anova(nls_211.4, nls_211.5))
} 

## AIC comparison
AIC3_211 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "211",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "211",]$Best.Mod <- Mod.Sel3
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "211",]$tau.variance <- vcov(get(paste("nls_211.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "211",]$alpha.variance <- vcov(get(paste("nls_211.",  Mod.Sel3, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "211",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "211",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "211",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "211",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "211",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "211",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "211",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "211",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "211",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "211",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "211",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "211",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "211",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "211",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "211",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "211",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "211",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "211",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
### define parameters from model selection 1
tau.fit <- round(coef(get(paste("nls_212.",  Mod.Sel1, sep ="")))["tau"], 1)
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "212",]$Sel.Mod.2 <- Mod.Sel2
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "212",]$tau.variance <- vcov(get(paste("nls_212.",  Mod.Sel2, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "212",]$alpha.variance <- vcov(get(paste("nls_212.",  Mod.Sel2, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "212",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "212",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "212",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "212",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "212",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "212",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "212",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "212",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "212",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "212",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "212",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "212",]$k.97.5 <- intervals_high["k"]
## end write coefficients to table 
...
## test
if(exists("nls_212.4") & exists("nls_212.5")){
  print(anova(nls_212.4, nls_212.5))
} 

## AIC comparison
AIC3_212 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "212",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "212",]$Best.Mod <- Mod.Sel3
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "212",]$tau.variance <- vcov(get(paste("nls_212.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "212",]$alpha.variance <- vcov(get(paste("nls_212.",  Mod.Sel3, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "212",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "212",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "212",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "212",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "212",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "212",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "212",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "212",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "212",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "212",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "212",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "212",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "212",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "212",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "212",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "212",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "212",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "212",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
### define parameters from model selection 1
tau.fit <- round(coef(get(paste("nls_221.",  Mod.Sel1, sep ="")))["tau"], 1)
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "221",]$Sel.Mod.2 <- Mod.Sel2
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "221",]$tau.variance <- vcov(get(paste("nls_221.",  Mod.Sel2, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "221",]$alpha.variance <- vcov(get(paste("nls_221.",  Mod.Sel2, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "221",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "221",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "221",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "221",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "221",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "221",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "221",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "221",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "221",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "221",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "221",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "221",]$k.97.5 <- intervals_high["k"]
## end write coefficients to table 
...
## test
if(exists("nls_221.4") & exists("nls_221.5")){
  print(anova(nls_221.4, nls_221.5))
} 

## AIC comparison
AIC3_221 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "221",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "221",]$Best.Mod <- Mod.Sel3
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "221",]$tau.variance <- vcov(get(paste("nls_221.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "221",]$alpha.variance <- vcov(get(paste("nls_221.",  Mod.Sel3, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "221",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "221",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "221",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "221",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "221",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "221",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "221",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "221",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "221",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "221",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "221",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "221",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "221",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "221",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "221",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "221",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "221",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "221",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
### define parameters from model selection 1
tau.fit <- round(coef(get(paste("nls_222.",  Mod.Sel1, sep ="")))["tau"], 1)
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "222",]$Sel.Mod.2 <- Mod.Sel2
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "222",]$tau.variance <- vcov(get(paste("nls_222.",  Mod.Sel2, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "222",]$alpha.variance <- vcov(get(paste("nls_222.",  Mod.Sel2, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "222",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "222",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "222",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "222",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "222",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "222",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "222",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "222",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "222",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "222",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "222",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "222",]$k.97.5 <- intervals_high["k"]
## end write coefficients to table 
...
## test
if(exists("nls_222.4") & exists("nls_222.5")){
  print(anova(nls_222.4, nls_222.5))
} 

## AIC comparison
AIC3_222 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "222",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "222",]$Best.Mod <- Mod.Sel3
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "222",]$tau.variance <- vcov(get(paste("nls_222.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "222",]$alpha.variance <- vcov(get(paste("nls_222.",  Mod.Sel3, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "222",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "222",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "222",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "222",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "222",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "222",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "222",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "222",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "222",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "222",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "222",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "222",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "222",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "222",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "222",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "222",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "222",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "222",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
### define parameters from model selection 1
tau.fit <- round(coef(get(paste("nls_223.",  Mod.Sel1, sep ="")))["tau"], 1)
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "223",]$Sel.Mod.2 <- Mod.Sel2
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "223",]$tau.variance <- vcov(get(paste("nls_223.",  Mod.Sel2, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "223",]$alpha.variance <- vcov(get(paste("nls_223.",  Mod.Sel2, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "223",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "223",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "223",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "223",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "223",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "223",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "223",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "223",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "223",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "223",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "223",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "223",]$k.97.5 <- intervals_high["k"]
## end write coefficients to table 
...
## test
if(exists("nls_223.4") & exists("nls_223.5")){
  print(anova(nls_223.4, nls_223.5))
} 

## AIC comparison
AIC3_223 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "223",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "223",]$Best.Mod <- Mod.Sel3
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "223",]$tau.variance <- vcov(get(paste("nls_223.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "223",]$alpha.variance <- vcov(get(paste("nls_223.",  Mod.Sel3, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "223",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "223",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "223",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "223",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "223",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "223",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "223",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "223",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "223",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "223",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "223",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "223",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "223",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "223",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "223",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "223",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "223",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "223",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
### define parameters from model selection 1
tau.fit <- round(coef(get(paste("nls_231.",  Mod.Sel1, sep ="")))["tau"], 1)
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "231",]$Sel.Mod.2 <- Mod.Sel2
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "231",]$tau.variance <- vcov(get(paste("nls_231.",  Mod.Sel2, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "231",]$alpha.variance <- vcov(get(paste("nls_231.",  Mod.Sel2, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "231",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "231",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "231",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "231",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "231",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "231",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "231",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "231",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "231",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "231",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "231",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "231",]$k.97.5 <- intervals_high["k"]
## end write coefficients to table 
...
## test
if(exists("nls_231.4") & exists("nls_231.5")){
  print(anova(nls_231.4, nls_231.5))
} 

## AIC comparison
AIC3_231 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "231",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "231",]$Best.Mod <- Mod.Sel3
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "231",]$tau.variance <- vcov(get(paste("nls_231.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "231",]$alpha.variance <- vcov(get(paste("nls_231.",  Mod.Sel3, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "231",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "231",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "231",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "231",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "231",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "231",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "231",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "231",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "231",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "231",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "231",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "231",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "231",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "231",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "231",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "231",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "231",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "231",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
### define parameters from model selection 1
tau.fit <- round(coef(get(paste("nls_232.",  Mod.Sel1, sep ="")))["tau"], 1)
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "232",]$Sel.Mod.2 <- Mod.Sel2
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "232",]$tau.variance <- vcov(get(paste("nls_232.",  Mod.Sel2, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "232",]$alpha.variance <- vcov(get(paste("nls_232.",  Mod.Sel2, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "232",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "232",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "232",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "232",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "232",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "232",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "232",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "232",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "232",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "232",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "232",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "232",]$k.97.5 <- intervals_high["k"]
## end write coefficients to table 
...
## test
if(exists("nls_232.4") & exists("nls_232.5")){
  print(anova(nls_232.4, nls_232.5))
} 

## AIC comparison
AIC3_232 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "232",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "232",]$Best.Mod <- Mod.Sel3
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "232",]$tau.variance <- vcov(get(paste("nls_232.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "232",]$alpha.variance <- vcov(get(paste("nls_232.",  Mod.Sel3, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "232",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "232",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "232",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "232",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "232",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "232",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "232",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "232",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "232",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "232",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "232",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "232",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "232",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "232",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "232",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "232",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "232",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "232",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
### define parameters from model selection 1
tau.fit <- round(coef(get(paste("nls_234.",  Mod.Sel1, sep ="")))["tau"], 1)
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "234",]$Sel.Mod.2 <- Mod.Sel2
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "234",]$tau.variance <- vcov(get(paste("nls_234.",  Mod.Sel2, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "234",]$alpha.variance <- vcov(get(paste("nls_234.",  Mod.Sel2, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "234",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "234",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "234",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "234",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "234",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "234",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "234",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "234",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "234",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "234",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "234",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "234",]$k.97.5 <- intervals_high["k"]
## end write coefficients to table 
...
## test
if(exists("nls_234.4") & exists("nls_234.5")){
  print(anova(nls_234.4, nls_234.5))
}

## AIC comparison
AIC3_234 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "234",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "234",]$Best.Mod <- Mod.Sel3
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "234",]$tau.variance <- vcov(get(paste("nls_234.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "234",]$alpha.variance <- vcov(get(paste("nls_234.",  Mod.Sel3, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "234",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "234",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "234",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "234",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "234",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "234",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "234",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "234",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "234",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "234",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "234",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "234",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "234",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "234",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "234",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "234",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "234",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "234",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
### define parameters from model selection 1
tau.fit <- round(coef(get(paste("nls_242.",  Mod.Sel1, sep ="")))["tau"], 1)
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "242",]$Sel.Mod.2 <- Mod.Sel2
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "242",]$tau.variance <- vcov(get(paste("nls_242.",  Mod.Sel2, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "242",]$alpha.variance <- vcov(get(paste("nls_242.",  Mod.Sel2, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "242",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "242",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "242",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "242",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "242",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "242",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "242",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "242",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "242",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "242",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "242",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "242",]$k.97.5 <- intervals_high["k"]
## end write coefficients to table 
...
## test
if(exists("nls_242.4") & exists("nls_242.5")){
  print(anova(nls_242.4, nls_242.5))
} 

## AIC comparison
AIC3_242 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "242",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "242",]$Best.Mod <- Mod.Sel3
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "242",]$tau.variance <- vcov(get(paste("nls_242.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "242",]$alpha.variance <- vcov(get(paste("nls_242.",  Mod.Sel3, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "242",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "242",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "242",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "242",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "242",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "242",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "242",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "242",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "242",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "242",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "242",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "242",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "242",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "242",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "242",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "242",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "242",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "242",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
### define parameters from model selection 1
tau.fit <- round(coef(get(paste("nls_251.",  Mod.Sel1, sep ="")))["tau"], 1)
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "251",]$Sel.Mod.2 <- Mod.Sel2
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "251",]$tau.variance <- vcov(get(paste("nls_251.",  Mod.Sel2, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "251",]$alpha.variance <- vcov(get(paste("nls_251.",  Mod.Sel2, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "251",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "251",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "251",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "251",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "251",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "251",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "251",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "251",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "251",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "251",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "251",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "251",]$k.97.5 <- intervals_high["k"]
## end write coefficients to table 
...
## test
if(exists("nls_251.4") & exists("nls_251.5")){
  print(anova(nls_251.4, nls_251.5))
}

## AIC comparison
AIC3_251 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "251",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "251",]$Best.Mod <- Mod.Sel3
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "251",]$tau.variance <- vcov(get(paste("nls_251.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "251",]$alpha.variance <- vcov(get(paste("nls_251.",  Mod.Sel3, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "251",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "251",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "251",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "251",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "251",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "251",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "251",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "251",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "251",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "251",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "251",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "251",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "251",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "251",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "251",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "251",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "251",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "251",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
### define parameters from model selection 1
tau.fit <- round(coef(get(paste("nls_255.",  Mod.Sel1, sep ="")))["tau"], 1)
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "255",]$Sel.Mod.2 <- Mod.Sel2
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "255",]$tau.variance <- vcov(get(paste("nls_255.",  Mod.Sel2, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "255",]$alpha.variance <- vcov(get(paste("nls_255.",  Mod.Sel2, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "255",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "255",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "255",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "255",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "255",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "255",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "255",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "255",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "255",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "255",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "255",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "255",]$k.97.5 <- intervals_high["k"]
## end write coefficients to table 
...
## test
if(exists("nls_255.4") & exists("nls_255.5")){
  print(anova(nls_255.4, nls_255.5))
} 

## AIC comparison
AIC3_255 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "255",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "255",]$Best.Mod <- Mod.Sel3
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "255",]$tau.variance <- vcov(get(paste("nls_255.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "255",]$alpha.variance <- vcov(get(paste("nls_255.",  Mod.Sel3, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "255",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "255",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "255",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "255",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "255",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "255",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "255",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "255",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "255",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "255",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "255",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "255",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "255",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "255",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "255",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "255",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "255",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "255",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
### define parameters from model selection 1
tau.fit <- round(coef(get(paste("nls_261.",  Mod.Sel1, sep ="")))["tau"], 1)
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "261",]$Sel.Mod.2 <- Mod.Sel2
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "261",]$tau.variance <- vcov(get(paste("nls_261.",  Mod.Sel2, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "261",]$alpha.variance <- vcov(get(paste("nls_261.",  Mod.Sel2, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "261",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "261",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "261",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "261",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "261",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "261",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "261",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "261",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "261",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "261",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "261",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "261",]$k.97.5 <- intervals_high["k"]
## end write coefficients to table 
...
## test
if(exists("nls_261.4") & exists("nls_261.5")){
  print(anova(nls_261.4, nls_261.5))
}

## AIC comparison
AIC3_261 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "261",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "261",]$Best.Mod <- Mod.Sel3


## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "261",]$tau.variance <- vcov(get(paste("nls_261.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "261",]$alpha.variance <- vcov(get(paste("nls_261.",  Mod.Sel3, sep ="")))["alpha","alpha"]
...
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "261",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "261",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "261",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "261",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "261",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "261",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "261",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "261",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "261",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "261",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "261",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "261",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "261",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "261",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "261",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "261",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "261",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
### define parameters from model selection 1
tau.fit <- round(coef(get(paste("nls_262.",  Mod.Sel1, sep ="")))["tau"], 1)
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "262",]$Sel.Mod.2 <- Mod.Sel2
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "262",]$tau.variance <- vcov(get(paste("nls_262.",  Mod.Sel2, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "262",]$alpha.variance <- vcov(get(paste("nls_262.",  Mod.Sel2, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "262",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "262",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "262",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "262",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "262",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "262",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "262",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "262",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "262",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "262",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "262",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "262",]$k.97.5 <- intervals_high["k"]
## end write coefficients to table 
...
## test
if(exists("nls_262.4") & exists("nls_262.5")){
  print(anova(nls_262.4, nls_262.5))
} 

## AIC comparison
AIC3_262 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "262",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "262",]$Best.Mod <- Mod.Sel3
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "262",]$tau.variance <- vcov(get(paste("nls_262.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "262",]$alpha.variance <- vcov(get(paste("nls_262.",  Mod.Sel3, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "262",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "262",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "262",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "262",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "262",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "262",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "262",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "262",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "262",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "262",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "262",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "262",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "262",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "262",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "262",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "262",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "262",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "262",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
### define parameters from model selection 1
tau.fit <- round(coef(get(paste("nls_263.",  Mod.Sel1, sep ="")))["tau"], 1)
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "263",]$Sel.Mod.2 <- Mod.Sel2
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "263",]$tau.variance <- vcov(get(paste("nls_263.",  Mod.Sel2, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "263",]$alpha.variance <- vcov(get(paste("nls_263.",  Mod.Sel2, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "263",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "263",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "263",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "263",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "263",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "263",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "263",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "263",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "263",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "263",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "263",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "263",]$k.97.5 <- intervals_high["k"]
## end write coefficients to table 
...
## test
if(exists("nls_263.4") & exists("nls_263.5")){
  print(anova(nls_263.4, nls_263.5))
} 

## AIC comparison
AIC3_263 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "263",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "263",]$Best.Mod <- Mod.Sel3
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "263",]$tau.variance <- vcov(get(paste("nls_263.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "263",]$alpha.variance <- vcov(get(paste("nls_263.",  Mod.Sel3, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "263",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "263",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "263",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "263",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "263",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "263",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "263",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "263",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "263",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "263",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "263",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "263",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "263",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "263",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "263",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "263",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "263",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "263",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
### define parameters from model selection 1
tau.fit <- round(coef(get(paste("nls_313.",  Mod.Sel1, sep ="")))["tau"], 1)
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "313",]$Sel.Mod.2 <- Mod.Sel2
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "313",]$tau.variance <- vcov(get(paste("nls_313.",  Mod.Sel2, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "313",]$alpha.variance <- vcov(get(paste("nls_313.",  Mod.Sel2, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "313",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "313",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "313",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "313",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "313",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "313",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "313",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "313",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "313",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "313",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "313",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "313",]$k.97.5 <- intervals_high["k"]
## end write coefficients to table 
...
## test
if(exists("nls_313.4") & exists("nls_313.5")){
  print(anova(nls_313.4, nls_313.5))
} 

## AIC comparison
AIC3_313 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "313",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "313",]$Best.Mod <- Mod.Sel3
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "313",]$tau.variance <- vcov(get(paste("nls_313.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "313",]$alpha.variance <- vcov(get(paste("nls_313.",  Mod.Sel3, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "313",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "313",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "313",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "313",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "313",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "313",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "313",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "313",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "313",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "313",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "313",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "313",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "313",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "313",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "313",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "313",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "313",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "313",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
### define parameters from model selection 1
tau.fit <- round(coef(get(paste("nls_315.",  Mod.Sel1, sep ="")))["tau"], 1)
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "315",]$Sel.Mod.2 <- Mod.Sel2
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "315",]$tau.variance <- vcov(get(paste("nls_315.",  Mod.Sel2, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "315",]$alpha.variance <- vcov(get(paste("nls_315.",  Mod.Sel2, sep ="")))["alpha","alpha"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "315",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "315",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "315",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "315",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "315",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "315",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "315",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "315",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "315",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "315",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "315",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "315",]$k.97.5 <- intervals_high["k"]
## end write coefficients to table 
...
## test
if(exists("nls_315.4") & exists("nls_315.5")){
  print(anova(nls_315.4, nls_315.5, nls_315.6))
} 
## AIC comparison
AIC3_315 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "315",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "315",]$Best.Mod <- Mod.Sel3
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "315",]$tau.variance <- vcov(get(paste("nls_315.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "315",]$alpha.variance <- vcov(get(paste("nls_315.",  Mod.Sel3, sep ="")))["alpha","alpha"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "315",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "315",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "315",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "315",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "315",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "315",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "315",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "315",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "315",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "315",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "315",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "315",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "315",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "315",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "315",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "315",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "315",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "315",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
### define parameters from model selection 1
tau.fit <- round(coef(get(paste("nls_321.",  Mod.Sel1, sep ="")))["tau"], 1)
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "321",]$Sel.Mod.2 <- Mod.Sel2
...
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "321",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "321",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "321",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "321",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "321",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "321",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "321",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "321",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "321",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "321",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "321",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "321",]$k.97.5 <- intervals_high["k"]
## end write coefficients to table 
...
## test
if(exists("nls_321.4") & exists("nls_321.5")){
  print(anova(nls_321.4, nls_321.5))
} 

## AIC comparison
AIC3_321 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "321",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "321",]$Best.Mod <- Mod.Sel3
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "321",]$tau.variance <- vcov(get(paste("nls_321.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "321",]$alpha.variance <- vcov(get(paste("nls_321.",  Mod.Sel3, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "321",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "321",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "321",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "321",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "321",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "321",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "321",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "321",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "321",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "321",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "321",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "321",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "321",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "321",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "321",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "321",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "321",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "321",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
### define parameters from model selection 1
tau.fit <- round(coef(get(paste("nls_322.",  Mod.Sel1, sep ="")))["tau"], 1)
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "322",]$Sel.Mod.2 <- Mod.Sel2
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "322",]$tau.variance <- vcov(get(paste("nls_322.",  Mod.Sel2, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "322",]$alpha.variance <- vcov(get(paste("nls_322.",  Mod.Sel2, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "322",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "322",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "322",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "322",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "322",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "322",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "322",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "322",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "322",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "322",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "322",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "322",]$k.97.5 <- intervals_high["k"]
## end write coefficients to table 
...
## test
if(exists("nls_322.4") & exists("nls_322.5")){
  print(anova(nls_322.4, nls_322.5))
} 

## AIC comparison
AIC3_322 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "322",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "322",]$Best.Mod <- Mod.Sel3
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "322",]$tau.variance <- vcov(get(paste("nls_322.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "322",]$alpha.variance <- vcov(get(paste("nls_322.",  Mod.Sel3, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "322",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "322",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "322",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "322",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "322",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "322",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "322",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "322",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "322",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "322",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "322",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "322",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "322",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "322",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "322",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "322",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "322",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "322",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
### define parameters from model selection 1
#tau.fit <- round(coef(get(paste("nls_331.",  Mod.Sel1, sep ="")))["tau"], 1)
tau.fit <-  -0.2
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "331",]$Sel.Mod.2 <- Mod.Sel2
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "331",]$tau.variance <- vcov(get(paste("nls_331.",  Mod.Sel2, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "331",]$alpha.variance <- vcov(get(paste("nls_331.",  Mod.Sel2, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "331",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "331",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "331",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "331",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "331",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "331",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "331",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "331",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "331",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "331",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "331",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "331",]$k.97.5 <- intervals_high["k"]
## end write coefficients to table 
...
## test
if(exists("nls_331.4") & exists("nls_331.5")){
  print(anova(nls_331.4, nls_331.5))
} 

## AIC comparison
AIC3_331 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "331",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "331",]$Best.Mod <- Mod.Sel3
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "331",]$tau.variance <- vcov(get(paste("nls_331.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "331",]$alpha.variance <- vcov(get(paste("nls_331.",  Mod.Sel3, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "331",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "331",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "331",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "331",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "331",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "331",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "331",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "331",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "331",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "331",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "331",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "331",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "331",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "331",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "331",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "331",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "331",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "331",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
### define parameters from model selection 1
tau.fit <- round(coef(get(paste("nls_332.",  Mod.Sel1, sep ="")))["tau"], 1)
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "332",]$Sel.Mod.2 <- Mod.Sel2
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "332",]$tau.variance <- vcov(get(paste("nls_332.",  Mod.Sel2, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "332",]$alpha.variance <- vcov(get(paste("nls_332.",  Mod.Sel2, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "332",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "332",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "332",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "332",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "332",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "332",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "332",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "332",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "332",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "332",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "332",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "332",]$k.97.5 <- intervals_high["k"]
## end write coefficients to table 
...
## test
if(exists("nls_332.4") & exists("nls_332.5")){
  print(anova(nls_332.4, nls_332.5))
}

## AIC comparison
AIC3_332 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "332",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "332",]$Best.Mod <- Mod.Sel3
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "332",]$tau.variance <- vcov(get(paste("nls_332.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "332",]$alpha.variance <- vcov(get(paste("nls_332.",  Mod.Sel3, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "332",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "332",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "332",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "332",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "332",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "332",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "332",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "332",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "332",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "332",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "332",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "332",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "332",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "332",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "332",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "332",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "332",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "332",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
### define parameters from model selection 1
tau.fit <- round(coef(get(paste("nls_341.",  Mod.Sel1, sep ="")))["tau"], 1)
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "341",]$Sel.Mod.2 <- Mod.Sel2
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "341",]$tau.variance <- vcov(get(paste("nls_341.",  Mod.Sel2, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "341",]$alpha.variance <- vcov(get(paste("nls_341.",  Mod.Sel2, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "341",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "341",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "341",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "341",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "341",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "341",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "341",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "341",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "341",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "341",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "341",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "341",]$k.97.5 <- intervals_high["k"]
## end write coefficients to table 
...
try(
  nls_341.6 <- nls(f_6, data = G_341, 
                   start = c(tau=tau.start, alpha=alpha.start, alpha=alpha.start, a=a.start, b=b.start, c=c.start, d=d.start), 
                   lower = c(ge=-5, alpha=0, alpha= 0, a=0, b=0, c=0, d=0), 
                   upper = c(ge=5, alpha=2, alpha= 2, a=1000, b=1000, c= 5000, d=10),
                   algorithm= "port",
                   weights = G_341$nls_weights.2)
 )

## test
if(exists("nls_341.4") & exists("nls_341.5")){
  print(anova(nls_341.4, nls_341.5))
} 

## AIC comparison
AIC3_341 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "341",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "341",]$Best.Mod <- Mod.Sel3
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "341",]$tau.variance <- vcov(get(paste("nls_341.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "341",]$alpha.variance <- vcov(get(paste("nls_341.",  Mod.Sel3, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "341",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "341",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "341",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "341",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "341",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "341",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "341",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "341",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "341",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "341",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "341",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "341",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "341",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "341",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "341",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "341",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "341",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "341",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
### define parameters from model selection 1
tau.fit <- round(coef(get(paste("nls_342.",  Mod.Sel1, sep ="")))["tau"], 1)
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "342",]$Sel.Mod.2 <- Mod.Sel2
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "342",]$tau.variance <- vcov(get(paste("nls_342.",  Mod.Sel2, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "342",]$alpha.variance <- vcov(get(paste("nls_342.",  Mod.Sel2, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "342",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "342",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "342",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "342",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "342",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "342",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "342",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "342",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "342",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "342",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "342",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "342",]$k.97.5 <- intervals_high["k"]
## end write coefficients to table 
...
## test
if(exists("nls_342.4") & exists("nls_342.5")){
  print(anova(nls_342.4, nls_342.5))
}

## AIC comparison
AIC3_342 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "342",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "342",]$Best.Mod <- Mod.Sel3
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "342",]$tau.variance <- vcov(get(paste("nls_342.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "342",]$alpha.variance <- vcov(get(paste("nls_342.",  Mod.Sel3, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "342",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "342",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "342",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "342",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "342",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "342",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "342",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "342",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "342",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "342",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "342",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "342",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "342",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "342",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "342",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "342",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "342",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "342",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
### define parameters from model selection 1
tau.fit <- round(coef(get(paste("nls_411.",  Mod.Sel1, sep ="")))["tau"], 1)
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "411",]$Sel.Mod.2 <- Mod.Sel2
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "411",]$tau.variance <- vcov(get(paste("nls_411.",  Mod.Sel2, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "411",]$alpha.variance <- vcov(get(paste("nls_411.",  Mod.Sel2, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "411",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "411",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "411",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "411",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "411",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "411",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "411",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "411",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "411",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "411",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "411",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "411",]$k.97.5 <- intervals_high["k"]
## end write coefficients to table 
...
## test
if(exists("nls_411.4") & exists("nls_411.5")){
  print(anova(nls_411.4, nls_411.5))
}

## AIC comparison
AIC3_411 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "411",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "411",]$Best.Mod <- Mod.Sel3
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "411",]$tau.variance <- vcov(get(paste("nls_411.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "411",]$alpha.variance <- vcov(get(paste("nls_411.",  Mod.Sel3, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "411",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "411",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "411",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "411",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "411",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "411",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "411",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "411",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "411",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "411",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "411",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "411",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "411",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "411",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "411",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "411",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "411",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "411",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
### define parameters from model selection 1
tau.fit <- round(coef(get(paste("nls_M211.",  Mod.Sel1, sep ="")))["tau"], 1)
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M211",]$Sel.Mod.2 <- Mod.Sel2
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M211",]$tau.variance <- vcov(get(paste("nls_M211.",  Mod.Sel2, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M211",]$alpha.variance <- vcov(get(paste("nls_M211.",  Mod.Sel2, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M211",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M211",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M211",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M211",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M211",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M211",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M211",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M211",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M211",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M211",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M211",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M211",]$k.97.5 <- intervals_high["k"]
## end write coefficients to table 
...
## test
if(exists("nls_M211.4") & exists("nls_M211.5")){
  print(anova(nls_M211.4, nls_M211.5))
} 

## AIC comparison
AIC3_M211 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M211",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M211",]$Best.Mod <- Mod.Sel3
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M211",]$tau.variance <- vcov(get(paste("nls_M211.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M211",]$alpha.variance <- vcov(get(paste("nls_M211.",  Mod.Sel3, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M211",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M211",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M211",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M211",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M211",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M211",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M211",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M211",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M211",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M211",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M211",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M211",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M211",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M211",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M211",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M211",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M211",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M211",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
G_M211_pred <- predict(get(paste("nls_M211.",  Mod.Sel3, sep ="")),
                      newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                        ...
                        data.frame("MEASTIME_t2" = rep(mean(G_M221$MEASTIME_t2, na.rm = T), max(G_M211$STDAGE_t2)),
                                   "STDAGE_t2" = seq(1, max(G_M211$STDAGE_t2), by = 1),
...
### define parameters from model selection 1
tau.fit <- round(coef(get(paste("nls_M221.",  Mod.Sel1, sep ="")))["tau"], 1)
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M221",]$Sel.Mod.2 <- Mod.Sel2
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M221",]$tau.variance <- vcov(get(paste("nls_M221.",  Mod.Sel2, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M221",]$alpha.variance <- vcov(get(paste("nls_M221.",  Mod.Sel2, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M221",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M221",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M221",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M221",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M221",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M221",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M221",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M221",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M221",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M221",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M221",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M221",]$k.97.5 <- intervals_high["k"]
## end write coefficients to table 
...
## test
if(exists("nls_M221.4") & exists("nls_M221.5")){
  print(anova(nls_M221.4, nls_M221.5))
} 

## AIC comparison
AIC3_M221 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M221",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M221",]$Best.Mod <- Mod.Sel3
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M221",]$tau.variance <- vcov(get(paste("nls_M221.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M221",]$alpha.variance <- vcov(get(paste("nls_M221.",  Mod.Sel3, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M221",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M221",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M221",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M221",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M221",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M221",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M221",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M221",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M221",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M221",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M221",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M221",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M221",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M221",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M221",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M221",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M221",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M221",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
### define parameters from model selection 1
tau.fit <- round(coef(get(paste("nls_M223.",  Mod.Sel1, sep ="")))["tau"], 1)
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M223",]$Sel.Mod.2 <- Mod.Sel2
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M223",]$tau.variance <- vcov(get(paste("nls_M223.",  Mod.Sel2, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M223",]$alpha.variance <- vcov(get(paste("nls_M223.",  Mod.Sel2, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M223",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M223",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M223",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M223",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M223",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M223",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M223",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M223",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M223",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M223",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M223",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M223",]$k.97.5 <- intervals_high["k"]
## end write coefficients to table 
...
## test
if(exists("nls_M223.4") & exists("nls_M223.5")){
  print(anova(nls_M223.4, nls_M223.5))
} 

## AIC comparison
AIC3_M223 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M223",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M223",]$Best.Mod <- Mod.Sel3
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M223",]$tau.variance <- vcov(get(paste("nls_M223.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M223",]$alpha.variance <- vcov(get(paste("nls_M223.",  Mod.Sel3, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M223",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M223",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M223",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M223",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M223",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M223",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M223",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M223",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M223",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M223",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M223",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M223",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M223",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M223",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M223",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M223",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M223",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M223",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
### define parameters from model selection 1
tau.fit <- round(coef(get(paste("nls_M231.",  Mod.Sel1, sep ="")))["tau"], 1)
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M231",]$Sel.Mod.2 <- Mod.Sel2
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M231",]$tau.variance <- vcov(get(paste("nls_M231.",  Mod.Sel2, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M231",]$alpha.variance <- vcov(get(paste("nls_M231.",  Mod.Sel2, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M231",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M231",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M231",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M231",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M231",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M231",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M231",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M231",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M231",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M231",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M231",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M231",]$k.97.5 <- intervals_high["k"]
## end write coefficients to table 
...
## test
if(exists("nls_M231.4") & exists("nls_M231.5")){
  print(anova(nls_M231.4, nls_M231.5))
}

## AIC comparison
AIC3_M231 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M231",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M231",]$Best.Mod <- Mod.Sel3
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M231",]$tau.variance <- vcov(get(paste("nls_M231.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M231",]$alpha.variance <- vcov(get(paste("nls_M231.",  Mod.Sel3, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M231",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M231",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M231",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M231",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M231",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M231",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M231",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M231",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M231",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M231",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M231",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M231",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M231",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M231",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M231",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M231",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M231",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M231",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
### define parameters from model selection 1
tau.fit <- round(coef(get(paste("nls_M242.",  Mod.Sel1, sep ="")))["tau"], 1)
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M242",]$Sel.Mod.2 <- Mod.Sel2
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M242",]$tau.variance <- vcov(get(paste("nls_M242.",  Mod.Sel2, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M242",]$alpha.variance <- vcov(get(paste("nls_M242.",  Mod.Sel2, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M242",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M242",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M242",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M242",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M242",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M242",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M242",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M242",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M242",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M242",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M242",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M242",]$k.97.5 <- intervals_high["k"]
## end write coefficients to table 
...
## test
if(exists("nls_M242.4") & exists("nls_M242.5")){
  print(anova(nls_M242.4, nls_M242.5))
} 

## AIC comparison
AIC3_M242 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M242",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M242",]$Best.Mod <- Mod.Sel3
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M242",]$tau.variance <- vcov(get(paste("nls_M242.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M242",]$alpha.variance <- vcov(get(paste("nls_M242.",  Mod.Sel3, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M242",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M242",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M242",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M242",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M242",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M242",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M242",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M242",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M242",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M242",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M242",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M242",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M242",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M242",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M242",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M242",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M242",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M242",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
### define parameters from model selection 1
tau.fit <- round(coef(get(paste("nls_M261.",  Mod.Sel1, sep ="")))["tau"], 1)
slpha.fit <- round(coef(get(paste("nls_M261.",  Mod.Sel1, sep ="")))["alpha"], 1)
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M261",]$Sel.Mod.2 <- Mod.Sel2
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M261",]$tau.variance <- vcov(get(paste("nls_M261.",  Mod.Sel2, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M261",]$alpha.variance <- vcov(get(paste("nls_M261.",  Mod.Sel2, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M261",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M261",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M261",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M261",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M261",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M261",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M261",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M261",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M261",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M261",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M261",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M261",]$k.97.5 <- intervals_high["k"]
## end write coefficients to table 
...
## test
if(exists("nls_M261.4") & exists("nls_M261.5")){
  print(anova(nls_M261.4, nls_M261.5))
} 

## AIC comparison
AIC3_M261 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M261",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M261",]$Best.Mod <- Mod.Sel3
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M261",]$tau.variance <- vcov(get(paste("nls_M261.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M261",]$alpha.variance <- vcov(get(paste("nls_M261.",  Mod.Sel3, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M261",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M261",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M261",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M261",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M261",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M261",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M261",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M261",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M261",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M261",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M261",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M261",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M261",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M261",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M261",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M261",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M261",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M261",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
### define parameters from model selection 1
tau.fit <- round(coef(get(paste("nls_M262.",  Mod.Sel1, sep ="")))["tau"], 1)
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M262",]$Sel.Mod.2 <- Mod.Sel2
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M262",]$tau.variance <- vcov(get(paste("nls_M262.",  Mod.Sel2, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M262",]$alpha.variance <- vcov(get(paste("nls_M262.",  Mod.Sel2, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M262",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M262",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M262",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M262",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M262",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M262",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M262",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M262",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M262",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M262",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M262",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M262",]$k.97.5 <- intervals_high["k"]
## end write coefficients to table 
...
# phi/alpha
## test
if(exists("nls_M262.4") & exists("nls_M262.5")){
  print(anova(nls_M262.4, nls_M262.5, nls_M262.6))
} 

## AIC comparison
AIC3_M262 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M262",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M262",]$Best.Mod <- Mod.Sel3
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M262",]$tau.variance <- vcov(get(paste("nls_M262.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M262",]$alpha.variance <- vcov(get(paste("nls_M262.",  Mod.Sel3, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M262",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M262",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M262",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M262",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M262",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M262",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M262",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M262",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M262",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M262",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M262",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M262",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M262",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M262",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M262",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M262",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M262",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M262",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
### define parameters from model selection 1
tau.fit <- round(coef(get(paste("nls_M313.",  Mod.Sel1, sep ="")))["tau"], 1)
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M313",]$Sel.Mod.2 <- Mod.Sel2
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M313",]$tau.variance <- vcov(get(paste("nls_M313.",  Mod.Sel2, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M313",]$alpha.variance <- vcov(get(paste("nls_M313.",  Mod.Sel2, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M313",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M313",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M313",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M313",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M313",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M313",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M313",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M313",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M313",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M313",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M313",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M313",]$k.97.5 <- intervals_high["k"]
## end write coefficients to table 
...
## test
if(exists("nls_M313.4") & exists("nls_M313.5")){
  print(anova(nls_M313.4, nls_M313.5))
} 

## AIC comparison
AIC3_M313 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M313",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M313",]$Best.Mod <- Mod.Sel3
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M313",]$tau.variance <- vcov(get(paste("nls_M313.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M313",]$alpha.variance <- vcov(get(paste("nls_M313.",  Mod.Sel3, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M313",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M313",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M313",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M313",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M313",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M313",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M313",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M313",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M313",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M313",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M313",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M313",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M313",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M313",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M313",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M313",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M313",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M313",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
### define parameters from model selection 1
tau.fit <- round(coef(get(paste("nls_M331.",  Mod.Sel1, sep ="")))["tau"], 1)
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M331",]$Sel.Mod.2 <- Mod.Sel2
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M313",]$tau.variance <- vcov(get(paste("nls_M331.",  Mod.Sel2, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M331",]$alpha.variance <- vcov(get(paste("nls_M331.",  Mod.Sel2, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M331",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M331",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M331",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M331",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M331",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M331",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M331",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M331",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M331",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M331",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M331",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M331",]$k.97.5 <- intervals_high["k"]
## end write coefficients to table 
...
## test
if(exists("nls_M331.4") & exists("nls_M331.5")){
  print(anova(nls_M331.4, nls_M331.5))
} 

## AIC comparison
AIC3_M331 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M331",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M331",]$Best.Mod <- Mod.Sel3
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M331",]$tau.variance <- vcov(get(paste("nls_M331.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M331",]$alpha.variance <- vcov(get(paste("nls_M331.",  Mod.Sel3, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M331",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M331",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M331",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M331",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M331",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M331",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M331",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M331",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M331",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M331",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M331",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M331",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M331",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M331",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M331",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M331",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M331",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M331",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
### define parameters from model selection 1
tau.fit <- round(coef(get(paste("nls_M332.",  Mod.Sel1, sep ="")))["tau"], 1)
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M332",]$Sel.Mod.2 <- Mod.Sel2
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M332",]$tau.variance <- vcov(get(paste("nls_M332.",  Mod.Sel2, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M332",]$alpha.variance <- vcov(get(paste("nls_M332.",  Mod.Sel2, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M332",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M332",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M332",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M332",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M332",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M332",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M332",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M332",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M332",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M332",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M332",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M332",]$k.97.5 <- intervals_high["k"]
## end write coefficients to table 
...
## test
if(exists("nls_M332.4") & exists("nls_M332.5")){
  print(anova(nls_M332.4, nls_M332.5))
} 

## AIC comparison
AIC3_M332 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M332",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M332",]$Best.Mod <- Mod.Sel3
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M332",]$tau.variance <- vcov(get(paste("nls_M332.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M332",]$alpha.variance <- vcov(get(paste("nls_M332.",  Mod.Sel3, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M332",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M332",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M332",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M332",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M332",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M332",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M332",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M332",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M332",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M332",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M332",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M332",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M332",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M332",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M332",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M332",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M332",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M332",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
### define parameters from model selection 1
tau.fit <- round(coef(get(paste("nls_M333.",  Mod.Sel1, sep ="")))["tau"], 1)
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M333",]$Sel.Mod.2 <- Mod.Sel2
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M333",]$tau.variance <- vcov(get(paste("nls_M333.",  Mod.Sel2, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M333",]$alpha.variance <- vcov(get(paste("nls_M333.",  Mod.Sel2, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M333",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M333",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M333",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M333",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M333",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M333",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M333",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M333",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M333",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M333",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M333",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M333",]$k.97.5 <- intervals_high["k"]
## end write coefficients to table 
...
## test
if(exists("nls_M333.4") & exists("nls_M333.5")){
  print(anova(nls_M333.4, nls_M333.5))
} 

## AIC comparison
AIC3_M333 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M333",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M333",]$Best.Mod <- Mod.Sel3
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M333",]$tau.variance <- vcov(get(paste("nls_M333.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M333",]$alpha.variance <- vcov(get(paste("nls_M333.",  Mod.Sel3, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M333",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M333",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M333",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M333",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M333",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M333",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M333",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M333",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M333",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M333",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M333",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M333",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M333",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M333",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M333",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M333",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M333",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M333",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
### define parameters from model selection 1
tau.fit <- round(coef(get(paste("nls_M334.",  Mod.Sel1, sep ="")))["tau"], 1)
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M334",]$Sel.Mod.2 <- Mod.Sel2
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M334",]$tau.variance <- vcov(get(paste("nls_M334.",  Mod.Sel2, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M334",]$alpha.variance <- vcov(get(paste("nls_M334.",  Mod.Sel2, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M334",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M334",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M334",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M334",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M334",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M334",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M334",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M334",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M334",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M334",]$phi.97.5 <- intervals_high["phi"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M334",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M334",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M334",]$k.97.5 <- intervals_high["k"]
## end write coefficients to table 
...
## test
if(exists("nls_M334.4") & exists("nls_M334.5")){
  print(anova(nls_M334.4, nls_M334.5))
} 


## AIC comparison
AIC3_M334 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M334",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M334",]$Best.Mod <- Mod.Sel3
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M334",]$tau.variance <- vcov(get(paste("nls_M334.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M334",]$alpha.variance <- vcov(get(paste("nls_M334.",  Mod.Sel3, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M334",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M334",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M334",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M334",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M334",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M334",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M334",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M334",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M334",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M334",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M334",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M334",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M334",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M334",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M334",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M334",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M334",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M334",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
### define parameters from model selection 1
tau.fit <- round(coef(get(paste("nls_M341.",  Mod.Sel1, sep ="")))["tau"], 1)
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M341",]$Sel.Mod.2 <- Mod.Sel2
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M341",]$tau.variance <- vcov(get(paste("nls_M341.",  Mod.Sel2, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M341",]$alpha.variance <- vcov(get(paste("nls_M341.",  Mod.Sel2, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M341",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M341",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M341",]$A <- coefs["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M341",]$k <- coefs["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M341",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M341",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M341",]$A.2.5 <- intervals_low["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M341",]$k.2.5 <- intervals_low["k"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M341",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M341",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M341",]$A.97.5 <- intervals_high["A"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M341",]$k.97.5 <- intervals_high["k"]
## end write coefficients to table 
...
## test
if(exists("nls_M341.4") & exists("nls_M341.5")){
  print(anova(nls_M341.4, nls_M341.5))
} 

## AIC comparison
AIC3_M341 <- data.frame(model = c(Mod.Sel2, 4,5), AIC = c(
...
## best model -- write the best fitting model to Best_mods df
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M341",]$Sel.Mod.3 <- Mod.Sel3
Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M341",]$Best.Mod <- Mod.Sel3
...
## get variance in ge, phi and alpha
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M341",]$tau.variance <- vcov(get(paste("nls_M341.",  Mod.Sel3, sep ="")))["tau","tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M341",]$alpha.variance <- vcov(get(paste("nls_M341.",  Mod.Sel3, sep ="")))["alpha","alpha"]


nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M341",]$tau <- coefs["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M341",]$alpha <- coefs["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M341",]$a <- coefs["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M341",]$b <- coefs["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M341",]$c <- coefs["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M341",]$d <- coefs["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M341",]$tau.2.5 <- intervals_low["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M341",]$alpha.2.5 <- intervals_low["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M341",]$a.2.5 <- intervals_low["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M341",]$b.2.5 <- intervals_low["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M341",]$c.2.5 <- intervals_low["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M341",]$d.2.5 <- intervals_low["d"]

nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M341",]$tau.97.5 <- intervals_high["tau"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M341",]$alpha.97.5 <- intervals_high["alpha"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M341",]$a.97.5 <- intervals_high["a"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M341",]$b.97.5 <- intervals_high["b"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M341",]$c.97.5 <- intervals_high["c"]
nls.param.df_bal[nls.param.df_bal$Ecoprovince == "M341",]$d.97.5 <- intervals_high["d"]
## end write coefficients to table 
...
Best_mods.P_bal$Ecoprovince <- as.factor(Best_mods.P_bal$Ecoprovince)
...
vcov_list <- list(
  #### 211
  `211` = vcov(get(paste("nls_211.", Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 211,]$Best.Mod, sep =""))),
  ####  212
  `212` = vcov(get(paste("nls_212.", Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 212,]$Best.Mod, sep =""))),
  ####  221
  `221` = vcov(get(paste("nls_221.", Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 221,]$Best.Mod, sep =""))),
  ####  222
  `222` = vcov(get(paste("nls_222.", Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 222,]$Best.Mod, sep =""))),
  ####  223
  `223` = vcov(get(paste("nls_223.", Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 223,]$Best.Mod, sep =""))),
  ####  231
  `231` = vcov(get(paste("nls_231.", Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 231,]$Best.Mod, sep =""))),
  ####  232
  `232` = vcov(get(paste("nls_232.", Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 232,]$Best.Mod, sep =""))),
  ####  234
  `234` = vcov(get(paste("nls_234.", Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 234,]$Best.Mod, sep =""))),
  ####  251
  `251` = vcov(get(paste("nls_251.", Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 251,]$Best.Mod, sep =""))),
  ####  255
  `255` = vcov(get(paste("nls_255.", Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 255,]$Best.Mod, sep =""))),
  ####  332
  `332` = vcov(get(paste("nls_255.", Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 332,]$Best.Mod, sep =""))),
  ####  M211
  `M211` = vcov(get(paste("nls_M211.", Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M211",]$Best.Mod, sep =""))),
  ####  M221
  `M221` = vcov(get(paste("nls_M221.", Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M221",]$Best.Mod, sep =""))),
  ####  M223
  `M223` = vcov(get(paste("nls_M223.", Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M223",]$Best.Mod, sep =""))),
  ####  M231
  `M231` = vcov(get(paste("nls_M231.", Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M231",]$Best.Mod, sep =""))),
  ####  M261
  `M261` = vcov(get(paste("nls_M261.", Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M261",]$Best.Mod, sep =""))),
  ####  M334
  `M334` = vcov(get(paste("nls_M334.", Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M334",]$Best.Mod, sep ="")))
)
...
### plot tau (productivity trend)

```{r, echo= FALSE, message = FALSE, warning = FALSE, out.width = "200%"}
## order data
nls.param.df_bal$Ecoregion<- factor(nls.param.df_bal$Ecoregion, levels = nls.param.df_bal[rev(order(nls.param.df_bal$tau)), "Ecoregion"])

library(ggplot2)

#### create color vector 
col_vec.tau <- rep(NA, length(nls.param.df_bal$tau))

for (i in 1:length(nls.param.df_bal$tau)) {
  if(is.na(nls.param.df_bal$tau[i])){
     col_vec.tau[i] <-  "white"
  } else if(nls.param.df_bal$tau.2.5[i]>0) {
       col_vec.tau[i] <- scales::muted("green")  
    } else if(nls.param.df_bal$tau.97.5[i]<0){
...
# tiff(file = "tau_nls3_plotB_StdAge_ReconciledG.tiff", width = 15, height = 12, units = "cm", res = 600, compression = "lzw")
gg.tau_ECOSUBCD
# dev.off()
```
...
tau_df <-  data.frame( ECOPROVCD = as.factor(c(nls.param.df_bal$Ecoprovince,"Water")), tau = as.numeric(c(nls.param.df_bal[,"tau"], NA)))
...
### left_join
spdf_tidy <- left_join(spdf_tidy, tau_df, by = c("MAP_UNIT_S" = "ECOPROVCD"), keep = F)
...
mapper2  <- ggplot(aes(x = long, y = lat, group = group), data = spdf_tidy) +
  ...
  metR::scale_fill_divergent(
                       ...
                       name = "biomass stock productivity trend (%/yr)",
...
# tiff(filename = "tau_mapper_biomass_age_11.7.22.tiff", width = 15, height = 8, units = "cm", res= 600, compression = "lzw")
mapper2
...
```{r}
### Calculate DeltaB_total  -- the differnce between t2 FIRST MEASUREMENT of temporally balanced and t2 LAST MEASUREMENT of temporally balanced

## done only for ECOPROVS where models fit well

####  211
G_211_last <- G_211 %>% group_by(pltID) %>% arrange(INVYR) %>% slice(n())
G_211_first <- G_211 %>% group_by(pltID) %>% arrange(INVYR) %>% slice(1)

DeltaB_df_211 <- data.frame("pltID_first" = G_211_first$pltID, 
                            "MEASTIME_t2_first" = G_211_first$MEASTIME_t2, 
                            "STDAGE_t2_first" = G_211_first$STDAGE_t2,
                            "B_L_prop_first" = G_211_first$B_L_prop,
                            "B_plt_t2_MgHa_first" = G_211_first$B_plt_t2_MgHa, 
                            
                            "pltID_last" = G_211_last$pltID,
                            "MEASTIME_t2_last" = G_211_last$MEASTIME_t2, 
                            "STDAGE_t2_last" = G_211_last$STDAGE_t2,
                            "B_L_prop_last" = G_211_last$B_L_prop,
                            "B_plt_t2_MgHa_last" = G_211_last$B_plt_t2_MgHa
                            )

DeltaB_df_211$DeltaB_total <- (G_211_last$B_plt_t2_MgHa - G_211_first$B_plt_t2_MgHa) / (G_211_last$MEASTIME_t2 - G_211_first$MEASTIME_t2)

## overwrite NaNs with zero -- all NaN B_L_props have B_t1 (biomass at t1) = 0
DeltaB_df_211[is.nan(DeltaB_df_211$B_L_prop_first),"B_L_prop_first"] <- 0
DeltaB_df_211[is.nan(DeltaB_df_211$B_L_prop_last),"B_L_prop_last"] <- 0


####  212
G_212_last <- G_212 %>% group_by(pltID) %>% arrange(INVYR) %>% slice(n())
G_212_first <- G_212 %>% group_by(pltID) %>% arrange(INVYR) %>% slice(1)

DeltaB_df_212 <- data.frame("pltID_first" = G_212_first$pltID, 
                            "MEASTIME_t2_first" = G_212_first$MEASTIME_t2, 
                            "STDAGE_t2_first" = G_212_first$STDAGE_t2,
                            "B_L_prop_first" = G_212_first$B_L_prop,
                            "B_plt_t2_MgHa_first" = G_212_first$B_plt_t2_MgHa, 
                            
                            "pltID_last" = G_212_last$pltID,
                            "MEASTIME_t2_last" = G_212_last$MEASTIME_t2, 
                            "STDAGE_t2_last" = G_212_last$STDAGE_t2,
                            "B_L_prop_last" = G_212_last$B_L_prop,
                            "B_plt_t2_MgHa_last" = G_212_last$B_plt_t2_MgHa
                            )

DeltaB_df_212$DeltaB_total <- (G_212_last$B_plt_t2_MgHa - G_212_first$B_plt_t2_MgHa) / (G_212_last$MEASTIME_t2 - G_212_first$MEASTIME_t2)

## overwrite NaNs with zero -- all NaN B_L_props have B_t1 (biomass at t1) = 0
DeltaB_df_212[is.nan(DeltaB_df_212$B_L_prop_first),"B_L_prop_first"] <- 0
DeltaB_df_212[is.nan(DeltaB_df_212$B_L_prop_last),"B_L_prop_last"] <- 0


####  221
G_221_last <- G_221 %>% group_by(pltID) %>% arrange(INVYR) %>% slice(n())
G_221_first <- G_221 %>% group_by(pltID) %>% arrange(INVYR) %>% slice(1)

DeltaB_df_221 <- data.frame("pltID_first" = G_221_first$pltID, 
                            "MEASTIME_t2_first" = G_221_first$MEASTIME_t2, 
                            "STDAGE_t2_first" = G_221_first$STDAGE_t2,
                            "B_L_prop_first" = G_221_first$B_L_prop,
                            "B_plt_t2_MgHa_first" = G_221_first$B_plt_t2_MgHa, 
                            
                            "pltID_last" = G_221_last$pltID,
                            "MEASTIME_t2_last" = G_221_last$MEASTIME_t2, 
                            "STDAGE_t2_last" = G_221_last$STDAGE_t2,
                            "B_L_prop_last" = G_221_last$B_L_prop,
                            "B_plt_t2_MgHa_last" = G_221_last$B_plt_t2_MgHa
                            )

DeltaB_df_221$DeltaB_total <- (G_221_last$B_plt_t2_MgHa - G_221_first$B_plt_t2_MgHa) / (G_221_last$MEASTIME_t2 - G_221_first$MEASTIME_t2)

## overwrite NaNs with zero -- all NaN B_L_props have B_t1 (biomass at t1) = 0
DeltaB_df_221[is.nan(DeltaB_df_221$B_L_prop_first),"B_L_prop_first"] <- 0
DeltaB_df_221[is.nan(DeltaB_df_221$B_L_prop_last),"B_L_prop_last"] <- 0


####  222
G_222_last <- G_222 %>% group_by(pltID) %>% arrange(INVYR) %>% slice(n())
G_222_first <- G_222 %>% group_by(pltID) %>% arrange(INVYR) %>% slice(1)

DeltaB_df_222 <- data.frame("pltID_first" = G_222_first$pltID, 
                            "MEASTIME_t2_first" = G_222_first$MEASTIME_t2, 
                            "STDAGE_t2_first" = G_222_first$STDAGE_t2,
                            "B_L_prop_first" = G_222_first$B_L_prop,
                            "B_plt_t2_MgHa_first" = G_222_first$B_plt_t2_MgHa, 
                            
                            "pltID_last" = G_222_last$pltID,
                            "MEASTIME_t2_last" = G_222_last$MEASTIME_t2, 
                            "STDAGE_t2_last" = G_222_last$STDAGE_t2,
                            "B_L_prop_last" = G_222_last$B_L_prop,
                            "B_plt_t2_MgHa_last" = G_222_last$B_plt_t2_MgHa
                            )

DeltaB_df_222$DeltaB_total <- (G_222_last$B_plt_t2_MgHa - G_222_first$B_plt_t2_MgHa) / (G_222_last$MEASTIME_t2 - G_222_first$MEASTIME_t2)

## overwrite NaNs with zero -- all NaN B_L_props have B_t1 (biomass at t1) = 0
DeltaB_df_222[is.nan(DeltaB_df_222$B_L_prop_first),"B_L_prop_first"] <- 0
DeltaB_df_222[is.nan(DeltaB_df_222$B_L_prop_last),"B_L_prop_last"] <- 0


####  223
G_223_last <- G_223 %>% group_by(pltID) %>% arrange(INVYR) %>% slice(n())
G_223_first <- G_223 %>% group_by(pltID) %>% arrange(INVYR) %>% slice(1)

DeltaB_df_223 <- data.frame("pltID_first" = G_223_first$pltID, 
                            "MEASTIME_t2_first" = G_223_first$MEASTIME_t2, 
                            "STDAGE_t2_first" = G_223_first$STDAGE_t2,
                            "B_L_prop_first" = G_223_first$B_L_prop,
                            "B_plt_t2_MgHa_first" = G_223_first$B_plt_t2_MgHa, 
                            
                            "pltID_last" = G_223_last$pltID,
                            "MEASTIME_t2_last" = G_223_last$MEASTIME_t2, 
                            "STDAGE_t2_last" = G_223_last$STDAGE_t2,
                            "B_L_prop_last" = G_223_last$B_L_prop,
                            "B_plt_t2_MgHa_last" = G_223_last$B_plt_t2_MgHa
                            )

DeltaB_df_223$DeltaB_total <- (G_223_last$B_plt_t2_MgHa - G_223_first$B_plt_t2_MgHa) / (G_223_last$MEASTIME_t2 - G_223_first$MEASTIME_t2)

## overwrite NaNs with zero -- all NaN B_L_props have B_t1 (biomass at t1) = 0
DeltaB_df_223[is.nan(DeltaB_df_223$B_L_prop_first),"B_L_prop_first"] <- 0
DeltaB_df_223[is.nan(DeltaB_df_223$B_L_prop_last),"B_L_prop_last"] <- 0


####  231
G_231_last <- G_231 %>% group_by(pltID) %>% arrange(INVYR) %>% slice(n())
G_231_first <- G_231 %>% group_by(pltID) %>% arrange(INVYR) %>% slice(1)

DeltaB_df_231 <- data.frame("pltID_first" = G_231_first$pltID, 
                            "MEASTIME_t2_first" = G_231_first$MEASTIME_t2, 
                            "STDAGE_t2_first" = G_231_first$STDAGE_t2,
                            "B_L_prop_first" = G_231_first$B_L_prop,
                            "B_plt_t2_MgHa_first" = G_231_first$B_plt_t2_MgHa, 
                            
                            "pltID_last" = G_231_last$pltID,
                            "MEASTIME_t2_last" = G_231_last$MEASTIME_t2, 
                            "STDAGE_t2_last" = G_231_last$STDAGE_t2,
                            "B_L_prop_last" = G_231_last$B_L_prop,
                            "B_plt_t2_MgHa_last" = G_231_last$B_plt_t2_MgHa
                            )

DeltaB_df_231$DeltaB_total <- (G_231_last$B_plt_t2_MgHa - G_231_first$B_plt_t2_MgHa) / (G_231_last$MEASTIME_t2 - G_231_first$MEASTIME_t2)

## overwrite NaNs with zero -- all NaN B_L_props have B_t1 (biomass at t1) = 0
DeltaB_df_231[is.nan(DeltaB_df_231$B_L_prop_first),"B_L_prop_first"] <- 0
DeltaB_df_231[is.nan(DeltaB_df_231$B_L_prop_last),"B_L_prop_last"] <- 0


####  232
G_232_last <- G_232 %>% group_by(pltID) %>% arrange(INVYR) %>% slice(n())
G_232_first <- G_232 %>% group_by(pltID) %>% arrange(INVYR) %>% slice(1)

DeltaB_df_232 <- data.frame("pltID_first" = G_232_first$pltID, 
                            "MEASTIME_t2_first" = G_232_first$MEASTIME_t2, 
                            "STDAGE_t2_first" = G_232_first$STDAGE_t2,
                            "B_L_prop_first" = G_232_first$B_L_prop,
                            "B_plt_t2_MgHa_first" = G_232_first$B_plt_t2_MgHa, 
                            
                            "pltID_last" = G_232_last$pltID,
                            "MEASTIME_t2_last" = G_232_last$MEASTIME_t2, 
                            "STDAGE_t2_last" = G_232_last$STDAGE_t2,
                            "B_L_prop_last" = G_232_last$B_L_prop,
                            "B_plt_t2_MgHa_last" = G_232_last$B_plt_t2_MgHa
                            )

DeltaB_df_232$DeltaB_total <- (G_232_last$B_plt_t2_MgHa - G_232_first$B_plt_t2_MgHa) / (G_232_last$MEASTIME_t2 - G_232_first$MEASTIME_t2)

## overwrite NaNs with zero -- all NaN B_L_props have B_t1 (biomass at t1) = 0
DeltaB_df_232[is.nan(DeltaB_df_232$B_L_prop_first),"B_L_prop_first"] <- 0
DeltaB_df_232[is.nan(DeltaB_df_232$B_L_prop_last),"B_L_prop_last"] <- 0


####  234
G_234_last <- G_234 %>% group_by(pltID) %>% arrange(INVYR) %>% slice(n())
G_234_first <- G_234 %>% group_by(pltID) %>% arrange(INVYR) %>% slice(1)

DeltaB_df_234 <- data.frame("pltID_first" = G_234_first$pltID, 
                            "MEASTIME_t2_first" = G_234_first$MEASTIME_t2, 
                            "STDAGE_t2_first"= G_234_first$STDAGE_t2,
                            "B_L_prop_first" = G_234_first$B_L_prop,
                            "B_plt_t2_MgHa_first" = G_234_first$B_plt_t2_MgHa, 
                            
                            "pltID_last" = G_234_last$pltID,
                            "MEASTIME_t2_last" = G_234_last$MEASTIME_t2, 
                            "STDAGE_t2_last" = G_234_last$STDAGE_t2,
                            "B_L_prop_last" = G_234_last$B_L_prop,
                            "B_plt_t2_MgHa_last" = G_234_last$B_plt_t2_MgHa
                            )

DeltaB_df_234$DeltaB_total <- (G_234_last$B_plt_t2_MgHa - G_234_first$B_plt_t2_MgHa) / (G_234_last$MEASTIME_t2 - G_234_first$MEASTIME_t2)

## overwrite NaNs with zero -- all NaN B_L_props have B_t1 (biomass at t1) = 0
DeltaB_df_234[is.nan(DeltaB_df_234$B_L_prop_first),"B_L_prop_first"] <- 0
DeltaB_df_234[is.nan(DeltaB_df_234$B_L_prop_last),"B_L_prop_last"] <- 0


####  251
G_251_last <- G_251 %>% group_by(pltID) %>% arrange(INVYR) %>% slice(n())
G_251_first <- G_251 %>% group_by(pltID) %>% arrange(INVYR) %>% slice(1)

DeltaB_df_251 <- data.frame("pltID_first" = G_251_first$pltID, 
                            "MEASTIME_t2_first" = G_251_first$MEASTIME_t2, 
                            "STDAGE_t2_first" = G_251_first$STDAGE_t2,
                            "B_L_prop_first" = G_251_first$B_L_prop,
                            "B_plt_t2_MgHa_first" = G_251_first$B_plt_t2_MgHa, 
                            
                            "pltID_last" = G_251_last$pltID,
                            "MEASTIME_t2_last" = G_251_last$MEASTIME_t2, 
                            "STDAGE_t2_last" = G_251_last$STDAGE_t2,
                            "B_L_prop_last" = G_251_last$B_L_prop,
                            "B_plt_t2_MgHa_last" = G_251_last$B_plt_t2_MgHa
                            )

DeltaB_df_251$DeltaB_total <- (G_251_last$B_plt_t2_MgHa - G_251_first$B_plt_t2_MgHa) / (G_251_last$MEASTIME_t2 - G_251_first$MEASTIME_t2)

## overwrite NaNs with zero -- all NaN B_L_props have B_t1 (biomass at t1) = 0
DeltaB_df_251[is.nan(DeltaB_df_251$B_L_prop_first),"B_L_prop_first"] <- 0
DeltaB_df_251[is.nan(DeltaB_df_251$B_L_prop_last),"B_L_prop_last"] <- 0


####  255
G_255_last <- G_255 %>% group_by(pltID) %>% arrange(INVYR) %>% slice(n())
G_255_first <- G_255 %>% group_by(pltID) %>% arrange(INVYR) %>% slice(1)

DeltaB_df_255 <- data.frame("pltID_first" = G_255_first$pltID, 
                            "MEASTIME_t2_first" = G_255_first$MEASTIME_t2, 
                            "STDAGE_t2_first" = G_255_first$STDAGE_t2,
                            "B_L_prop_first" = G_255_first$B_L_prop,
                            "B_plt_t2_MgHa_first" = G_255_first$B_plt_t2_MgHa, 
                            
                            "pltID_last" = G_255_last$pltID,
                            "MEASTIME_t2_last" = G_255_last$MEASTIME_t2, 
                            "STDAGE_t2_last" = G_255_last$STDAGE_t2,
                            "B_L_prop_last" = G_255_last$B_L_prop,
                            "B_plt_t2_MgHa_last" = G_255_last$B_plt_t2_MgHa
                            )

DeltaB_df_255$DeltaB_total <- (G_255_last$B_plt_t2_MgHa - G_255_first$B_plt_t2_MgHa) / (G_255_last$MEASTIME_t2 - G_255_first$MEASTIME_t2)

## overwrite NaNs with zero -- all NaN B_L_props have B_t1 (biomass at t1) = 0
DeltaB_df_255[is.nan(DeltaB_df_255$B_L_prop_first),"B_L_prop_first"] <- 0
DeltaB_df_255[is.nan(DeltaB_df_255$B_L_prop_last),"B_L_prop_last"] <- 0


####  332
G_332_last <- G_332 %>% group_by(pltID) %>% arrange(INVYR) %>% slice(n())
G_332_first <- G_332 %>% group_by(pltID) %>% arrange(INVYR) %>% slice(1)

DeltaB_df_332 <- data.frame("pltID_first" = G_332_first$pltID, 
                            "MEASTIME_t2_first" = G_332_first$MEASTIME_t2, 
                            "STDAGE_t2_first" = G_332_first$STDAGE_t2,
                            "B_L_prop_first" = G_332_first$B_L_prop,
                            "B_plt_t2_MgHa_first" = G_332_first$B_plt_t2_MgHa, 
                            
                            "pltID_last" = G_332_last$pltID,
                            "MEASTIME_t2_last" = G_332_last$MEASTIME_t2, 
                            "STDAGE_t2_last" = G_332_last$STDAGE_t2,
                            "B_L_prop_last" = G_332_last$B_L_prop,
                            "B_plt_t2_MgHa_last" = G_332_last$B_plt_t2_MgHa
                            )

DeltaB_df_332$DeltaB_total <- (G_332_last$B_plt_t2_MgHa - G_332_first$B_plt_t2_MgHa) / (G_332_last$MEASTIME_t2 - G_332_first$MEASTIME_t2)

## overwrite NaNs with zero -- all NaN B_L_props have B_t1 (biomass at t1) = 0
DeltaB_df_332[is.nan(DeltaB_df_332$B_L_prop_first),"B_L_prop_first"] <- 0
DeltaB_df_332[is.nan(DeltaB_df_332$B_L_prop_last),"B_L_prop_last"] <- 0


####  M211
G_M211_last <- G_M211 %>% group_by(pltID) %>% arrange(INVYR) %>% slice(n())
G_M211_first <- G_M211 %>% group_by(pltID) %>% arrange(INVYR) %>% slice(1)

DeltaB_df_M211 <- data.frame("pltID_first" = G_M211_first$pltID, 
                            "MEASTIME_t2_first" = G_M211_first$MEASTIME_t2, 
                            "STDAGE_t2_first" = G_M211_first$STDAGE_t2,
                            "B_L_prop_first" = G_M211_first$B_L_prop,
                            "B_plt_t2_MgHa_first" = G_M211_first$B_plt_t2_MgHa, 
                            
                            "pltID_last" = G_M211_last$pltID,
                            "MEASTIME_t2_last" = G_M211_last$MEASTIME_t2, 
                            "STDAGE_t2_last" = G_M211_last$STDAGE_t2,
                            "B_L_prop_last" = G_M211_last$B_L_prop,
                            "B_plt_t2_MgHa_last" = G_M211_last$B_plt_t2_MgHa
                            )

DeltaB_df_M211$DeltaB_total <- (G_M211_last$B_plt_t2_MgHa - G_M211_first$B_plt_t2_MgHa) / (G_M211_last$MEASTIME_t2 - G_M211_first$MEASTIME_t2)

## overwrite NaNs with zero -- all NaN B_L_props have B_t1 (biomass at t1) = 0
DeltaB_df_M211[is.nan(DeltaB_df_M211$B_L_prop_first),"B_L_prop_first"] <- 0
DeltaB_df_M211[is.nan(DeltaB_df_M211$B_L_prop_last),"B_L_prop_last"] <- 0


####  M221
G_M221_last <- G_M221 %>% group_by(pltID) %>% arrange(INVYR) %>% slice(n())
G_M221_first <- G_M221 %>% group_by(pltID) %>% arrange(INVYR) %>% slice(1)

DeltaB_df_M221 <- data.frame("pltID_first" = G_M221_first$pltID, 
                            "MEASTIME_t2_first" = G_M221_first$MEASTIME_t2, 
                            "STDAGE_t2_first" = G_M221_first$STDAGE_t2,
                            "B_L_prop_first" = G_M221_first$B_L_prop,
                            "B_plt_t2_MgHa_first" = G_M221_first$B_plt_t2_MgHa, 
                            
                            "pltID_last" = G_M221_last$pltID,
                            "MEASTIME_t2_last" = G_M221_last$MEASTIME_t2, 
                            "STDAGE_t2_last" = G_M221_last$STDAGE_t2,
                            "B_L_prop_last" = G_M221_last$B_L_prop,
                            "B_plt_t2_MgHa_last" = G_M221_last$B_plt_t2_MgHa
                            )

DeltaB_df_M221$DeltaB_total <- (G_M221_last$B_plt_t2_MgHa - G_M221_first$B_plt_t2_MgHa) / (G_M221_last$MEASTIME_t2 - G_M221_first$MEASTIME_t2)

## overwrite NaNs with zero -- all NaN B_L_props have B_t1 (biomass at t1) = 0
DeltaB_df_M221[is.nan(DeltaB_df_M221$B_L_prop_first),"B_L_prop_first"] <- 0
DeltaB_df_M221[is.nan(DeltaB_df_M221$B_L_prop_last),"B_L_prop_last"] <- 0


####  M223
G_M223_last <- G_M223 %>% group_by(pltID) %>% arrange(INVYR) %>% slice(n())
G_M223_first <- G_M223 %>% group_by(pltID) %>% arrange(INVYR) %>% slice(1)

DeltaB_df_M223 <- data.frame("pltID_first" = G_M223_first$pltID, 
                            "MEASTIME_t2_first" = G_M223_first$MEASTIME_t2, 
                            "STDAGE_t2_first" = G_M223_first$STDAGE_t2,
                            "B_L_prop_first" = G_M223_first$B_L_prop,
                            "B_plt_t2_MgHa_first" = G_M223_first$B_plt_t2_MgHa, 
                            
                            "pltID_last" = G_M223_last$pltID,
                            "MEASTIME_t2_last" = G_M223_last$MEASTIME_t2, 
                            "STDAGE_t2_last" = G_M223_last$STDAGE_t2,
                            "B_L_prop_last" = G_M223_last$B_L_prop,
                            "B_plt_t2_MgHa_last" = G_M223_last$B_plt_t2_MgHa
                            )

DeltaB_df_M223$DeltaB_total <- (G_M223_last$B_plt_t2_MgHa - G_M223_first$B_plt_t2_MgHa) / (G_M223_last$MEASTIME_t2 - G_M223_first$MEASTIME_t2)

## overwrite NaNs with zero -- all NaN B_L_props have B_t1 (biomass at t1) = 0
DeltaB_df_M223[is.nan(DeltaB_df_M223$B_L_prop_first),"B_L_prop_first"] <- 0
DeltaB_df_M223[is.nan(DeltaB_df_M223$B_L_prop_last),"B_L_prop_last"] <- 0


####  M231
G_M231_last <- G_M231 %>% group_by(pltID) %>% arrange(INVYR) %>% slice(n())
G_M231_first <- G_M231 %>% group_by(pltID) %>% arrange(INVYR) %>% slice(1)

DeltaB_df_M231 <- data.frame("pltID_first" = G_M231_first$pltID, 
                            "MEASTIME_t2_first" = G_M231_first$MEASTIME_t2, 
                            "STDAGE_t2_first" = G_M231_first$STDAGE_t2,
                            "B_L_prop_first" = G_M231_first$B_L_prop,
                            "B_plt_t2_MgHa_first" = G_M231_first$B_plt_t2_MgHa, 
                            
                            "pltID_last" = G_M231_last$pltID,
                            "MEASTIME_t2_last" = G_M231_last$MEASTIME_t2, 
                            "STDAGE_t2_last" = G_M231_last$STDAGE_t2,
                            "B_L_prop_last" = G_M231_last$B_L_prop,
                            "B_plt_t2_MgHa_last" = G_M231_last$B_plt_t2_MgHa
                            )

DeltaB_df_M231$DeltaB_total <- (G_M231_last$B_plt_t2_MgHa - G_M231_first$B_plt_t2_MgHa) / (G_M231_last$MEASTIME_t2 - G_M231_first$MEASTIME_t2)

## overwrite NaNs with zero -- all NaN B_L_props have B_t1 (biomass at t1) = 0
DeltaB_df_M231[is.nan(DeltaB_df_M231$B_L_prop_first),"B_L_prop_first"] <- 0
DeltaB_df_M231[is.nan(DeltaB_df_M231$B_L_prop_last),"B_L_prop_last"] <- 0


####  M261
G_M261_last <- G_M261 %>% group_by(pltID) %>% arrange(INVYR) %>% slice(n())
G_M261_first <- G_M261 %>% group_by(pltID) %>% arrange(INVYR) %>% slice(1)

DeltaB_df_M261 <- data.frame("pltID_first" = G_M261_first$pltID, 
                            "MEASTIME_t2_first" = G_M261_first$MEASTIME_t2, 
                            "STDAGE_t2_first" = G_M261_first$STDAGE_t2,
                            "B_L_prop_first" = G_M261_first$B_L_prop,
                            "B_plt_t2_MgHa_first" = G_M261_first$B_plt_t2_MgHa, 
                            
                            "pltID_last" = G_M261_last$pltID,
                            "MEASTIME_t2_last" = G_M261_last$MEASTIME_t2, 
                            "STDAGE_t2_last" = G_M261_last$STDAGE_t2,
                            "B_L_prop_last" = G_M261_last$B_L_prop,
                            "B_plt_t2_MgHa_last" = G_M261_last$B_plt_t2_MgHa
                            )

DeltaB_df_M261$DeltaB_total <- (G_M261_last$B_plt_t2_MgHa - G_M261_first$B_plt_t2_MgHa) / (G_M261_last$MEASTIME_t2 - G_M261_first$MEASTIME_t2)

## overwrite NaNs with zero -- all NaN B_L_props have B_t1 (biomass at t1) = 0
DeltaB_df_M261[is.nan(DeltaB_df_M261$B_L_prop_first),"B_L_prop_first"] <- 0
DeltaB_df_M261[is.nan(DeltaB_df_M261$B_L_prop_last),"B_L_prop_last"] <- 0


####  M334
G_M334_last <- G_M334 %>% group_by(pltID) %>% arrange(INVYR) %>% slice(n())
G_M334_first <- G_M334 %>% group_by(pltID) %>% arrange(INVYR) %>% slice(1)

DeltaB_df_M334 <- data.frame("pltID_first" = G_M334_first$pltID, 
                            "MEASTIME_t2_first" = G_M334_first$MEASTIME_t2, 
                            "STDAGE_t2_first" = G_M334_first$STDAGE_t2,
                            "B_L_prop_first" = G_M334_first$B_L_prop,
                            "B_plt_t2_MgHa_first" = G_M334_first$B_plt_t2_MgHa, 
                            
                            "pltID_last" = G_M334_last$pltID,
                            "MEASTIME_t2_last" = G_M334_last$MEASTIME_t2, 
                            "STDAGE_t2_last" = G_M334_last$STDAGE_t2,
                            "B_L_prop_last" = G_M334_last$B_L_prop,
                            "B_plt_t2_MgHa_last" = G_M334_last$B_plt_t2_MgHa
                            )

DeltaB_df_M334$DeltaB_total <- (G_M334_last$B_plt_t2_MgHa - G_M334_first$B_plt_t2_MgHa) / (G_M334_last$MEASTIME_t2 - G_M334_first$MEASTIME_t2)

## overwrite NaNs with zero -- all NaN B_L_props have B_t1 (biomass at t1) = 0
DeltaB_df_M334[is.nan(DeltaB_df_M334$B_L_prop_first),"B_L_prop_first"] <- 0
DeltaB_df_M334[is.nan(DeltaB_df_M334$B_L_prop_last),"B_L_prop_last"] <- 0
```
...
if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "211",]$Best.Mod == "NA"){
    
    DeltaB_df_211$B_pred_age2 <- predict(get(paste("nls_211.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 211,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_211$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_211$STDAGE_t2_last)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_211$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_211$STDAGE_t2_last,
                                                 "B_L_prop" = DeltaB_df_211$B_L_prop_first)
                                      } )
    
    DeltaB_df_211$B_pred_age1 <- predict(get(paste("nls_211.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 211,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_211$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_211$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_211$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_211$STDAGE_t2_first,
                                                 "B_L_prop" = DeltaB_df_211$B_L_prop_first)
    ...
    DeltaB_df_211$DeltaB_age <- (DeltaB_df_211$B_pred_age2 - DeltaB_df_211$B_pred_age1) / 
                                (DeltaB_df_211$MEASTIME_t2_last - DeltaB_df_211$MEASTIME_t2_first)
}
    
    
#### 212

if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "212",]$Best.Mod == "NA"){
    
    DeltaB_df_212$B_pred_age2 <- predict(get(paste("nls_212.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 212,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_212$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_212$STDAGE_t2_last)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_212$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_212$STDAGE_t2_last,
                                                 "B_L_prop" = DeltaB_df_212$B_L_prop_first)
                                      } )
    
    DeltaB_df_212$B_pred_age1 <- predict(get(paste("nls_212.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 212,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_212$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_212$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_212$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_212$STDAGE_t2_first,
                                                 "B_L_prop" = DeltaB_df_212$B_L_prop_first)
    ...
    # calculate delta-B due to delta STDAGE -- simple subtraction
    DeltaB_df_212$DeltaB_age <- (DeltaB_df_212$B_pred_age2 - DeltaB_df_212$B_pred_age1) / 
                                (DeltaB_df_212$MEASTIME_t2_last - DeltaB_df_212$MEASTIME_t2_first)
}

#### 221

if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "221",]$Best.Mod == "NA"){
    
    DeltaB_df_221$B_pred_age2 <- predict(get(paste("nls_221.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 221,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_221$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_221$STDAGE_t2_last)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_221$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_221$STDAGE_t2_last,
                                                 "B_L_prop" = DeltaB_df_221$B_L_prop_first)
                                      } )
    
    DeltaB_df_221$B_pred_age1 <- predict(get(paste("nls_221.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 221,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_221$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_221$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_221$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_221$STDAGE_t2_first,
                                                 "B_L_prop" = DeltaB_df_221$B_L_prop_first)
    ...
    # calculate delta-B due to delta STDAGE -- simple subtraction
    DeltaB_df_221$DeltaB_age <- (DeltaB_df_221$B_pred_age2 - DeltaB_df_221$B_pred_age1) / 
                                (DeltaB_df_221$MEASTIME_t2_last - DeltaB_df_221$MEASTIME_t2_first)
}

#### 222

if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "222",]$Best.Mod == "NA"){
    
    DeltaB_df_222$B_pred_age2 <- predict(get(paste("nls_222.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 222,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_222$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_222$STDAGE_t2_last)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_222$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_222$STDAGE_t2_last,
                                                 "B_L_prop" = DeltaB_df_222$B_L_prop_first)
                                      } )
    
    DeltaB_df_222$B_pred_age1 <- predict(get(paste("nls_222.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 222,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_222$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_222$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_222$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_222$STDAGE_t2_first,
                                                 "B_L_prop" = DeltaB_df_222$B_L_prop_first)
    ...
    # calculate delta-B due to delta STDAGE -- simple subtraction
    DeltaB_df_222$DeltaB_age <- (DeltaB_df_222$B_pred_age2 - DeltaB_df_222$B_pred_age1) / 
                                (DeltaB_df_222$MEASTIME_t2_last - DeltaB_df_222$MEASTIME_t2_first)
}

#### 223

if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "223",]$Best.Mod == "NA"){
    
    DeltaB_df_223$B_pred_age2 <- predict(get(paste("nls_223.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 223,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_223$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_223$STDAGE_t2_last)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_223$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_223$STDAGE_t2_last,
                                                 "B_L_prop" = DeltaB_df_223$B_L_prop_first)
                                      } )
    
    DeltaB_df_223$B_pred_age1 <- predict(get(paste("nls_223.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 223,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_223$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_223$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_223$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_223$STDAGE_t2_first,
                                                 "B_L_prop" = DeltaB_df_223$B_L_prop_first)
    ...
    # calculate delta-B due to delta STDAGE -- simple subtraction
    DeltaB_df_223$DeltaB_age <- (DeltaB_df_223$B_pred_age2 - DeltaB_df_223$B_pred_age1) / 
                                (DeltaB_df_223$MEASTIME_t2_last - DeltaB_df_223$MEASTIME_t2_first)
}

#### 231

if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "231",]$Best.Mod == "NA"){
    
    DeltaB_df_231$B_pred_age2 <- predict(get(paste("nls_231.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 231,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_231$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_231$STDAGE_t2_last)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_231$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_231$STDAGE_t2_last,
                                                 "B_L_prop" = DeltaB_df_231$B_L_prop_first)
                                      } )
    
    DeltaB_df_231$B_pred_age1 <- predict(get(paste("nls_231.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 231,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_231$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_231$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_231$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_231$STDAGE_t2_first,
                                                 "B_L_prop" = DeltaB_df_231$B_L_prop_first)
    ...
    # calculate delta-B due to delta STDAGE -- simple subtraction
    DeltaB_df_231$DeltaB_age <- (DeltaB_df_231$B_pred_age2 - DeltaB_df_231$B_pred_age1) / 
                                (DeltaB_df_231$MEASTIME_t2_last - DeltaB_df_231$MEASTIME_t2_first)
}

#### 232

if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "232",]$Best.Mod == "NA"){
    
    DeltaB_df_232$B_pred_age2 <- predict(get(paste("nls_232.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 232,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_232$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_232$STDAGE_t2_last)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_232$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_232$STDAGE_t2_last,
                                                 "B_L_prop" = DeltaB_df_232$B_L_prop_first)
                                      } )
    
    DeltaB_df_232$B_pred_age1 <- predict(get(paste("nls_232.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 232,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_232$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_232$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_232$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_232$STDAGE_t2_first,
                                                 "B_L_prop" = DeltaB_df_232$B_L_prop_first)
    ...
    # calculate delta-B due to delta STDAGE -- simple subtraction
    DeltaB_df_232$DeltaB_age <- (DeltaB_df_232$B_pred_age2 - DeltaB_df_232$B_pred_age1) / 
                                (DeltaB_df_232$MEASTIME_t2_last - DeltaB_df_232$MEASTIME_t2_first)
}

#### 234

if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "234",]$Best.Mod == "NA"){
    
    DeltaB_df_234$B_pred_age2 <- predict(get(paste("nls_234.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 234,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_234$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_234$STDAGE_t2_last)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_234$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_234$STDAGE_t2_last,
                                                 "B_L_prop" = DeltaB_df_234$B_L_prop_first)
                                      } )
    
    DeltaB_df_234$B_pred_age1 <- predict(get(paste("nls_234.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 234,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_234$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_234$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_234$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_234$STDAGE_t2_first,
                                                 "B_L_prop" = DeltaB_df_234$B_L_prop_first)
    ...
    # calculate delta-B due to delta STDAGE -- simple subtraction
    DeltaB_df_234$DeltaB_age <- (DeltaB_df_234$B_pred_age2 - DeltaB_df_234$B_pred_age1) / 
                                (DeltaB_df_234$MEASTIME_t2_last - DeltaB_df_234$MEASTIME_t2_first)
}

#### 251

if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "251",]$Best.Mod == "NA"){
    
    DeltaB_df_251$B_pred_age2 <- predict(get(paste("nls_251.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 251,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_251$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_251$STDAGE_t2_last)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_251$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_251$STDAGE_t2_last,
                                                 "B_L_prop" = DeltaB_df_251$B_L_prop_first)
                                      } )
    
    DeltaB_df_251$B_pred_age1 <- predict(get(paste("nls_251.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 251,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_251$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_251$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_251$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_251$STDAGE_t2_first,
                                                 "B_L_prop" = DeltaB_df_251$B_L_prop_first)
    ...
    # calculate delta-B due to delta STDAGE -- simple subtraction
    DeltaB_df_251$DeltaB_age <- (DeltaB_df_251$B_pred_age2 - DeltaB_df_251$B_pred_age1) / 
                                (DeltaB_df_251$MEASTIME_t2_last - DeltaB_df_251$MEASTIME_t2_first)
}

#### 255

if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "255",]$Best.Mod == "NA"){
    
    DeltaB_df_255$B_pred_age2 <- predict(get(paste("nls_255.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 255,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_255$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_255$STDAGE_t2_last)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_255$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_255$STDAGE_t2_last,
                                                 "B_L_prop" = DeltaB_df_255$B_L_prop_first)
                                      } )
    
    DeltaB_df_255$B_pred_age1 <- predict(get(paste("nls_255.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 255,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_255$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_255$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_255$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_255$STDAGE_t2_first,
                                                 "B_L_prop" = DeltaB_df_255$B_L_prop_first)
    ...
    # calculate delta-B due to delta STDAGE -- simple subtraction
    DeltaB_df_255$DeltaB_age <- (DeltaB_df_255$B_pred_age2 - DeltaB_df_255$B_pred_age1) / 
                                (DeltaB_df_255$MEASTIME_t2_last - DeltaB_df_255$MEASTIME_t2_first)
}


#### 332

if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "332",]$Best.Mod == "NA"){
    
    DeltaB_df_332$B_pred_age2 <- predict(get(paste("nls_332.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 332,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_332$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_332$STDAGE_t2_last)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_332$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_332$STDAGE_t2_last,
                                                 "B_L_prop" = DeltaB_df_332$B_L_prop_first)
                                      } )
    
    DeltaB_df_332$B_pred_age1 <- predict(get(paste("nls_332.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 332,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_332$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_332$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_332$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_332$STDAGE_t2_first,
                                                 "B_L_prop" = DeltaB_df_332$B_L_prop_first)
    ...
    # calculate delta-B due to delta STDAGE -- simple subtraction
    DeltaB_df_332$DeltaB_age <- (DeltaB_df_332$B_pred_age2 - DeltaB_df_332$B_pred_age1) / 
                                (DeltaB_df_332$MEASTIME_t2_last - DeltaB_df_332$MEASTIME_t2_first)
}


#### M211

if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M211",]$Best.Mod == "NA"){
    
    DeltaB_df_M211$B_pred_age2 <- predict(get(paste("nls_M211.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M211",]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_M211$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_M211$STDAGE_t2_last)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_M211$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_M211$STDAGE_t2_last,
                                                 "B_L_prop" = DeltaB_df_M211$B_L_prop_first)
                                      } )
    
    DeltaB_df_M211$B_pred_age1 <- predict(get(paste("nls_M211.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M211",]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_M211$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_M211$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_M211$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_M211$STDAGE_t2_first,
                                                 "B_L_prop" = DeltaB_df_M211$B_L_prop_first)
    ...
    # calculate delta-B due to delta STDAGE -- simple subtraction
    DeltaB_df_M211$DeltaB_age <- (DeltaB_df_M211$B_pred_age2 - DeltaB_df_M211$B_pred_age1) / 
                                (DeltaB_df_M211$MEASTIME_t2_last - DeltaB_df_M211$MEASTIME_t2_first)
}

#### M221

if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M221",]$Best.Mod == "NA"){
    
    DeltaB_df_M221$B_pred_age2 <- predict(get(paste("nls_M221.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M221",]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_M221$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_M221$STDAGE_t2_last)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_M221$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_M221$STDAGE_t2_last,
                                                 "B_L_prop" = DeltaB_df_M221$B_L_prop_first)
                                      } )
    
    DeltaB_df_M221$B_pred_age1 <- predict(get(paste("nls_M221.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M221",]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_M221$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_M221$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_M221$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_M221$STDAGE_t2_first,
                                                 "B_L_prop" = DeltaB_df_M221$B_L_prop_first)
    ...
    # calculate delta-B due to delta STDAGE -- simple subtraction
    DeltaB_df_M221$DeltaB_age <- (DeltaB_df_M221$B_pred_age2 - DeltaB_df_M221$B_pred_age1) / 
                                (DeltaB_df_M221$MEASTIME_t2_last - DeltaB_df_M221$MEASTIME_t2_first)
}


#### M223

if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M223",]$Best.Mod == "NA"){
    
    DeltaB_df_M223$B_pred_age2 <- predict(get(paste("nls_M223.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M223",]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_M223$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_M223$STDAGE_t2_last)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_M223$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_M223$STDAGE_t2_last,
                                                 "B_L_prop" = DeltaB_df_M223$B_L_prop_first)
                                      } )
    
    DeltaB_df_M223$B_pred_age1 <- predict(get(paste("nls_M223.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M223",]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_M223$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_M223$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_M223$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_M223$STDAGE_t2_first,
                                                 "B_L_prop" = DeltaB_df_M223$B_L_prop_first)
    ...
    # calculate delta-B due to delta STDAGE -- simple subtraction
    DeltaB_df_M223$DeltaB_age <- (DeltaB_df_M223$B_pred_age2 - DeltaB_df_M223$B_pred_age1) / 
                                (DeltaB_df_M223$MEASTIME_t2_last - DeltaB_df_M223$MEASTIME_t2_first)
}

#### M231

if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M231",]$Best.Mod == "NA"){
    
    DeltaB_df_M231$B_pred_age2 <- predict(get(paste("nls_M231.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M231",]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_M231$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_M231$STDAGE_t2_last)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_M231$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_M231$STDAGE_t2_last,
                                                 "B_L_prop" = DeltaB_df_M231$B_L_prop_first)
                                      } )
    
    DeltaB_df_M231$B_pred_age1 <- predict(get(paste("nls_M231.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M231",]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_M231$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_M231$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_M231$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_M231$STDAGE_t2_first,
                                                 "B_L_prop" = DeltaB_df_M231$B_L_prop_first)
    ...
    # calculate delta-B due to delta STDAGE -- simple subtraction
    DeltaB_df_M231$DeltaB_age <- (DeltaB_df_M231$B_pred_age2 - DeltaB_df_M231$B_pred_age1) / 
                                (DeltaB_df_M231$MEASTIME_t2_last - DeltaB_df_M231$MEASTIME_t2_first)
}

#### M261

if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M261",]$Best.Mod == "NA"){
    
    DeltaB_df_M261$B_pred_age2 <- predict(get(paste("nls_M261.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M261",]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_M261$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_M261$STDAGE_t2_last)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_M261$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_M261$STDAGE_t2_last,
                                                 "B_L_prop" = DeltaB_df_M261$B_L_prop_first)
                                      } )
    
    DeltaB_df_M261$B_pred_age1 <- predict(get(paste("nls_M261.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M261",]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_M261$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_M261$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_M261$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_M261$STDAGE_t2_first,
                                                 "B_L_prop" = DeltaB_df_M261$B_L_prop_first)
    ...
    # calculate delta-B due to delta STDAGE -- simple subtraction
    DeltaB_df_M261$DeltaB_age <- (DeltaB_df_M261$B_pred_age2 - DeltaB_df_M261$B_pred_age1) / 
                                (DeltaB_df_M261$MEASTIME_t2_last - DeltaB_df_M261$MEASTIME_t2_first)
}

#### M334

if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M334",]$Best.Mod == "NA") {
    
    DeltaB_df_M334$B_pred_age2 <- predict(get(paste("nls_M334.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M334",]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_M334$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_M334$STDAGE_t2_last)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_M334$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_M334$STDAGE_t2_last,
                                                 "B_L_prop" = DeltaB_df_M334$B_L_prop_first)
                                      } )
    
    DeltaB_df_M334$B_pred_age1 <- predict(get(paste("nls_M334.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M334",]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_M334$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_M334$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_M334$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_M334$STDAGE_t2_first,
                                                 "B_L_prop" = DeltaB_df_M334$B_L_prop_first)
    ...
    # calculate delta-B due to delta STDAGE -- simple subtraction
    DeltaB_df_M334$DeltaB_age <- (DeltaB_df_M334$B_pred_age2 - DeltaB_df_M334$B_pred_age1) / 
                                (DeltaB_df_M334$MEASTIME_t2_last - DeltaB_df_M334$MEASTIME_t2_first)
}
...
```{r}
## function which uses f(age_first)
predict_f.age_first <- function(X, mod.num, data) {
  ## if loop to solve function given STDAGE_t1 data
  if (mod.num == 5) {
    tau = X["tau"]
    alpha = X["alpha"]
    a = X["a"]
    ...
    c = X["c"]
    d = X["d"]
    B <- (1 + (data$MEASTIME_t2_first-1990)*tau/100) * (1-alpha*data$B_L_prop_first) * 
          (a+b*exp(-((log((data$STDAGE_t2_first/c)))/d)^2))
  } else if (mod.num == 2) {
    tau = X["tau"]
    alpha = X["alpha"]
    A = X["A"]
    k = X["k"]
    B <- (1 + (data$MEASTIME_t2_first-1990)*tau/100) * (1-alpha*data$B_L_prop_first) * 
          A*data$STDAGE_t2_first / (k+data$STDAGE_t2_first)
  } else if (mod.num == "2a") {
    tau = X["tau"]
    alpha = X["alpha"]
    A = X["A"]
    ...
    p = X["p"]
    B <- (1 + (data$MEASTIME_t2_first-1990)*tau/100) * (1-alpha*data$B_L_prop_first) * 
          (p*A+((1-p)*A*data$STDAGE_t2_first / (k+data$STDAGE_t2_first)))
  } else if (mod.num == "2b") {
    tau = X["tau"]
    alpha = X["alpha"]
    A = X["A"]
    ...
    s = X["s"]
    B <- (1 + (data$MEASTIME_t2_first-1990)*tau/100) * (1-alpha*data$B_L_prop_first) *
          (A*data$STDAGE_t2_first^s / (k^s+data$STDAGE_t2_first^s))
  } else if (mod.num == "2c") {
    tau = X["tau"]
    alpha = X["alpha"]
    A = X["A"]
    ...
    s = X["s"]
    B <- (1 + (data$MEASTIME_t2_first-1990)*tau/100) * (1-alpha*data$B_L_prop_first) *
          (p*A+((1-p)*A*data$STDAGE_t2_first^s / (k^s+data$STDAGE_t2_first^s)))    
  } ## end if 
  return(B)
} ## end function

## for f(age_last)
predict_f.age_last <- function(X, mod.num, data) {
  ## if loop to solve function given STDAGE_t2 data
  if (mod.num == 5) {
    tau = X["tau"]
    alpha = X["alpha"]
    a = X["a"]
    ...
    d = X["d"]
    B <- (1 + (data$MEASTIME_t2_first-1990)*tau/100) * (1-alpha*data$B_L_prop_first) *
          (a+b*exp(-((log((data$STDAGE_t2_last/c)))/d)^2))
  } else if (mod.num == 2) {
    tau = X["tau"]
    alpha = X["alpha"]
    A = X["A"]
    k = X["k"]
    B <- (1 + (data$MEASTIME_t2_first-1990)*tau/100) * (1-alpha*data$B_L_prop_first) *
          A*data$STDAGE_t2_last / (k+data$STDAGE_t2_last)
  } else if (mod.num == "2a") {
    tau = X["tau"]
    alpha = X["alpha"]
    A = X["A"]
    ...
    p = X["p"]
    B <- (1 + (data$MEASTIME_t2_first-1990)*tau/100) * (1-alpha*data$B_L_prop_first) *
          (p*A+((1-p)*A*data$STDAGE_t2_last / (k+data$STDAGE_t2_last)))
  } else if (mod.num == "2b") {
    tau = X["tau"]
    alpha = X["alpha"]
    A = X["A"]
    ...
    s = X["s"]
    B <- (1 + (data$MEASTIME_t2_first-1990)*tau/100) * (1-alpha*data$B_L_prop_first) *
          (A*data$STDAGE_t2_last^s / (k^s+data$STDAGE_t2_last^s))
  } else if (mod.num == "2c") {
    tau = X["tau"]
    alpha = X["alpha"]
    A = X["A"]
    ...
    s = X["s"]
    B <- (1 + (data$MEASTIME_t2_first-1990)*tau/100) * (1-alpha*data$B_L_prop_first) *   
          (p*A+((1-p)*A*data$STDAGE_t2_last^s / (k^s+data$STDAGE_t2_last^s)))    
  } ## end if 
  return(B)
} ## end function
...
```{r}
## make a function to get parameters and predict B
predict_sim_DeltaB_age <- function(ecoprov) {
  
  ecoprov = paste(ecoprov, sep = "")
  Best_mods.P_bal <-  get("Best_mods.P_bal", envir = .GlobalEnv)
  vcov_list <- get("vcov_list", envir = .GlobalEnv)
  
  mod.num <- Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == paste(ecoprov, sep = ""),]$Best.Mod
  
  mod <- get(paste("nls_", paste(ecoprov, sep =""), ".",  mod.num, sep =""), envir = .GlobalEnv)
  
  data <- get(paste("DeltaB_df_", paste(ecoprov, sep =""), sep = ""), envir = .GlobalEnv)
  vcov_mat <- vcov_list[[paste(ecoprov, sep ="")]]
  
  
  ### sample parameters from MVNormal dist
  set.seed(77) ## for reproducibility
  parameters <- MASS::mvrnorm(nsamp, summary(mod)$param[,"Estimate"], vcov_mat)
  
  #### overwrite alpha and tau with MLE values from nls 
  parameters[,"alpha"] <- rep(summary(mod)$parameters["alpha","Estimate"], nsamp)
  parameters[,"tau"] <- rep(summary(mod)$parameters["tau","Estimate"], nsamp)
  
  ### solve f(age) function at age_first and age_last
  sim_f_age_first <- apply(parameters, 1, predict_f.age_first, data = data, mod.num = mod.num,
                           simplify = T)
  sim_f_age_last <- apply(parameters, 1, predict_f.age_last, data = data, mod.num = mod.num,
                          simplify = T)
  
  ### take difference in f(age_first) and f(age_last) over deltaT (aka MEASTIME_t2_last - MEASTIME_t2_first)  --  need to transform the numerator to get plots in rows and sim samples down columns -- then can do division 
  sim_DeltaB <- t(sim_f_age_last - sim_f_age_first) / (data$MEASTIME_t2_last - data$MEASTIME_t2_first) 
  
  ## take means
  res <- rowMeans(sim_DeltaB, na.rm = T)
...
```{r, warning = FALSE}
## the number of samples to work with
nsamp = 9999

## make a empty list
B_sim_pred_f_age_bar <- list()

############################################################################################################
## PREDICT f(age) for each ecoprovince 
############################################################################################################

## store in a list

#### 211
B_sim_pred_f_age_bar[["211"]] <- predict_sim_DeltaB_age(211)

####  212
B_sim_pred_f_age_bar[["212"]] <- predict_sim_DeltaB_age(212)

####  221
B_sim_pred_f_age_bar[["221"]] <- predict_sim_DeltaB_age(211)

####  222
B_sim_pred_f_age_bar[["222"]] <- predict_sim_DeltaB_age(222)

####  223
B_sim_pred_f_age_bar[["223"]] <- predict_sim_DeltaB_age(223)

####  231
B_sim_pred_f_age_bar[["231"]] <- predict_sim_DeltaB_age(231)

####  232
B_sim_pred_f_age_bar[["232"]] <- predict_sim_DeltaB_age(232)

####  234
B_sim_pred_f_age_bar[["234"]] <- predict_sim_DeltaB_age(234)

####  251
B_sim_pred_f_age_bar[["251"]] <- predict_sim_DeltaB_age(251)

####  255
B_sim_pred_f_age_bar[["255"]] <- predict_sim_DeltaB_age(255)

####  M211
B_sim_pred_f_age_bar[["M211"]] <- predict_sim_DeltaB_age("M211")

####  M221
B_sim_pred_f_age_bar[["M221"]] <- predict_sim_DeltaB_age("M221")

####  M223
B_sim_pred_f_age_bar[["M223"]] <- predict_sim_DeltaB_age("M223")

####  M231
B_sim_pred_f_age_bar[["M231"]] <- predict_sim_DeltaB_age("M231")
...
if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "211",]$Best.Mod == "NA"){
    ...
    DeltaB_df_211$B_pred_year1 <- predict(get(paste("nls_211.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 211,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_211$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_211$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
    ...
    DeltaB_df_211$B_pred_year2 <- predict(get(paste("nls_211.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 211,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_211$MEASTIME_t2_last,
                                               "STDAGE_t2" = DeltaB_df_211$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_211$MEASTIME_t2_last,
                                                 "STDAGE_t2" =  DeltaB_df_211$STDAGE_t2_first,
    ...
    DeltaB_df_211$DeltaB_year<- (DeltaB_df_211$B_pred_year2 - DeltaB_df_211$B_pred_year1) / 
                                (DeltaB_df_211$MEASTIME_t2_last - DeltaB_df_211$MEASTIME_t2_first)
}
...
if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "212",]$Best.Mod == "NA"){
    ...
    DeltaB_df_212$B_pred_year1 <- predict(get(paste("nls_212.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 212,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_212$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_212$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
    ...
    DeltaB_df_212$B_pred_year2 <- predict(get(paste("nls_212.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 212,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_212$MEASTIME_t2_last,
                                               "STDAGE_t2" = DeltaB_df_212$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_212$MEASTIME_t2_last,
                                                 "STDAGE_t2" =  DeltaB_df_212$STDAGE_t2_first,
    ...
    # calculate delta-B due to year (ge) -- simple subtraction
    DeltaB_df_212$DeltaB_year<- (DeltaB_df_212$B_pred_year2 - DeltaB_df_212$B_pred_year1) / 
                                (DeltaB_df_212$MEASTIME_t2_last - DeltaB_df_212$MEASTIME_t2_first)
}
...
if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "221",]$Best.Mod == "NA"){
    ...
    DeltaB_df_221$B_pred_year1 <- predict(get(paste("nls_221.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 221,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_221$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_221$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
    ...
    DeltaB_df_221$B_pred_year2 <- predict(get(paste("nls_221.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 221,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_221$MEASTIME_t2_last,
                                               "STDAGE_t2" = DeltaB_df_221$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_221$MEASTIME_t2_last,
                                                 "STDAGE_t2" =  DeltaB_df_221$STDAGE_t2_first,
    ...
    # calculate delta-B due to year (ge) -- simple subtraction
    DeltaB_df_221$DeltaB_year<- (DeltaB_df_221$B_pred_year2 - DeltaB_df_221$B_pred_year1) / 
                                (DeltaB_df_221$MEASTIME_t2_last - DeltaB_df_221$MEASTIME_t2_first)
}
...
if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "222",]$Best.Mod == "NA"){
    ...
    DeltaB_df_222$B_pred_year1 <- predict(get(paste("nls_222.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 222,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_222$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_222$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
    ...
    DeltaB_df_222$B_pred_year2 <- predict(get(paste("nls_222.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 222,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_222$MEASTIME_t2_last,
                                               "STDAGE_t2" = DeltaB_df_222$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_222$MEASTIME_t2_last,
                                                 "STDAGE_t2" =  DeltaB_df_222$STDAGE_t2_first,
    ...
    # calculate delta-B due to year (ge) -- simple subtraction
    DeltaB_df_222$DeltaB_year<- (DeltaB_df_222$B_pred_year2 - DeltaB_df_222$B_pred_year1) / 
                                (DeltaB_df_222$MEASTIME_t2_last - DeltaB_df_222$MEASTIME_t2_first)
}
...
if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "223",]$Best.Mod == "NA"){
    ...
    DeltaB_df_223$B_pred_year1 <- predict(get(paste("nls_223.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 223,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_223$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_223$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
    ...
    DeltaB_df_223$B_pred_year2 <- predict(get(paste("nls_223.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 223,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_223$MEASTIME_t2_last,
                                               "STDAGE_t2" = DeltaB_df_223$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_223$MEASTIME_t2_last,
                                                 "STDAGE_t2" =  DeltaB_df_223$STDAGE_t2_first,
    ...
    # calculate delta-B due to year (ge) -- simple subtraction
    DeltaB_df_223$DeltaB_year<- (DeltaB_df_223$B_pred_year2 - DeltaB_df_223$B_pred_year1) / 
                                (DeltaB_df_223$MEASTIME_t2_last - DeltaB_df_223$MEASTIME_t2_first)
}
...
if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "231",]$Best.Mod == "NA"){
    ...
    DeltaB_df_231$B_pred_year1 <- predict(get(paste("nls_231.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 231,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_231$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_231$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
    ...
    DeltaB_df_231$B_pred_year2 <- predict(get(paste("nls_231.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 231,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_231$MEASTIME_t2_last,
                                               "STDAGE_t2" = DeltaB_df_231$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_231$MEASTIME_t2_last,
                                                 "STDAGE_t2" =  DeltaB_df_231$STDAGE_t2_first,
    ...
    # calculate delta-B due to year (ge) -- simple subtraction
    DeltaB_df_231$DeltaB_year<- (DeltaB_df_231$B_pred_year2 - DeltaB_df_231$B_pred_year1) / 
                                (DeltaB_df_231$MEASTIME_t2_last - DeltaB_df_231$MEASTIME_t2_first)
}
...
if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "232",]$Best.Mod == "NA"){
    ...
    DeltaB_df_232$B_pred_year1 <- predict(get(paste("nls_232.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 232,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_232$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_232$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
    ...
    DeltaB_df_232$B_pred_year2 <- predict(get(paste("nls_232.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 232,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_232$MEASTIME_t2_last,
                                               "STDAGE_t2" = DeltaB_df_232$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_232$MEASTIME_t2_last,
                                                 "STDAGE_t2" =  DeltaB_df_232$STDAGE_t2_first,
    ...
    # calculate delta-B due to year (ge) -- simple subtraction
    DeltaB_df_232$DeltaB_year<- (DeltaB_df_232$B_pred_year2 - DeltaB_df_232$B_pred_year1) / 
                                (DeltaB_df_232$MEASTIME_t2_last - DeltaB_df_232$MEASTIME_t2_first)
}
...
if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "234",]$Best.Mod == "NA"){
    ...
    DeltaB_df_234$B_pred_year1 <- predict(get(paste("nls_234.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 234,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_234$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_234$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
    ...
    DeltaB_df_234$B_pred_year2 <- predict(get(paste("nls_234.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 234,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_234$MEASTIME_t2_last,
                                               "STDAGE_t2" = DeltaB_df_234$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_234$MEASTIME_t2_last,
                                                 "STDAGE_t2" =  DeltaB_df_234$STDAGE_t2_first,
    ...
    # calculate delta-B due to year (ge) -- simple subtraction
    DeltaB_df_234$DeltaB_year<- (DeltaB_df_234$B_pred_year2 - DeltaB_df_234$B_pred_year1) / 
                                (DeltaB_df_234$MEASTIME_t2_last - DeltaB_df_234$MEASTIME_t2_first)
}
...
if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "251",]$Best.Mod == "NA"){
    ...
    DeltaB_df_251$B_pred_year1 <- predict(get(paste("nls_251.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 251,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_251$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_251$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
    ...
    DeltaB_df_251$B_pred_year2 <- predict(get(paste("nls_251.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 251,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_251$MEASTIME_t2_last,
                                               "STDAGE_t2" = DeltaB_df_251$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_251$MEASTIME_t2_last,
                                                 "STDAGE_t2" =  DeltaB_df_251$STDAGE_t2_first,
    ...
    # calculate delta-B due to year (ge) -- simple subtraction
    DeltaB_df_251$DeltaB_year<- (DeltaB_df_251$B_pred_year2 - DeltaB_df_251$B_pred_year1) / 
                                (DeltaB_df_251$MEASTIME_t2_last - DeltaB_df_251$MEASTIME_t2_first)
}
...
if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "255",]$Best.Mod == "NA"){
    ...
    DeltaB_df_255$B_pred_year1 <- predict(get(paste("nls_255.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 255,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_255$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_255$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
    ...
    DeltaB_df_255$B_pred_year2 <- predict(get(paste("nls_255.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 255,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_255$MEASTIME_t2_last,
                                               "STDAGE_t2" = DeltaB_df_255$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_255$MEASTIME_t2_last,
                                                 "STDAGE_t2" =  DeltaB_df_255$STDAGE_t2_first,
    ...
    # calculate delta-B due to year (ge) -- simple subtraction
    DeltaB_df_255$DeltaB_year<- (DeltaB_df_255$B_pred_year2 - DeltaB_df_255$B_pred_year1) / 
                                (DeltaB_df_255$MEASTIME_t2_last - DeltaB_df_255$MEASTIME_t2_first)
}
...
if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M211",]$Best.Mod == "NA"){
    ...
    DeltaB_df_M211$B_pred_year1 <- predict(get(paste("nls_M211.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M211",]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_M211$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_M211$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
    ...
    DeltaB_df_M211$B_pred_year2 <- predict(get(paste("nls_M211.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M211",]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_M211$MEASTIME_t2_last,
                                               "STDAGE_t2" = DeltaB_df_M211$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_M211$MEASTIME_t2_last,
                                                 "STDAGE_t2" =  DeltaB_df_M211$STDAGE_t2_first,
    ...
    # calculate delta-B due to year (ge) -- simple subtraction
    DeltaB_df_M211$DeltaB_year<- (DeltaB_df_M211$B_pred_year2 - DeltaB_df_M211$B_pred_year1) / 
                                (DeltaB_df_M211$MEASTIME_t2_last - DeltaB_df_M211$MEASTIME_t2_first)
}
...
if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M221",]$Best.Mod == "NA"){
    ...
    DeltaB_df_M221$B_pred_year1 <- predict(get(paste("nls_M221.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M221",]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_M221$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_M221$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
    ...
    DeltaB_df_M221$B_pred_year2 <- predict(get(paste("nls_M221.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M221",]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_M221$MEASTIME_t2_last,
                                               "STDAGE_t2" = DeltaB_df_M221$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_M221$MEASTIME_t2_last,
                                                 "STDAGE_t2" =  DeltaB_df_M221$STDAGE_t2_first,
    ...
    # calculate delta-B due to year (ge) -- simple subtraction
    DeltaB_df_M221$DeltaB_year<- (DeltaB_df_M221$B_pred_year2 - DeltaB_df_M221$B_pred_year1) / 
                                (DeltaB_df_M221$MEASTIME_t2_last - DeltaB_df_M221$MEASTIME_t2_first)
}
...
if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M223",]$Best.Mod == "NA"){
    ...
    DeltaB_df_M223$B_pred_year1 <- predict(get(paste("nls_M223.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M223",]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_M223$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_M223$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
    ...
    DeltaB_df_M223$B_pred_year2 <- predict(get(paste("nls_M223.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M223",]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_M223$MEASTIME_t2_last,
                                               "STDAGE_t2" = DeltaB_df_M223$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_M223$MEASTIME_t2_last,
                                                 "STDAGE_t2" =  DeltaB_df_M223$STDAGE_t2_first,
    ...
    # calculate delta-B due to year (ge) -- simple subtraction
    DeltaB_df_M223$DeltaB_year<- (DeltaB_df_M223$B_pred_year2 - DeltaB_df_M223$B_pred_year1) /
                                (DeltaB_df_M223$MEASTIME_t2_last - DeltaB_df_M223$MEASTIME_t2_first)
}
...
if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M231",]$Best.Mod == "NA"){
    ...
    DeltaB_df_M231$B_pred_year1 <- predict(get(paste("nls_M231.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M231",]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_M231$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_M231$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
    ...
    DeltaB_df_M231$B_pred_year2 <- predict(get(paste("nls_M231.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M231",]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_M231$MEASTIME_t2_last,
                                               "STDAGE_t2" = DeltaB_df_M231$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_M231$MEASTIME_t2_last,
                                                 "STDAGE_t2" =  DeltaB_df_M231$STDAGE_t2_first,
    ...
    # calculate delta-B due to year (ge) -- simple subtraction
    DeltaB_df_M231$DeltaB_year<- (DeltaB_df_M231$B_pred_year2 - DeltaB_df_M231$B_pred_year1) / 
                                (DeltaB_df_M231$MEASTIME_t2_last - DeltaB_df_M231$MEASTIME_t2_first)
}
...
```{r}
## function which uses f(year_first)
predict_f.year_first <- function(X, mod.num, data) {
  ## if loop to solve function given MEASTIME_first data
  if (mod.num == 5) {
    tau = X["tau"]
    alpha = X["alpha"]
    a = X["a"]
...
## for f(year_last)
predict_f.year_last <- function(X, mod.num, data) {
  ## if loop to solve function given MEASTIME_last data
  if (mod.num == 5) {
    tau = X["tau"]
    alpha = X["alpha"]
    a = X["a"]
    ...
    c = X["c"]
    d = X["d"]
    B <- (1 + (data$MEASTIME_t2_last-1990)*tau/100) * (1-alpha*data$B_L_prop_first) *
          (a+b*exp(-((log((data$STDAGE_t2_first/c)))/d)^2))
  } else if (mod.num == 2) {
    tau = X["tau"]
    alpha = X["alpha"]
...
```{r}
### make a function to get parameters and predict B
predict_sim_DeltaB_year <- function(ecoprov) {
  ...
  set.seed(77) ## for reproducibility
  parameters <- if(mod.num == 2){
    matrix(data = c(rnorm(nsamp, summary(mod)$param["tau","Estimate"], sqrt(vcov_mat["tau","tau"])), ### sample parameters from Random Normal dist
                    rep(summary(mod)$param["alpha","Estimate"], 9999), 
                    rep(summary(mod)$param["A","Estimate"], 9999),
                    rep(summary(mod)$param["k","Estimate"], 9999)), nrow = 9999, ncol =4,
           dimnames = list(NULL, c("tau", "alpha", "A", "k")))
    } else if(mod.num == "2a"){
      matrix(data = c(rnorm(nsamp, summary(mod)$param["tau","Estimate"], sqrt(vcov_mat["tau","tau"])), ### sample parameters from Random Normal dist
                      rep(summary(mod)$param["alpha","Estimate"], 9999), 
                      rep(summary(mod)$param["A","Estimate"], 9999),
                      rep(summary(mod)$param["k","Estimate"], 9999), 
                      rep(summary(mod)$param["p","Estimate"], 9999),), nrow = 9999, ncol =5,
             dimnames = list(NULL, c("tau", "alpha", "A", "k", "p")))
      } else if(mod.num == "2b"){
        matrix(data = c(rnorm(nsamp, summary(mod)$param["tau","Estimate"], sqrt(vcov_mat["tau","tau"])), ### sample parameters from Random Normal dist
                        rep(summary(mod)$param["alpha","Estimate"], 9999), 
                        rep(summary(mod)$param["A","Estimate"], 9999),
                        rep(summary(mod)$param["k","Estimate"], 9999), 
                        rep(summary(mod)$param["s","Estimate"], 9999)), nrow = 9999, ncol =5,
                dimnames = list(NULL, c("tau", "alpha", "A", "k", "s")))
        } else if (mod.num == "2c"){
          matrix(data = c(rnorm(nsamp, summary(mod)$param["tau","Estimate"], sqrt(vcov_mat["tau","tau"])), ### sample parameters from Random Normal dist
                          rep(summary(mod)$param["alpha","Estimate"], 9999), 
                          rep(summary(mod)$param["A","Estimate"], 9999),
                          rep(summary(mod)$param["k","Estimate"], 9999), 
                          rep(summary(mod)$param["s","Estimate"], 9999), 
                          rep(summary(mod)$param["p","Estimate"], 9999)), nrow = 9999, ncol =6,
           dimnames = list(NULL, c("tau", "alpha", "A", "k", "s", "p")))
          } else if(mod.num == 5){
            matrix(data = c(rnorm(nsamp, summary(mod)$param["tau","Estimate"], sqrt(vcov_mat["tau","tau"])),
                            rep(summary(mod)$param["alpha","Estimate"], 9999), 
                            rep(summary(mod)$param["a","Estimate"], 9999),
                            rep(summary(mod)$param["b","Estimate"], 9999), 
                            rep(summary(mod)$param["c","Estimate"], 9999),
                            rep(summary(mod)$param["d","Estimate"], 9999)), nrow = 9999, ncol =6,
                   dimnames = list(NULL, c("tau", "alpha", "a", "b", "c", "d")))
              }
  
  ### solve f(year) function at year_first and year_last
  sim_f_year_first <- apply(parameters, 1, predict_f.year_first, data = data, mod.num = mod.num, 
                            simplify = TRUE)
  sim_f_year_last <- apply(parameters, 1, predict_f.year_last, data = data, mod.num = mod.num,
                          simplify = TRUE)
  
  ### take difference in f(year_first) and f(year_last) over deltaT (aka MEASTIME_t2_last - MEASTIME_t2_first)  --  need to transform the numerator to get plots in rows and sim samples down columns -- then can do division 
  sim_DeltaB <- t(sim_f_year_last - sim_f_year_first) / (data$MEASTIME_t2_last - data$MEASTIME_t2_first)
...
## make a empty list
B_sim_pred_f_year_bar <- list()

############################################################################################################
## PREDICT f(year) for each ecoprovince 
############################################################################################################

#### 211
B_sim_pred_f_year_bar[["211"]] <- predict_sim_DeltaB_year(211)

####  212
B_sim_pred_f_year_bar[["212"]] <- predict_sim_DeltaB_year(212)

####  221
B_sim_pred_f_year_bar[["221"]] <- predict_sim_DeltaB_year(221)

####  222
B_sim_pred_f_year_bar[["222"]] <- predict_sim_DeltaB_year(222)

####  223
B_sim_pred_f_year_bar[["223"]] <- predict_sim_DeltaB_year(223)

####  231
B_sim_pred_f_year_bar[["231"]] <- predict_sim_DeltaB_year(231)

####  232
B_sim_pred_f_year_bar[["232"]] <- predict_sim_DeltaB_year(232)

####  234
B_sim_pred_f_year_bar[["234"]] <- predict_sim_DeltaB_year(234)

####  251
B_sim_pred_f_year_bar[["251"]] <- predict_sim_DeltaB_year(251)

####  255
B_sim_pred_f_year_bar[["255"]] <- predict_sim_DeltaB_year(255)

####  M211
B_sim_pred_f_year_bar[["M211"]] <- predict_sim_DeltaB_year("M211")

####  M221
B_sim_pred_f_year_bar[["M221"]] <- predict_sim_DeltaB_year("M221")

####  M223
B_sim_pred_f_year_bar[["M223"]] <- predict_sim_DeltaB_year("M223")

####  M231
B_sim_pred_f_year_bar[["M231"]] <- predict_sim_DeltaB_year("M231")
...
if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "211",]$Best.Mod == "NA"){
    ...
    DeltaB_df_211$B_pred_disturbance1 <- predict(get(paste("nls_211.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 211,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_211$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_211$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
    ...
    DeltaB_df_211$B_pred_disturbance2 <- predict(get(paste("nls_211.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 211,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_211$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_211$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_211$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_211$STDAGE_t2_first,
                                                 "B_L_prop" = DeltaB_df_211$B_L_prop_last)
                                      }  )
    
    # calculate delta-B due to B-L (disturbance) -- simple subtraction
    DeltaB_df_211$DeltaB_disturbance<- (DeltaB_df_211$B_pred_disturbance2 - DeltaB_df_211$B_pred_disturbance1) / 
                                (DeltaB_df_211$MEASTIME_t2_last - DeltaB_df_211$MEASTIME_t2_first)
}
...
if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "212",]$Best.Mod == "NA"){
    ...
    DeltaB_df_212$B_pred_disturbance1 <- predict(get(paste("nls_212.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 212,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_212$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_212$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
    ...
    DeltaB_df_212$B_pred_disturbance2 <- predict(get(paste("nls_212.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 212,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_212$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_212$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_212$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_212$STDAGE_t2_first,
                                                 "B_L_prop" = DeltaB_df_212$B_L_prop_last)
                                      }  )

    # calculate delta-B due to B-L (disturbance) -- simple subtraction
    DeltaB_df_212$DeltaB_disturbance<- (DeltaB_df_212$B_pred_disturbance2 - DeltaB_df_212$B_pred_disturbance1) / 
                                (DeltaB_df_212$MEASTIME_t2_last - DeltaB_df_212$MEASTIME_t2_first)
}
...
if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "221",]$Best.Mod == "NA"){
    ...
    DeltaB_df_221$B_pred_disturbance1 <- predict(get(paste("nls_221.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 221,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_221$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_221$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
    ...
    DeltaB_df_221$B_pred_disturbance2 <- predict(get(paste("nls_221.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 221,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_221$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_221$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_221$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_221$STDAGE_t2_first,
                                                 "B_L_prop" = DeltaB_df_221$B_L_prop_last)
                                      } )
    
    # calculate delta-B due to B-L (disturbance) -- simple subtraction
    DeltaB_df_221$DeltaB_disturbance<- (DeltaB_df_221$B_pred_disturbance2 - DeltaB_df_221$B_pred_disturbance1) / 
                                (DeltaB_df_221$MEASTIME_t2_last - DeltaB_df_221$MEASTIME_t2_first)
}
...
if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "222",]$Best.Mod == "NA"){
    ...
    DeltaB_df_222$B_pred_disturbance1 <- predict(get(paste("nls_222.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 222,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_222$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_222$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
    ...
    DeltaB_df_222$B_pred_disturbance2 <- predict(get(paste("nls_222.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 222,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_222$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_222$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_222$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_222$STDAGE_t2_first,
                                                 "B_L_prop" = DeltaB_df_222$B_L_prop_last)
                                      } )
    
    # calculate delta-B due to B-L (disturbance) -- simple subtraction
    DeltaB_df_222$DeltaB_disturbance<- (DeltaB_df_222$B_pred_disturbance2 - DeltaB_df_222$B_pred_disturbance1) / 
                                (DeltaB_df_222$MEASTIME_t2_last - DeltaB_df_222$MEASTIME_t2_first)
}
...
if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "223",]$Best.Mod == "NA"){
    ...
    DeltaB_df_223$B_pred_disturbance1 <- predict(get(paste("nls_223.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 223,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_223$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_223$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
    ...
    DeltaB_df_223$B_pred_disturbance2 <- predict(get(paste("nls_223.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 223,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_223$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_223$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_223$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_223$STDAGE_t2_first,
                                                 "B_L_prop" = DeltaB_df_223$B_L_prop_last)
                                      } )
    
    # calculate delta-B due to B-L (disturbance) -- simple subtraction
    DeltaB_df_223$DeltaB_disturbance<- (DeltaB_df_223$B_pred_disturbance2 - DeltaB_df_223$B_pred_disturbance1) / 
                                (DeltaB_df_223$MEASTIME_t2_last - DeltaB_df_223$MEASTIME_t2_first)
}
...
if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "231",]$Best.Mod == "NA"){
    ...
    DeltaB_df_231$B_pred_disturbance1 <- predict(get(paste("nls_231.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 231,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_231$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_231$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
    ...
    DeltaB_df_231$B_pred_disturbance2 <- predict(get(paste("nls_231.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 231,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_231$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_231$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_231$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_231$STDAGE_t2_first,
                                                 "B_L_prop" = DeltaB_df_231$B_L_prop_last)
                                      } )
    
    # calculate delta-B due to B-L (disturbance) -- simple subtraction
    DeltaB_df_231$DeltaB_disturbance<- (DeltaB_df_231$B_pred_disturbance2 - DeltaB_df_231$B_pred_disturbance1) / 
                                (DeltaB_df_231$MEASTIME_t2_last - DeltaB_df_231$MEASTIME_t2_first)
}
...
if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "232",]$Best.Mod == "NA"){
    ...
    DeltaB_df_232$B_pred_disturbance1 <- predict(get(paste("nls_232.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 232,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_232$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_232$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
    ...
    DeltaB_df_232$B_pred_disturbance2 <- predict(get(paste("nls_232.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 232,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_232$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_232$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_232$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_232$STDAGE_t2_first,
                                                 "B_L_prop" = DeltaB_df_232$B_L_prop_last)
                                      } )
    
    # calculate delta-B due to B-L (disturbance) -- simple subtraction
    DeltaB_df_232$DeltaB_disturbance<- (DeltaB_df_232$B_pred_disturbance2 - DeltaB_df_232$B_pred_disturbance1) / 
                                (DeltaB_df_232$MEASTIME_t2_last - DeltaB_df_232$MEASTIME_t2_first)
}
...
if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "234",]$Best.Mod == "NA"){
    ...
    DeltaB_df_234$B_pred_disturbance1 <- predict(get(paste("nls_234.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 234,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_234$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_234$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
    ...
    DeltaB_df_234$B_pred_disturbance2 <- predict(get(paste("nls_234.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 234,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_234$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_234$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_234$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_234$STDAGE_t2_first,
                                                 "B_L_prop" = DeltaB_df_234$B_L_prop_last)
                                      } )
    
    # calculate delta-B due to B-L (disturbance) -- simple subtraction
    DeltaB_df_234$DeltaB_disturbance<- (DeltaB_df_234$B_pred_disturbance2 - DeltaB_df_234$B_pred_disturbance1) / 
                                (DeltaB_df_234$MEASTIME_t2_last - DeltaB_df_234$MEASTIME_t2_first)
}
...
if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "251",]$Best.Mod == "NA"){
    ...
    DeltaB_df_251$B_pred_disturbance1 <- predict(get(paste("nls_251.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 251,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_251$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_251$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
    ...
    DeltaB_df_251$B_pred_disturbance2 <- predict(get(paste("nls_251.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 251,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_251$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_251$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_251$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_251$STDAGE_t2_first,
                                                 "B_L_prop" = DeltaB_df_251$B_L_prop_last)
                                      } )
    
    # calculate delta-B due to B-L (disturbance) -- simple subtraction
    DeltaB_df_251$DeltaB_disturbance<- (DeltaB_df_251$B_pred_disturbance2 - DeltaB_df_251$B_pred_disturbance1) / 
                                (DeltaB_df_251$MEASTIME_t2_last - DeltaB_df_251$MEASTIME_t2_first)
}
...
if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "255",]$Best.Mod == "NA"){
    ...
    DeltaB_df_255$B_pred_disturbance1 <- predict(get(paste("nls_255.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 255,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_255$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_255$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
    ...
    DeltaB_df_255$B_pred_disturbance2 <- predict(get(paste("nls_255.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == 255,]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_255$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_255$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_255$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_255$STDAGE_t2_first,
                                                 "B_L_prop" = DeltaB_df_255$B_L_prop_last)
                                      } )
    
    # calculate delta-B due to B-L (disturbance) -- simple subtraction
    DeltaB_df_255$DeltaB_disturbance<- (DeltaB_df_255$B_pred_disturbance2 - DeltaB_df_255$B_pred_disturbance1) / 
                                (DeltaB_df_255$MEASTIME_t2_last - DeltaB_df_255$MEASTIME_t2_first)
}
...
if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M211",]$Best.Mod == "NA"){
    ...
    DeltaB_df_M211$B_pred_disturbance1 <- predict(get(paste("nls_M211.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M211",]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_M211$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_M211$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
    ...
    DeltaB_df_M211$B_pred_disturbance2 <- predict(get(paste("nls_M211.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M211",]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_M211$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_M211$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_M211$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_M211$STDAGE_t2_first,
                                                 "B_L_prop" = DeltaB_df_M211$B_L_prop_last)
                                      } )
    
    # calculate delta-B due to B-L (disturbance) -- simple subtraction
    DeltaB_df_M211$DeltaB_disturbance<- (DeltaB_df_M211$B_pred_disturbance2 - DeltaB_df_M211$B_pred_disturbance1) / 
                                 (DeltaB_df_M211$MEASTIME_t2_last - DeltaB_df_M211$MEASTIME_t2_first)
}
...
if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M221",]$Best.Mod == "NA"){
    ...
    DeltaB_df_M221$B_pred_disturbance1 <- predict(get(paste("nls_M221.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M221",]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_M221$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_M221$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
    ...
    DeltaB_df_M221$B_pred_disturbance2 <- predict(get(paste("nls_M221.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M221",]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_M221$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_M221$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_M221$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_M221$STDAGE_t2_first,
                                                 "B_L_prop" = DeltaB_df_M221$B_L_prop_last)
                                      } )
    
    # calculate delta-B due to B-L (disturbance) -- simple subtraction
    DeltaB_df_M221$DeltaB_disturbance<- (DeltaB_df_M221$B_pred_disturbance2 - DeltaB_df_M221$B_pred_disturbance1) / 
                                 (DeltaB_df_M221$MEASTIME_t2_last - DeltaB_df_M221$MEASTIME_t2_first)
}
...
if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M223",]$Best.Mod == "NA"){
    ...
    DeltaB_df_M223$B_pred_disturbance1 <- predict(get(paste("nls_M223.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M223",]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_M223$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_M223$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
    ...
    DeltaB_df_M223$B_pred_disturbance2 <- predict(get(paste("nls_M223.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M223",]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_M223$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_M223$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_M223$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_M223$STDAGE_t2_first,
                                                 "B_L_prop" = DeltaB_df_M223$B_L_prop_last)
                                      } )
    
    # calculate delta-B due to B-L (disturbance) -- simple subtraction
    DeltaB_df_M223$DeltaB_disturbance<- (DeltaB_df_M223$B_pred_disturbance2 - DeltaB_df_M223$B_pred_disturbance1) / 
                                 (DeltaB_df_M223$MEASTIME_t2_last - DeltaB_df_M223$MEASTIME_t2_first)
}
...
if(!Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M231",]$Best.Mod == "NA"){
    ...
    DeltaB_df_M231$B_pred_disturbance1 <- predict(get(paste("nls_M231.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M231",]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_M231$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_M231$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
    ...
    DeltaB_df_M231$B_pred_disturbance2 <- predict(get(paste("nls_M231.",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == "M231",]$Best.Mod, sep ="")),
                                  newdata = if(Mod.Sel3 %in% c(1,"1a","1b","1c",4)){
                                    data.frame("MEASTIME_t2" = DeltaB_df_M231$MEASTIME_t2_first,
                                               "STDAGE_t2" = DeltaB_df_M231$STDAGE_t2_first)
                                    } else if (Mod.Sel3 %in% c(2,"2a","2b","2c",5)){
                                      data.frame("MEASTIME_t2" =  DeltaB_df_M231$MEASTIME_t2_first,
                                                 "STDAGE_t2" =  DeltaB_df_M231$STDAGE_t2_first,
                                                 "B_L_prop" = DeltaB_df_M231$B_L_prop_last)
                                      } )
    
    # calculate delta-B due to B-L (disturbance) -- simple subtraction
    DeltaB_df_M231$DeltaB_disturbance<- (DeltaB_df_M231$B_pred_disturbance2 - DeltaB_df_M231$B_pred_disturbance1) / 
                                 (DeltaB_df_M231$MEASTIME_t2_last - DeltaB_df_M231$MEASTIME_t2_first)
}
```

```{r}
### make column for DIFF STDAGE

DeltaB_df_211$diff_STDAGE <- DeltaB_df_211$STDAGE_t2_last -  DeltaB_df_211$STDAGE_t2_first
DeltaB_df_212$diff_STDAGE <- DeltaB_df_212$STDAGE_t2_last -  DeltaB_df_212$STDAGE_t2_first
DeltaB_df_221$diff_STDAGE <- DeltaB_df_221$STDAGE_t2_last -  DeltaB_df_221$STDAGE_t2_first
DeltaB_df_222$diff_STDAGE <- DeltaB_df_222$STDAGE_t2_last -  DeltaB_df_222$STDAGE_t2_first
DeltaB_df_223$diff_STDAGE <- DeltaB_df_223$STDAGE_t2_last -  DeltaB_df_223$STDAGE_t2_first
DeltaB_df_231$diff_STDAGE <- DeltaB_df_231$STDAGE_t2_last -  DeltaB_df_231$STDAGE_t2_first
DeltaB_df_232$diff_STDAGE <- DeltaB_df_232$STDAGE_t2_last -  DeltaB_df_232$STDAGE_t2_first
DeltaB_df_234$diff_STDAGE <- DeltaB_df_234$STDAGE_t2_last -  DeltaB_df_234$STDAGE_t2_first
DeltaB_df_251$diff_STDAGE <- DeltaB_df_251$STDAGE_t2_last -  DeltaB_df_251$STDAGE_t2_first
DeltaB_df_255$diff_STDAGE <- DeltaB_df_255$STDAGE_t2_last -  DeltaB_df_255$STDAGE_t2_first
DeltaB_df_M211$diff_STDAGE <- DeltaB_df_M211$STDAGE_t2_last -  DeltaB_df_M211$STDAGE_t2_first
DeltaB_df_M221$diff_STDAGE <- DeltaB_df_M221$STDAGE_t2_last -  DeltaB_df_M221$STDAGE_t2_first
DeltaB_df_M223$diff_STDAGE <- DeltaB_df_M223$STDAGE_t2_last -  DeltaB_df_M223$STDAGE_t2_first
DeltaB_df_M231$diff_STDAGE <- DeltaB_df_M231$STDAGE_t2_last -  DeltaB_df_M231$STDAGE_t2_first
```

```{r}
### make column for SUM DeltaB_TOTAL 

DeltaB_df_211$DeltaB_SUM_total <- DeltaB_df_211$DeltaB_age +  DeltaB_df_211$DeltaB_year + DeltaB_df_211$DeltaB_disturbance
DeltaB_df_212$DeltaB_SUM_total <- DeltaB_df_212$DeltaB_age +  DeltaB_df_212$DeltaB_year + DeltaB_df_212$DeltaB_disturbance
DeltaB_df_221$DeltaB_SUM_total <- DeltaB_df_221$DeltaB_age +  DeltaB_df_221$DeltaB_year + DeltaB_df_221$DeltaB_disturbance
DeltaB_df_222$DeltaB_SUM_total <- DeltaB_df_222$DeltaB_age +  DeltaB_df_222$DeltaB_year + DeltaB_df_222$DeltaB_disturbance
DeltaB_df_223$DeltaB_SUM_total <- DeltaB_df_223$DeltaB_age +  DeltaB_df_223$DeltaB_year + DeltaB_df_223$DeltaB_disturbance
DeltaB_df_231$DeltaB_SUM_total <- DeltaB_df_231$DeltaB_age +  DeltaB_df_231$DeltaB_year + DeltaB_df_231$DeltaB_disturbance
DeltaB_df_232$DeltaB_SUM_total <- DeltaB_df_232$DeltaB_age +  DeltaB_df_232$DeltaB_year + DeltaB_df_232$DeltaB_disturbance
DeltaB_df_234$DeltaB_SUM_total <- DeltaB_df_234$DeltaB_age +  DeltaB_df_234$DeltaB_year + DeltaB_df_234$DeltaB_disturbance
DeltaB_df_251$DeltaB_SUM_total <- DeltaB_df_251$DeltaB_age +  DeltaB_df_251$DeltaB_year + DeltaB_df_251$DeltaB_disturbance
DeltaB_df_255$DeltaB_SUM_total <- DeltaB_df_255$DeltaB_age +  DeltaB_df_255$DeltaB_year + DeltaB_df_255$DeltaB_disturbance
DeltaB_df_M211$DeltaB_SUM_total <- DeltaB_df_M211$DeltaB_age +  DeltaB_df_M211$DeltaB_year + DeltaB_df_M211$DeltaB_disturbance
DeltaB_df_M221$DeltaB_SUM_total <- DeltaB_df_M221$DeltaB_age +  DeltaB_df_M221$DeltaB_year + DeltaB_df_M221$DeltaB_disturbance
DeltaB_df_M223$DeltaB_SUM_total <- DeltaB_df_M223$DeltaB_age +  DeltaB_df_M223$DeltaB_year + DeltaB_df_M223$DeltaB_disturbance
DeltaB_df_M231$DeltaB_SUM_total <- DeltaB_df_M231$DeltaB_age +  DeltaB_df_M231$DeltaB_year + DeltaB_df_M231$DeltaB_disturbance
```

#### simulate f(disturbance) - Variance in DeltaB_disturbance -- rmvnorm

```{r}
## function which uses f(B_L_prop_first)
predict_f.disturbance_first <- function(X, mod.num, data) {
  ## if loop to solve function given first data
  if (mod.num == 5) {
    tau = X["tau"]
    alpha = X["alpha"]
    a = X["a"]
...
## for f(B_L_prop_last)
predict_f.disturbance_last <- function(X, mod.num, data) {
  ## if loop to solve function given last data
  if (mod.num == 5) {
    tau = X["tau"]
    alpha = X["alpha"]
    a = X["a"]
    ...
    c = X["c"]
    d = X["d"]
    B <- (1 + (data$MEASTIME_t2_first-1990)*tau/100) * (1-alpha*data$B_L_prop_last) *
          (a+b*exp(-((log((data$STDAGE_t2_first/c)))/d)^2))
  } else if (mod.num == 2) {
    tau = X["tau"]
    alpha = X["alpha"]
...
```{r}
### make a function to get parameters and predict B
predict_sim_DeltaB_disturbance <- function(ecoprov) {
  ...
  set.seed(77) ## for reproducibility
  parameters <- if(mod.num == 2){
    matrix(data = c(rep(summary(mod)$param["tau","Estimate"], 9999), 
                    rnorm(nsamp, summary(mod)$param["alpha","Estimate"], sqrt(vcov_mat["alpha","alpha"])), ### sample parameters from Random Normal dist
                    rep(summary(mod)$param["A","Estimate"], 9999),
                    rep(summary(mod)$param["k","Estimate"], 9999)), nrow = 9999, ncol =4,
           dimnames = list(NULL, c("tau", "alpha", "A", "k")))
    } else if(mod.num == "2a"){
  ...
  ## solve f(disturbance) function at B_L_first and B_L_last
  sim_f_disturbance_first <- apply(parameters, 1, predict_f.disturbance_first, data = data, mod.num = mod.num,
                                   simplify = TRUE)
  sim_f_disturbance_last <- apply(parameters, 1, predict_f.disturbance_last, data = data, mod.num = mod.num,
                                  simplify = TRUE)
  
  ### take difference in f(disturbance_first) and f(disturbance_last) over deltaT (aka MEASTIME_t2_last - MEASTIME_t2_first)  --  need to transform the numerator to get plots in rows and sim samples down columns -- then can do division 
  sim_DeltaB <- t(sim_f_disturbance_last - sim_f_disturbance_first) / (data$MEASTIME_t2_last - data$MEASTIME_t2_first)
...
## make a empty list
B_sim_pred_f_disturbance_bar <- list()
...
#### 211
B_sim_pred_f_disturbance_bar[["211"]] <- predict_sim_DeltaB_disturbance(211)

####  212
B_sim_pred_f_disturbance_bar[["212"]] <- predict_sim_DeltaB_disturbance(212)

####  221
B_sim_pred_f_disturbance_bar[["221"]] <- predict_sim_DeltaB_disturbance(221)

####  222
B_sim_pred_f_disturbance_bar[["222"]] <- predict_sim_DeltaB_disturbance(222)

####  223
B_sim_pred_f_disturbance_bar[["223"]] <- predict_sim_DeltaB_disturbance(223)

####  231
B_sim_pred_f_disturbance_bar[["231"]] <- predict_sim_DeltaB_disturbance(231)

####  232
B_sim_pred_f_disturbance_bar[["232"]] <- predict_sim_DeltaB_disturbance(232)

####  234
B_sim_pred_f_disturbance_bar[["234"]] <- predict_sim_DeltaB_disturbance(234)

####  251
B_sim_pred_f_disturbance_bar[["251"]] <- predict_sim_DeltaB_disturbance(251)

####  255
B_sim_pred_f_disturbance_bar[["255"]] <- predict_sim_DeltaB_disturbance(255)

####  M211
B_sim_pred_f_disturbance_bar[["M211"]] <- predict_sim_DeltaB_disturbance("M211")

####  M221
B_sim_pred_f_disturbance_bar[["M221"]] <- predict_sim_DeltaB_disturbance("M221")

####  M223
B_sim_pred_f_disturbance_bar[["M223"]] <- predict_sim_DeltaB_disturbance("M223")

####  M231
B_sim_pred_f_disturbance_bar[["M231"]] <- predict_sim_DeltaB_disturbance("M231")
...
```{r}
## create mean Loss Rate Frame

lm2_B_L_frame <- data.frame(Ecoprovince = c("211", "212", "221", "222", "223", "231", "232", 
                                            "234", "251", "255", "M211", "M221", "M223", "M231"),
                            mean_B_L_prop_first = c(mean(DeltaB_df_211$B_L_prop_first), mean(DeltaB_df_212$B_L_prop_first),
                                                    mean(DeltaB_df_221$B_L_prop_first), mean(DeltaB_df_222$B_L_prop_first),
                                                    mean(DeltaB_df_223$B_L_prop_first), mean(DeltaB_df_231$B_L_prop_first),
                                                    mean(DeltaB_df_232$B_L_prop_first), mean(DeltaB_df_234$B_L_prop_first),
                                                    mean(DeltaB_df_251$B_L_prop_first), mean(DeltaB_df_255$B_L_prop_first),
                                                    mean(DeltaB_df_M211$B_L_prop_first), mean(DeltaB_df_M221$B_L_prop_first),
                                                    mean(DeltaB_df_M223$B_L_prop_first), mean(DeltaB_df_M231$B_L_prop_first)),
                            mean_B_L_prop_last = c(mean(DeltaB_df_211$B_L_prop_last), mean(DeltaB_df_212$B_L_prop_last),
                                                    mean(DeltaB_df_221$B_L_prop_last), mean(DeltaB_df_222$B_L_prop_last),
                                                    mean(DeltaB_df_223$B_L_prop_last), mean(DeltaB_df_231$B_L_prop_last),
                                                    mean(DeltaB_df_232$B_L_prop_last), mean(DeltaB_df_234$B_L_prop_last),
                                                    mean(DeltaB_df_251$B_L_prop_last), mean(DeltaB_df_255$B_L_prop_last),
                                                    mean(DeltaB_df_M211$B_L_prop_last), mean(DeltaB_df_M221$B_L_prop_last),
                                                    mean(DeltaB_df_M223$B_L_prop_last), mean(DeltaB_df_M231$B_L_prop_last)),
                            mean_LossRate =  c(mean((DeltaB_df_211$B_L_prop_last - DeltaB_df_211$B_L_prop_first) /  
                                      (DeltaB_df_211$MEASTIME_t2_last - DeltaB_df_211$MEASTIME_t2_first)),  
                                mean((DeltaB_df_212$B_L_prop_last - DeltaB_df_212$B_L_prop_first) /  
                                      (DeltaB_df_212$MEASTIME_t2_last - DeltaB_df_212$MEASTIME_t2_first)), 
                                mean((DeltaB_df_221$B_L_prop_last - DeltaB_df_221$B_L_prop_first) /  
                                      (DeltaB_df_221$MEASTIME_t2_last - DeltaB_df_221$MEASTIME_t2_first)),
                                mean((DeltaB_df_222$B_L_prop_last - DeltaB_df_222$B_L_prop_first) /  
                                      (DeltaB_df_222$MEASTIME_t2_last - DeltaB_df_222$MEASTIME_t2_first)), 
                                mean((DeltaB_df_223$B_L_prop_last - DeltaB_df_223$B_L_prop_first) /  
                                      (DeltaB_df_223$MEASTIME_t2_last - DeltaB_df_223$MEASTIME_t2_first)),
                                mean((DeltaB_df_231$B_L_prop_last - DeltaB_df_231$B_L_prop_first) /  
                                      (DeltaB_df_231$MEASTIME_t2_last - DeltaB_df_231$MEASTIME_t2_first)), 
                                mean((DeltaB_df_232$B_L_prop_last - DeltaB_df_232$B_L_prop_first) /  
                                      (DeltaB_df_232$MEASTIME_t2_last - DeltaB_df_232$MEASTIME_t2_first)),
                                mean((DeltaB_df_234$B_L_prop_last - DeltaB_df_234$B_L_prop_first) /  
                                      (DeltaB_df_234$MEASTIME_t2_last - DeltaB_df_234$MEASTIME_t2_first)), 
                                mean((DeltaB_df_251$B_L_prop_last - DeltaB_df_251$B_L_prop_first) /  
                                      (DeltaB_df_251$MEASTIME_t2_last - DeltaB_df_251$MEASTIME_t2_first)),
                                mean((DeltaB_df_255$B_L_prop_last - DeltaB_df_255$B_L_prop_first) /  
                                      (DeltaB_df_255$MEASTIME_t2_last - DeltaB_df_255$MEASTIME_t2_first)),
                                mean((DeltaB_df_M211$B_L_prop_last - DeltaB_df_M211$B_L_prop_first) /  
                                      (DeltaB_df_M211$MEASTIME_t2_last - DeltaB_df_M211$MEASTIME_t2_first)), 
                                mean((DeltaB_df_M221$B_L_prop_last - DeltaB_df_M221$B_L_prop_first) /  
                                      (DeltaB_df_M221$MEASTIME_t2_last - DeltaB_df_M221$MEASTIME_t2_first)),
                                mean((DeltaB_df_M223$B_L_prop_last - DeltaB_df_M223$B_L_prop_first) /  
                                      (DeltaB_df_M223$MEASTIME_t2_last - DeltaB_df_M223$MEASTIME_t2_first)), 
                                mean((DeltaB_df_M231$B_L_prop_last - DeltaB_df_M231$B_L_prop_first) /  
                                      (DeltaB_df_M231$MEASTIME_t2_last - DeltaB_df_M231$MEASTIME_t2_first))), 
                            
                            se_LossRate = c(plotrix::std.error((DeltaB_df_211$B_L_prop_last - DeltaB_df_211$B_L_prop_first) /  
                                      (DeltaB_df_211$MEASTIME_t2_last - DeltaB_df_211$MEASTIME_t2_first)),  
                                plotrix::std.error((DeltaB_df_212$B_L_prop_last - DeltaB_df_212$B_L_prop_first) /  
                                      (DeltaB_df_212$MEASTIME_t2_last - DeltaB_df_212$MEASTIME_t2_first)), 
                                plotrix::std.error((DeltaB_df_221$B_L_prop_last - DeltaB_df_221$B_L_prop_first) /  
                                      (DeltaB_df_221$MEASTIME_t2_last - DeltaB_df_221$MEASTIME_t2_first)),
                                plotrix::std.error((DeltaB_df_222$B_L_prop_last - DeltaB_df_222$B_L_prop_first) /  
                                      (DeltaB_df_222$MEASTIME_t2_last - DeltaB_df_222$MEASTIME_t2_first)), 
                                plotrix::std.error((DeltaB_df_223$B_L_prop_last - DeltaB_df_223$B_L_prop_first) /  
                                      (DeltaB_df_223$MEASTIME_t2_last - DeltaB_df_223$MEASTIME_t2_first)),
                                plotrix::std.error((DeltaB_df_231$B_L_prop_last - DeltaB_df_231$B_L_prop_first) /  
                                      (DeltaB_df_231$MEASTIME_t2_last - DeltaB_df_231$MEASTIME_t2_first)), 
                                plotrix::std.error((DeltaB_df_232$B_L_prop_last - DeltaB_df_232$B_L_prop_first) /  
                                      (DeltaB_df_232$MEASTIME_t2_last - DeltaB_df_232$MEASTIME_t2_first)),
                                plotrix::std.error((DeltaB_df_234$B_L_prop_last - DeltaB_df_234$B_L_prop_first) /  
                                      (DeltaB_df_234$MEASTIME_t2_last - DeltaB_df_234$MEASTIME_t2_first)), 
                                plotrix::std.error((DeltaB_df_251$B_L_prop_last - DeltaB_df_251$B_L_prop_first) /  
                                      (DeltaB_df_251$MEASTIME_t2_last - DeltaB_df_251$MEASTIME_t2_first)),
                                plotrix::std.error((DeltaB_df_255$B_L_prop_last - DeltaB_df_255$B_L_prop_first) /  
                                      (DeltaB_df_255$MEASTIME_t2_last - DeltaB_df_255$MEASTIME_t2_first)),
                                plotrix::std.error((DeltaB_df_M211$B_L_prop_last - DeltaB_df_M211$B_L_prop_first) /  
                                      (DeltaB_df_M211$MEASTIME_t2_last - DeltaB_df_M211$MEASTIME_t2_first)), 
                                plotrix::std.error((DeltaB_df_M221$B_L_prop_last - DeltaB_df_M221$B_L_prop_first) /  
                                      (DeltaB_df_M221$MEASTIME_t2_last - DeltaB_df_M221$MEASTIME_t2_first)),
                                plotrix::std.error((DeltaB_df_M223$B_L_prop_last - DeltaB_df_M223$B_L_prop_first) /  
                                      (DeltaB_df_M223$MEASTIME_t2_last - DeltaB_df_M223$MEASTIME_t2_first)), 
                                plotrix::std.error((DeltaB_df_M231$B_L_prop_last - DeltaB_df_M231$B_L_prop_first) /  
                                      (DeltaB_df_M231$MEASTIME_t2_last - DeltaB_df_M231$MEASTIME_t2_first))), 
                            
                            var_LossRate = c(var((DeltaB_df_211$B_L_prop_last - DeltaB_df_211$B_L_prop_first) /  
                                      (DeltaB_df_211$MEASTIME_t2_last - DeltaB_df_211$MEASTIME_t2_first)),  
                                var((DeltaB_df_212$B_L_prop_last - DeltaB_df_212$B_L_prop_first) /  
                                      (DeltaB_df_212$MEASTIME_t2_last - DeltaB_df_212$MEASTIME_t2_first)), 
                                var((DeltaB_df_221$B_L_prop_last - DeltaB_df_221$B_L_prop_first) /  
                                      (DeltaB_df_221$MEASTIME_t2_last - DeltaB_df_221$MEASTIME_t2_first)),
                                var((DeltaB_df_222$B_L_prop_last - DeltaB_df_222$B_L_prop_first) /  
                                      (DeltaB_df_222$MEASTIME_t2_last - DeltaB_df_222$MEASTIME_t2_first)), 
                                var((DeltaB_df_223$B_L_prop_last - DeltaB_df_223$B_L_prop_first) /  
                                      (DeltaB_df_223$MEASTIME_t2_last - DeltaB_df_223$MEASTIME_t2_first)),
                                var((DeltaB_df_231$B_L_prop_last - DeltaB_df_231$B_L_prop_first) /  
                                      (DeltaB_df_231$MEASTIME_t2_last - DeltaB_df_231$MEASTIME_t2_first)), 
                                var((DeltaB_df_232$B_L_prop_last - DeltaB_df_232$B_L_prop_first) /  
                                      (DeltaB_df_232$MEASTIME_t2_last - DeltaB_df_232$MEASTIME_t2_first)),
                                var((DeltaB_df_234$B_L_prop_last - DeltaB_df_234$B_L_prop_first) /  
                                      (DeltaB_df_234$MEASTIME_t2_last - DeltaB_df_234$MEASTIME_t2_first)), 
                                var((DeltaB_df_251$B_L_prop_last - DeltaB_df_251$B_L_prop_first) /  
                                      (DeltaB_df_251$MEASTIME_t2_last - DeltaB_df_251$MEASTIME_t2_first)),
                                var((DeltaB_df_255$B_L_prop_last - DeltaB_df_255$B_L_prop_first) /  
                                      (DeltaB_df_255$MEASTIME_t2_last - DeltaB_df_255$MEASTIME_t2_first)),
                                var((DeltaB_df_M211$B_L_prop_last - DeltaB_df_M211$B_L_prop_first) /  
                                      (DeltaB_df_M211$MEASTIME_t2_last - DeltaB_df_M211$MEASTIME_t2_first)), 
                                var((DeltaB_df_M221$B_L_prop_last - DeltaB_df_M221$B_L_prop_first) /  
                                      (DeltaB_df_M221$MEASTIME_t2_last - DeltaB_df_M221$MEASTIME_t2_first)),
                                var((DeltaB_df_M223$B_L_prop_last - DeltaB_df_M223$B_L_prop_first) /  
                                      (DeltaB_df_M223$MEASTIME_t2_last - DeltaB_df_M223$MEASTIME_t2_first)), 
                                var((DeltaB_df_M231$B_L_prop_last - DeltaB_df_M231$B_L_prop_first) /  
                                      (DeltaB_df_M231$MEASTIME_t2_last - DeltaB_df_M231$MEASTIME_t2_first))))

lm2_B_L_frame$Ecoprovince <- as.factor(lm2_B_L_frame$Ecoprovince)
```


```{r}
plot(lm2_B_L_frame$Ecoprovince, lm2_B_L_frame$mean_B_L_prop_first, las = 2, xlab = "Ecoprovince", ylab = "mean B_L_prop", ylim = c(0,0.2), pch = 16, type = "p")
points(lm2_B_L_frame$Ecoprovince, lm2_B_L_frame$mean_B_L_prop_last, col = "gray", pch= 15, ylim = c(0,0.2))
legend("topright", lty = c(1, NA), lwd = c(3, NA), pch = c(NA, 15),  col = c("black", "gray"), legend = c("  first remasurement", "  last remeaurement"), bty ="n")
```
...
BK_df <-  cbind(Ecoprovince = c(rep("211", length(DeltaB_df_211$pltID_first)), 
                                rep("212", length(DeltaB_df_212$pltID_first)),
                                rep("221", length(DeltaB_df_221$pltID_first)),
                                rep("222", length(DeltaB_df_222$pltID_first)),
                                rep("223", length(DeltaB_df_223$pltID_first)),
                                rep("231", length(DeltaB_df_231$pltID_first)),
                                rep("232", length(DeltaB_df_232$pltID_first)),
                                rep("234", length(DeltaB_df_234$pltID_first)),
                                rep("251", length(DeltaB_df_251$pltID_first)),
                                rep("255", length(DeltaB_df_255$pltID_first)),
                                rep("M211", length(DeltaB_df_M211$pltID_first)),
                                rep("M221", length(DeltaB_df_M221$pltID_first)),
                                rep("M223", length(DeltaB_df_M223$pltID_first)),
                                rep("M231", length(DeltaB_df_M231$pltID_first))),
                
                rbind(DeltaB_df_211[,c("diff_STDAGE", "DeltaB_total", "DeltaB_age", "DeltaB_year", "DeltaB_disturbance", "DeltaB_SUM_total")],
                      DeltaB_df_212[,c("diff_STDAGE", "DeltaB_total", "DeltaB_age", "DeltaB_year", "DeltaB_disturbance", "DeltaB_SUM_total")],
                      DeltaB_df_221[,c("diff_STDAGE", "DeltaB_total", "DeltaB_age", "DeltaB_year", "DeltaB_disturbance", "DeltaB_SUM_total")],
                      DeltaB_df_222[,c("diff_STDAGE", "DeltaB_total",  "DeltaB_age", "DeltaB_year", "DeltaB_disturbance", "DeltaB_SUM_total")],
                      DeltaB_df_223[,c("diff_STDAGE", "DeltaB_total", "DeltaB_age", "DeltaB_year", "DeltaB_disturbance", "DeltaB_SUM_total")],
                      DeltaB_df_231[,c("diff_STDAGE", "DeltaB_total", "DeltaB_age", "DeltaB_year", "DeltaB_disturbance", "DeltaB_SUM_total")],
                      DeltaB_df_232[,c("diff_STDAGE", "DeltaB_total", "DeltaB_age", "DeltaB_year", "DeltaB_disturbance", "DeltaB_SUM_total")],
                      DeltaB_df_234[,c("diff_STDAGE", "DeltaB_total", "DeltaB_age", "DeltaB_year", "DeltaB_disturbance", "DeltaB_SUM_total")],
                      DeltaB_df_251[,c("diff_STDAGE", "DeltaB_total", "DeltaB_age", "DeltaB_year", "DeltaB_disturbance", "DeltaB_SUM_total")],
                      DeltaB_df_255[,c("diff_STDAGE", "DeltaB_total", "DeltaB_age", "DeltaB_year", "DeltaB_disturbance", "DeltaB_SUM_total")],
                      DeltaB_df_M211[,c("diff_STDAGE", "DeltaB_total", "DeltaB_age", "DeltaB_year", "DeltaB_disturbance", "DeltaB_SUM_total")],
                      DeltaB_df_M221[,c("diff_STDAGE", "DeltaB_total", "DeltaB_age", "DeltaB_year", "DeltaB_disturbance", "DeltaB_SUM_total")],
                      DeltaB_df_M223[,c("diff_STDAGE", "DeltaB_total", "DeltaB_age", "DeltaB_year", "DeltaB_disturbance", "DeltaB_SUM_total")],
                      DeltaB_df_M231[,c("diff_STDAGE", "DeltaB_total", "DeltaB_age", "DeltaB_year", "DeltaB_disturbance", "DeltaB_SUM_total")]))

## reshape data wide to long
BK_df_long <- reshape2::melt(BK_df, id.vars = c("Ecoprovince"), measure.vars = c("DeltaB_total", "DeltaB_age", "DeltaB_year", "DeltaB_disturbance"))
```

#### BK_df avgs (Bookeeping averages by Ecoprovince)
```{r}
## for DeltaB_age we just take mean and standard error 
    ...
    datac <- plyr::ddply(data, groupvars, .drop=.drop,
      .fun = function(xx, col) {
        c(N    = length2(xx[[col]], na.rm=na.rm),
          ...
          var  = var     (xx[[col]], na.rm=na.rm)  ## this is the variance
        )
      },
      measurevar
    )
...
## end summarySE function

#############
#### DeltaB_age
## apply summary SE function to age data in new dataframe to get average and se 
BK_df_age_avg <- summarySE(BK_df, measurevar = "DeltaB_age", groupvars = c("Ecoprovince"), na.rm = T)
colnames(BK_df_age_avg)[3] <- "value" # rename the mean to "Value" to match other df

### calculate variance from list of DeltaB_age bar samples (by ecoprov)
var.f_age <- unlist(lapply(B_sim_pred_f_age_bar, var, na.rm = T))   ## this is var_f(age)

##### important!:  need to re-calculate standard error in DeltaB_age
## NOTE: var in the BK_df_age_avg is var_age
# se of mean(DeltaB_age) =  square root of the variance in the f(age) function
## note this overwrites previous se calculation from summarySE

BK_df_age_avg$se <- sqrt(var.f_age[match(BK_df_age_avg$Ecoprovince, names(var.f_age))])

# BK_df_age_avg$se <- sqrt(BK_df_age_avg$se^2 + var.f_age[match(BK_df_age_avg$Ecoprovince, names(var.f_age))]) ## this commented code adds the variance in the data (among plots) the paramerter uncertainty in the s.e. calculation  -- the simplier method (just with parameter uncertainty is used)

BK_df_age_avg <- BK_df_age_avg[order(BK_df_age_avg$value, decreasing = T),] ### order from greatest to least

#############
#### DeltaB_productivity trend

## apply summary SE function to age data in new dataframe to get average and se 
BK_df_year_avg <- summarySE(BK_df, measurevar = "DeltaB_year", groupvars = c("Ecoprovince"), na.rm = T)
colnames(BK_df_year_avg)[3] <- "value" # rename the mean to "Value" to match other df

## order to match order of DeltaB_age
BK_df_year_avg$Ecoprovince <- factor(BK_df_year_avg$Ecoprovince, levels=BK_df_age_avg$Ecoprovince)

### calculate variance from list of DeltaB_age bar samples (by ecoprov)
var.f_year <- unlist(lapply(B_sim_pred_f_year_bar, var, na.rm = T))   ## this is var_f(year) -- for the productivity trend 

##### important!:  need to re-calculate standard error in DeltaB_tau/ productivity trend 
## note this overwrites previous se calculation from summarySE
## this is done the same as above for DeltaB_age

BK_df_year_avg$se <- sqrt(var.f_year[match(BK_df_year_avg$Ecoprovince, names(var.f_year))])

# BK_df_year_avg$se <- sqrt(BK_df_year_avg$se^2 + var.f_year[match(BK_df_year_avg$Ecoprovince, names(var.f_age))]) ## this commented code adds the variance in the data (among plots) the paramerter uncertainty in the s.e. calculation  -- the simplier method (just with parameter uncertainty is used)
                    
#############
#### DeltaB_disturbance

## apply summary SE function to age data in new dataframe to get average and se 
BK_df_disturbance_avg <- summarySE(BK_df, measurevar = "DeltaB_disturbance", groupvars = c("Ecoprovince"), na.rm = T)
colnames(BK_df_disturbance_avg)[3] <- "value" # rename the mean to "Value" to match other df

## order to match order of DeltaB_age
BK_df_disturbance_avg$Ecoprovince <- factor(BK_df_disturbance_avg$Ecoprovince, levels=BK_df_age_avg$Ecoprovince)

### calculate variance from list of DeltaB_age bar samples (by ecoprov)
var.f_disturbance <- unlist(lapply(B_sim_pred_f_disturbance_bar, var, na.rm = T)) ## this is var_f(disturbance)

##### important!:  need to re-calculate standard error in DeltaB_disturabnce
## note this overwrites previous se calculation from summarySE
## this is done the same as above for DeltaB_age

BK_df_disturbance_avg$se <- sqrt(var.f_disturbance[match(BK_df_disturbance_avg$Ecoprovince, names(var.f_disturbance))])

# BK_df_disturbance_avg$se <- sqrt(BK_df_disturbance_avg$se^2 + var.f_year[match(BK_df_disturbance_avg$Ecoprovince, names(var.f_age))])  ## this commented code adds the variance in the data (among plots) the parameter uncertainty in the s.e. calculation  -- the simplier method (just with parameter uncertainty is used)

##############
#### TOTAL   

## apply summary SE function to age data in new dataframe to get average and se 
BK_df_total_avg <- summarySE(BK_df, measurevar = "DeltaB_total", groupvars = c("Ecoprovince"), na.rm = T)
colnames(BK_df_total_avg)[3] <- "value" # rename the mean to "Value" to match other df

BK_df_total_avg$Ecoprovince <- factor(BK_df_total_avg$Ecoprovince, levels=BK_df_age_avg$Ecoprovince)

# se of DeltaB_total is the se of the mean of the data
...
## apply summary SE function to age data in new dataframe to get average and se 
BK_df_SUM_total_avg <- summarySE(BK_df, measurevar = "DeltaB_SUM_total", groupvars = c("Ecoprovince"), na.rm = T)
colnames(BK_df_SUM_total_avg)[3] <- "value" # rename the mean to "Value" to match other df

## order to match order of DeltaB_age
BK_df_SUM_total_avg$Ecoprovince <- factor(BK_df_SUM_total_avg$Ecoprovince, levels=BK_df_age_avg$Ecoprovince)

## standard error of the mean for DetlaB_SUM_total -- this is the square root of  the sum of the variances in DeltaB_age, disturbance and productivity trend (i.e., year)
BK_df_SUM_total_avg$se <- sqrt(var.f_age[match(BK_df_SUM_total_avg$Ecoprovince, names(var.f_age))] +
                               var.f_year[match(BK_df_SUM_total_avg$Ecoprovince, names(var.f_year))] +
                               var.f_disturbance[match(BK_df_SUM_total_avg$Ecoprovince, names(var.f_disturbance))])

###################
#### Data wrangling

## stack the dataframes
############################## avg1  --- for plotting just year/ge and age effects
BK_df_avg <- rbind(BK_df_total_avg, BK_df_age_avg, BK_df_year_avg, BK_df_disturbance_avg)
BK_df_avg$variable <- c(rep("DeltaB_total", 14), rep("DeltaB_age", 14), rep("DeltaB_year", 14), rep("DeltaB_disturbance", 14))

BK_df_avg$Ecoprovince <- factor(BK_df_avg$Ecoprovince, levels=BK_df_age_avg$Ecoprovince)
BK_df_avg$variable <- factor(BK_df_avg$variable, levels = c("DeltaB_total", "DeltaB_age", "DeltaB_year", "DeltaB_disturbance"))
## reorder the levels of the df to match decraseing Delta_B_age order
...
lm_eqn = function(m) {
   ...
   as.character(
    as.expression(
      substitute(italic(y) == b* italic(x) + a*","~~italic(p)*"="~prob*","~~italic(r)^2~"="~r2,
                 list(a = as.numeric(format(coef(m)[1], digits = 2)),
                      b = as.numeric(format(coef(m)[2], digits = 2)),
                      prob = as.numeric(format(summary(m)$coefficients[2,"Pr(>|t|)"], digits = 1)),
...
```{r}
### col25 color pallette
col14 <-  c("#00FF00", "#00E2E5", "#36648B", "#0000FF", "#C814FA", "#FF83FA", "#C8308C", "#EEE685", "#999999", "#FDBF6F", "#b2df8a", "#FB6496", "#a6cee3", "#FFD70080")


col14_tp <-  c("#00FF0080", "#00E2E580", "#36648B80", "#0000FF80", "#C814FA80", "#FF83FA80", "#C8308C80", "#EEE68580", "#99999980", "#FDBF6F80", "#b2df8a80", "#FB649680", "#a6cee380", "#FFD70080")
...
```{r}
BK_total_df_compare <-  merge(BK_df_total_avg, BK_df_SUM_total_avg, by = c("Ecoprovince", "N"))

BK_total_df_compare$Ecoprovince <- factor(BK_total_df_compare$Ecoprovince, 
                                          levels = c("211","212", "221", "222", "223", 
                                                     "231","232","234", "251", "255", 
                                                     "M211", "M221", "M223","M231"))

### fit lm
lm_Delta_B_total_compare <- lm(value.y~value.x, data = BK_total_df_compare)
...
```{r}
gg.Delta_B_total_compare <- ggplot(aes(x= value.x, y=value.y, color = Ecoprovince),
                        data = BK_total_df_compare) + 
                 geom_abline(intercept = 0, slope = 1, lty = 2, size =1, alpha = 0.5, col = "red") +   
                 geom_hline(yintercept = 0, lty =3) + geom_vline(xintercept = 0, lty=3)+
                 geom_smooth(method = "lm", col = "#00000080", fullrange = T) +
                 geom_point(size =1) +
                 geom_errorbarh(aes(xmin = value.x-(1.96*se.x), xmax =  value.x+(1.96*se.x))) +
                 geom_errorbar(aes(ymin = value.y-(1.96*se.y), ymax = value.y+(1.96*se.y))) +
                 theme_classic() + 
                 theme(legend.position = "top", strip.text.x = element_text(size = 6), aspect.ratio =1, 
                       legend.margin = margin(0, 0, 0, 0),
                       plot.margin = unit(c(0, 0, 0, 0), "lines"), plot.subtitle = element_text(size = 10)) + 
                 xlab(expression(~Delta*"B"[observed]*" (Mg ha"^-1*" year"^-1*")")) + 
                 ylab(expression(~Delta*"B"[sum]*" (Mg ha"^-1*" year"^-1*")"))  + 
                 scale_color_manual(values = col14) +  #c("#134B19BF", "#E66100BF", "#5D3A9BBF")
                 # labs(col="region")+  #+ facet_wrap(vars(Region)) +
                 ggrepel::geom_text_repel(aes(label = Ecoprovince),
                                          data = BK_total_df_compare, size = 2, direction = "both",
                                          min.segment.length = 10) +
                  ggtitle(label = NULL, subtitle = parse(text = lm_eqn(lm_Delta_B_total_compare)))  + ylim(c(-1.5,3)) + xlim(c(-1.5,3))

# gg.Delta_B_total_compare               
```
...
## data wrangling 
Age_effect_df <- merge(summarySE(BK_df, measurevar = "diff_STDAGE", groupvars = c("Ecoprovince"), na.rm = T), summarySE(BK_df, measurevar = "DeltaB_age", groupvars = c("Ecoprovince"), na.rm = T), by = c("Ecoprovince"))

### fit lm
lm_age <- lm(DeltaB_age~diff_STDAGE, data = Age_effect_df)

## ggplot

gg.age_effect <- ggplot(aes(x= diff_STDAGE, y= DeltaB_age, col = Ecoprovince),
                        data = Age_effect_df) + 
                 #geom_abline(intercept = 0, slope = 1, lty = 2, size =1, alpha = 0.5) +   
                 geom_hline(yintercept = 0, lty =3) + geom_vline(xintercept = 0, lty=3) +
                 geom_smooth(method = "lm", col = "#00000080", fullrange = T) +
                 geom_point( size =1) +
                 geom_errorbarh(aes(xmin = diff_STDAGE-ci.x, xmax = diff_STDAGE+ci.x)) +
                 geom_errorbar(aes(ymin = DeltaB_age-(1.96*se.y), ymax = DeltaB_age+(1.96*se.y))) +
                 theme_classic() + 
                 theme(legend.position = "top", strip.text.x = element_text(size = 6), aspect.ratio =1, 
                       legend.margin = margin(0, 0, 0, 0),
                       plot.margin = unit(c(0, 0, 0, 0), "lines"), plot.subtitle = element_text(size = 10)) + 
                 xlab(expression(~Delta*" stand age (years)")) + 
                 ylab(expression(~Delta*"B"[age]*" (Mg ha"^-1*" year"^-1*")"))  + 
                 scale_color_manual(values = col14) + 
                 labs(col="region") + 
                 ggrepel::geom_text_repel(aes(label = Ecoprovince), 
                                          data = Age_effect_df, size = 2, direction = "both", 
                                          min.segment.length = 10) +
                ggtitle(label = NULL, subtitle = parse(text = lm_eqn(lm_age)))
...
# tiff(filename = "DeltaB-DeltaAge scatter plot2_no263.tiff", width = 11, height = 12, units = "cm", res = 600, compression = "lzw")
# gg.age_effect
# dev.off()
```
...
```{r}
DELTA_frame1 <-  merge(BK_df_year_avg, nls.param.df_bal[,c("Ecoprovince", "tau", "tau.2.5", "tau.97.5")], by = "Ecoprovince")


DELTA_frame1$Ecoprovince <- factor(DELTA_frame1$Ecoprovince, 
                                          levels = c("211","212", "221", "222", "223", 
                                                     "231","232","234", "251", "255", 
                                                     "M211", "M221", "M223","M231"))

### fit lm
lm_Delta_tau <- lm(value~tau, data = DELTA_frame1)

```

```{r}

gg.Delta1_tau <- ggplot(aes(x= tau, y=value, col = Ecoprovince),
                        data = DELTA_frame1) + 
                 geom_hline(yintercept = 0, lty =3) + geom_vline(xintercept = 0, lty=3) +
                 geom_smooth(method = "lm", col = "#00000080", fullrange = T) +
                 geom_point(size =1) +
                 geom_errorbarh(aes(xmin = tau.2.5, xmax = tau.97.5)) +
                 geom_errorbar(aes(ymin = value-(1.96*se), ymax = value+(1.96*se))) +
                 theme_classic() + 
                 theme(legend.position = "top", strip.text.x = element_text(size = 6), aspect.ratio =1, 
                       legend.margin = margin(0, 0, 0, 0),
                       plot.margin = unit(c(0, 0, 0, 0), "lines"), plot.subtitle = element_text(size = 10)) + 
                 xlab(expression(~tau*" (% year"^-1*")")) + 
                 ylab(expression(~Delta*"B"[productivity~trend]*" (Mg ha"^-1*" year"^-1*")"))  + 
                 scale_color_manual(values = col14) + 
                 labs(col="region") + 
                 ggrepel::geom_text_repel(aes(label = Ecoprovince), 
                                          data = DELTA_frame1, size = 2, direction = "both", 
                                          min.segment.length = 10) +
                 ggtitle(label = NULL, subtitle = parse(text = lm_eqn(lm_Delta_tau))) #+ 
...
```{r}
DELTA_frame2 <- merge(BK_df_disturbance_avg, nls.param.df_bal[c("Ecoprovince", "alpha",  "alpha.variance", "alpha.2.5", "alpha.97.5")], by = "Ecoprovince")

DELTA_frame2$Ecoprovince <- factor(DELTA_frame2$Ecoprovince, levels = c("211", "212", "221", "222", "223", "231", "232", "234", "251", "255", "M211", "M221", "M223", "M231"))

DELTA_frame2$mean_LossRate <- lm2_B_L_frame$mean_LossRate
DELTA_frame2$var_LossRate <- lm2_B_L_frame$var_LossRate

DELTA_frame2$alpha_ML <- (DELTA_frame2$alpha * DELTA_frame2$mean_LossRate) *-1 
DELTA_frame2$se_alpha_ML <- (DELTA_frame2$alpha.variance + DELTA_frame2$var_LossRate) / sqrt(DELTA_frame2$N) ## var(x+y) = var(x) + var(y) #&# se = var/sqrt(N)

### fit lm
lm_Delta_B_L1 <- lm(value~alpha_ML, data = DELTA_frame2)


```

```{r}
gg.Delta2_alpha <- ggplot(aes(x= alpha_ML, y=value, col = Ecoprovince),
                        data = DELTA_frame2) + 
                 geom_hline(yintercept = 0, lty =3) + geom_vline(xintercept = 0, lty=3) +
                 geom_smooth(method = "lm", col = "#00000080", fullrange = T) +
                 geom_point( size =1) +
                 geom_errorbarh(aes(xmin = alpha_ML-(1.96*se_alpha_ML), xmax = alpha_ML+(1.96*se_alpha_ML))) +
                 geom_errorbar(aes(ymin = value-(1.96*se), ymax = (value+1.96*se))) +
                 theme_classic() + 
                 theme(legend.position = "top", strip.text.x = element_text(size = 6), aspect.ratio =1, 
                       legend.margin = margin(0, 0, 0, 0),
                       plot.margin = unit(c(0, 0, 0, 0), "lines"), plot.subtitle = element_text(size = 10)) + 
                 xlab(expression("- "*alpha %*% bar(frac(L[last]-L[first], T[last]-T[first])))) + 
                 ylab(expression(~Delta*"B"[disturbance]*" (Mg ha"^-1*" year"^-1*")"))  + 
                 scale_color_manual(values = col14) +  
                 labs(col="region") + 
                 ggrepel::geom_text_repel(aes(label = Ecoprovince), 
                                          data = DELTA_frame2, size = 2, direction = "both", 
                                          min.segment.length = 10) +
                 ggtitle(label = NULL, subtitle = parse(text = lm_eqn(lm_Delta_B_L1))) # + xlim(c(-0.01,0.01)) #+ ylim(c(-1.,0.5)) #+ xlim(c(-0.5,1.5))
...
gg_BK1 <- ggplot(aes(x=Ecoprovince, y = value, group = variable), data = BK_df_long) +
          ...
          geom_errorbar(aes(x=Ecoprovince, y = value, ymax = value+(1.96*se), ymin = value-(1.96*se), 
                            col = variable), 
                    data = BK_df_avg, position = position_dodge(width = 0.75), width = 0.75) +
          scale_color_manual(values = c("black", "#DC3220BF", "#005AB5BF", "#FFC107BF"),
                             labels = c(expression(Delta*B[observed]), expression(Delta*B[age]), 
                                                   expression(Delta*B[productivity~trend]), 
                                        expression(Delta*B[disturbance])), name=" ") +
...
```{r}
# BK_df_long3 <- BK_df_long[BK_df_long$variable %in% c("DeltaB_total", "DeltaB_age", "DeltaB_year", "Delta"),]
# 
# BK_df_avg3 <- BK_df_avg[BK_df_avg$variable %in% c("DeltaB_total", "DeltaB_age", "DeltaB_year"),]
# 

# 
# BK_df_long3$Ecoprovince <- factor(BK_df_long3$Ecoprovince, levels = c("221","251","222","234","223","211","M223", "M221","M211","M231","232","212","255","231","M242","M261","313","332","M333","M313","331","M332","M334","M341","M331"))
# 
# BK_df_avg3$Ecoprovince <- factor(BK_df_avg3$Ecoprovince, levels = c("221","251","222","234","223","211","M223", "M221","M211","M231","232","212","255","231","M242","M261","313","332","M333","M313","331","M332","M334","M341","M331"))

gg_BK2b <- ggplot(aes(x=Ecoprovince, y = value, group = variable), 
                 data = BK_df_avg) +
  
           # annotate('rect', xmin = seq(0.5,13.5, by =2), 
           #          xmax = seq(1.5,14.5, by = 2), 
           #          ymin=-5, ymax=7, alpha=0.2, fill="lightblue") +
           # 
          geom_jitter(aes(x=Ecoprovince, y = value, fill = variable), 
                      data = BK_df_long,
                      position = position_jitterdodge(1.75), 
                      shape = ".", alpha = 0.25, col = "gray") +
  
          geom_hline(yintercept = 0, lty = 2) +
          
          #facet_grid(variable~Region, scales= "free", space = "free_x", drop = TRUE) +
          
          facet_wrap(vars(variable), strip.position = "right", ncol =1, nrow =4, 
                     labeller = as_labeller(facet_names)) +
  
          geom_point(aes(x=Ecoprovince, y = value, col = variable), 
                    data = BK_df_avg, 
                    position = position_dodge(width =0.75), size =2.5, shape = 20) + 
          
          geom_errorbar(aes(x=Ecoprovince, y = value, ymax = value+(1.96*se), ymin = value-(1.96*se), col = variable), 
                    data = BK_df_avg, 
                    position = position_dodge(width = 0.75), width = 0.75) +
          
          scale_color_manual(values = c("#000000", "#E22046", "#1D00FF","#FFC107"),
                             labels = c(expression(Delta*B[observed]), 
                                        expression(Delta*B[age]),
                                        expression(Delta*B[productivity~trend]),
                                        expression(Delta*B[disturbance])), name=" ") +
          
          guides(fill="none") +  ## removed second legend
         
          ylab(expression(Delta*B~"(Mg ha"^-1~year^-1*")")) +
          theme_classic() +
...
# tiff("FIA_Bookkeeper_v2C_April2023.tiff", width = 19, height = 16, units = "cm", res = 600, compression ="lzw")        
gg_BK2b + coord_cartesian(ylim=c(-1,3))
# dev.off()
```

```{r}

BK_df_long4 <- BK_df_long[BK_df_long$variable %in% c("DeltaB_total",  "DeltaB_disturbance"),]


BK_df_avg4 <- BK_df_avg[BK_df_avg$variable %in% c("DeltaB_total",  "DeltaB_disturbance"),]


gg_BK2c <- ggplot(aes(x=Ecoprovince, y = value, group = variable), 
                 data = BK_df_long4[complete.cases(BK_df_long4),]) +
          ...
          facet_wrap(vars(variable), ncol =1, nrow =2) +        
  
          geom_point(aes(x=Ecoprovince, y = value, col = variable), 
                    data = BK_df_avg4[complete.cases(BK_df_avg4),], 
                    position = position_dodge(width =0.75), size =2.5, shape = 20) + 
          ...
          scale_color_manual(values = c("#00000080", "#FFC10780", "#004D4080"),
                             #c("#00000080", "#DC322080", "#005AB580"), #,"#FFC10780", "#004D4080"),
                             labels = c(expression(Delta*B), 
                                        #expression(Delta*B[age]),
                                        #expression(Delta*B[year])), name = " ") +
                                        #expression(Delta*B[climate]), 
                                        expression(Delta*B[disturbance])), name=" ") +
          ...
          theme(axis.text.x = element_text(angle = 33, vjust = 1, hjust=1),
                legend.position = "top", 
                legend.margin = margin(0, 0, 0, 0), 
                strip.text.y = element_blank())  + ylim(c(-3,3))

          
          
          
# tiff("FIA_Bookkeeper_disturbance_June_2023.tiff", width = 20, height = 9.33, units = "cm", res = 600, compression ="lzw")        
# gg_BK2c
# dev.off()
...
### grab the pltIDs with >= 2 observations
# g1.pltIDs <-  names(table(G$pltID_first) [table(G$pltID) >1])
# 
# # Subset 
# G_bal <- G[G$pltID %in% g1.pltIDs,] 
...
G_first <- G_first[,c("ECOPROVCD", "STDAGE_t2")]  #subset the needed columns
colnames(G_first)[2] <- "STDAGE"
G_first$measurement <- "first"

G_last <- G_last[,c("ECOPROVCD", "STDAGE_t2")]  #subset the needed columns
colnames(G_last)[2] <- "STDAGE"
G_last$measurement <- "most-recent"

## stack the two data.frames
G_age <- rbind(G_first, G_last)
G_age$measurement <- as.factor(G_age$measurement)
G_age$ECOPROVCD <- droplevels(G_age$ECOPROVCD)



## subset 
G_age <- G_age[G_age$ECOPROVCD %in% c("221","251", "222", "234", "M223", "M231", "211", "M221", "231", "223", "M211", "232", "212", "255"),]

G_age$ECOPROVCD <- factor(G_age$ECOPROVCD, levels= c("221","251", "222", "234", "M223", "M231", "211", "M221", "231",  "223", "M211", "232", "212", "255"))
...
gg_age_dist <- ggplot(aes(y=ECOPROVCD, x=STDAGE), data = G_age) +
               geom_density_ridges(aes(fill = measurement, point_color = measurement, linetype = measurement), 
                                   scale = 1,
               ...
               scale_fill_manual(values = c("#696969", "#DC3220")) +
               scale_linetype_cyclical(values =c("dashed","solid")) +
               xlab("stand age (years)") + ylab("Ecoprovince") +
               ...
               # scale_y_discrete(limits = c("221", "234", "251", "222", "M223", "223",  "M221", "211", "M231", "M211",
               #                             "231", "232", "212", "255", "313", "332", "M333", "M313",
               #                             "331", "M332", "M334", "M341", "M331", "263", "M242", "M261")) +
               coord_flip() + xlim(c(-10,250))  #+ 
               # ggh4x::facet_grid2(.~region, scales = "free", space = "free_x", drop = TRUE)
...
# ### combine
gg_fig3.Xb <- cowplot::plot_grid(gg_BK2b + coord_cartesian(ylim=c(-2, 2.5)),
                                 
                                 gg_age_dist + theme(strip.text.x = element_blank(),
                                                     legend.position = "bottom",
                                                     plot.margin = margin(t = 0, r = 24, 
                                                                          b = 0, l = 0, unit = "pt")), 
                                 
                                 nrow =2, ncol =1, align = "hv", axis = "l", 
                                 labels = "AUTO",
                                 rel_heights = c(2.2,1))


## make the graphic
# tiff(filename = "Fig3_07.18.2023_3_stacker_v2.tiff", width = 13.5, height = 22, units = "cm", res = 600, compression = "lzw")
gg_fig3.Xb
# dev.off()
```
...
```{r}
year_avg_merger1 <- merge(BK_df_year_avg, BK_df_total_avg, by = c("Ecoprovince", "N"), suffixes = c("_year", "_total"))

year_avg_merger1$Ecoprovince <-  factor(year_avg_merger1$Ecoprovince, levels =c("211","212","221","222","223","231","232","234","251","255","M211","M221","M223", "M231"))

### fit lm
lm_merger1 <- lm(value_total~value_year, data = year_avg_merger1)
```


```{r}
## plot level correlation
cor.test(BK_df$DeltaB_year, BK_df$DeltaB_total)
## ecopov level correlation
cor.test(year_avg_merger1$value_year, year_avg_merger1$value_total)
```


```{r}
gg.DELTAB_prod_corr1 <- ggplot(aes(y= value_total, x = value_year, col = Ecoprovince),
                                  data = year_avg_merger1) + 
                       # geom_point(aes(y= DeltaB_total, x = DeltaB_year, col = Ecoprovince),
                       #            data = BK_df, shape = "+") +
                       geom_hline(yintercept = 0, lty =3) + geom_vline(xintercept = 0, lty=3) +
                       geom_smooth(method = "lm", col = "#00000080", fullrange = T) +
                       geom_point(size =0.5) +
                       geom_errorbar(aes(ymax= value_total+1.96*se_total, ymin = value_total-1.96*se_total, 
                                         col = Ecoprovince),
                                  data = year_avg_merger1) + 
                       geom_errorbarh(aes(xmax= value_year+1.96*se_year, xmin = value_year-1.96*se_year, 
                                          col = Ecoprovince),
                                  data = year_avg_merger1) + 
                       scale_color_manual(values = col14) +
                       theme_classic() + # ylim(c(-1,2.5)) + xlim(c(-1,2.5)) +
                       ggtitle(label = NULL, subtitle = parse(text = lm_eqn(lm_merger1))) +
                       theme(legend.position ="none", plot.subtitle = element_text(size = 9))  +
                       ggrepel::geom_text_repel(aes(label = Ecoprovince), 
                                          size = 3, direction = "both", 
                                          min.segment.length = 10) + 
                       xlab(expression(Delta*B[productivity~trend]~"(Mg ha"^-1*" year"^-1*")")) + 
                       ylab(expression(Delta*B[observed]~"(Mg ha"^-1*" year"^-1*")")) 
                       
  

# tiff(filename = "DeltaB_year vs. DeltaB_total.tiff", width =10, height =10, units="cm", res = 600, )
# gg.DELTAB_prod_corr1
# dev.off()  
```
...
```{r}
year_avg_merger2 <- merge(BK_df_age_avg, BK_df_total_avg, by = c("Ecoprovince", "N"), suffixes = c("_age", "_total"))

year_avg_merger2$Ecoprovince <-  factor(year_avg_merger2$Ecoprovince, levels =c("211","212","221","222","223","231","232","234","251","255","M211","M221","M223", "M231"))

### fit lm
lm_merger2 <- lm(value_total~value_age, data = year_avg_merger2)
```


```{r}
## plot level correlation
cor.test(BK_df$DeltaB_age, BK_df$DeltaB_total)
## ecopov level correlation
cor.test(year_avg_merger2$value_age, year_avg_merger2$value_total)
```


```{r}
gg.DELTAB_prod_corr2 <- ggplot(aes(y= value_total, x = value_age, col = Ecoprovince),
                                  data = year_avg_merger2) + 
                       # geom_point(aes(y= DeltaB_total, x = DeltaB_year, col = Ecoprovince),
                       #            data = BK_df, shape = "+") +
                       geom_hline(yintercept = 0, lty =3) + geom_vline(xintercept = 0, lty=3) +
                       geom_smooth(method = "lm", col = "#00000080", fullrange = T) +
                       ...
                       geom_errorbar(aes(ymax= value_total+1.96*se_total, ymin = value_total-1.96*se_total, 
                                         col = Ecoprovince),
                                  data = year_avg_merger2) + 
                       geom_errorbarh(aes(xmax= value_age+1.96*se_age, xmin = value_age-1.96*se_age, 
                                          col = Ecoprovince),
                                  data = year_avg_merger2) + 
                       scale_color_manual(values = col14) +
                       theme_classic() + # ylim(c(-1,2.5)) + xlim(c(-1,2.5)) +
                       ggtitle(label = NULL, subtitle = parse(text = lm_eqn(lm_merger2))) +
                       theme(legend.position ="none", plot.subtitle = element_text(size = 9))  +
                       ggrepel::geom_text_repel(aes(label = Ecoprovince), 
                                          size = 3, direction = "both", 
                                          min.segment.length = 10) + 
                       xlab(expression(Delta*B[age]~"(Mg ha"^-1*" year"^-1*")")) + 
                       ylab(expression(Delta*B[observed]~"(Mg ha"^-1*" year"^-1*")")) 
                       
  
# tiff(filename = "DeltaB_year vs. DeltaB_total.tiff", width =10, height =10, units="cm", res = 600, )
# gg.DELTAB_prod_corr2  
# dev.off()  
```
...
```{r}
year_avg_merger3 <- merge(BK_df_disturbance_avg, BK_df_total_avg, by = c("Ecoprovince", "N"), suffixes = c("_disturbance", "_total"))

year_avg_merger3$Ecoprovince <-  factor(year_avg_merger3$Ecoprovince, levels =c("211","212","221","222","223","231","232","234","251","255","M211","M221","M223", "M231"))

### fit lm
lm_merger3 <- lm(value_total~value_disturbance, data = year_avg_merger3)
```


```{r}
## plot level correlation
cor.test(BK_df$DeltaB_disturbance, BK_df$DeltaB_total)
## ecopov level correlation
cor.test(year_avg_merger3$value_disturbance, year_avg_merger3$value_total)
```


```{r}
gg.DELTAB_prod_corr3 <- ggplot(aes(y= value_total, x = value_disturbance, col = Ecoprovince),
                                  data = year_avg_merger3) + 
                       # geom_point(aes(y= DeltaB_total, x = DeltaB_year, col = Ecoprovince),
                       #            data = BK_df, shape = "+") +
                       geom_hline(yintercept = 0, lty =3) + geom_vline(xintercept = 0, lty=3) +
                       geom_smooth(method = "lm", col = "#00000080", fullrange = T) +
                       ...
                       geom_errorbar(aes(ymax= value_total+1.96*se_total, ymin = value_total-1.96*se_total, 
                                         col = Ecoprovince),
                                  data = year_avg_merger3) + 
                       geom_errorbarh(aes(xmax= value_disturbance+1.96*se_disturbance, xmin = value_disturbance-1.96*se_disturbance, 
                                          col = Ecoprovince),
                                  data = year_avg_merger3) + 
                       scale_color_manual(values = col14) +
                       theme_classic() + # ylim(c(-1,2.5)) + xlim(c(-1,2.5)) +
                       ggtitle(label = NULL, subtitle = parse(text = lm_eqn(lm_merger3))) +
                       theme(legend.position ="none", plot.subtitle = element_text(size = 9))  +
                       ggrepel::geom_text_repel(aes(label = Ecoprovince), 
                                          size = 3, direction = "both", 
                                          min.segment.length = 10) + 
                       xlab(expression(Delta*B[disturbance]~"(Mg ha"^-1*" year"^-1*")")) + 
                       ylab(expression(Delta*B[observed]~"(Mg ha"^-1*" year"^-1*")")) 
                       
  

# tiff(filename = "DeltaB_year vs. DeltaB_total.tiff", width =10, height =10, units="cm", res = 600, )
# gg.DELTAB_prod_corr3  
# dev.off()  
...
# tiff("fig_S7_3wide.tiff", width = 20, height = 8.5, units = "cm", res = 600, compression = "lzw")

ggpubr::ggarrange(gg.DELTAB_prod_corr1 + ylim(-1,3), 
                  gg.DELTAB_prod_corr2 + ylim(-1,3) + theme(axis.title.y = element_blank()),
                  gg.DELTAB_prod_corr3 + ylim(-1,3) + theme(axis.title.y = element_blank()), labels = "AUTO", ncol=3, nrow =1, widths = c(1.03,0.95,1.02))
...
```{r}
# tiff(file = "DeltaB_compare_2x2_TemporallyBalanced.tiff", width = 18, height =18, units = "cm", compression="lzw", res= 600)

ggpubr::ggarrange(gg.Delta_B_total_compare + theme(legend.position = "none"),
                  gg.age_effect + theme(legend.position = "none"),
                  gg.Delta1_tau + theme(legend.position = "none"),
                  gg.Delta2_alpha + theme(legend.position = "none"), 
                  ncol = 2, nrow =2, align = "h", common.legend = F, labels = "AUTO")
# dev.off()
```

...
predict_f.age_first <- function(X, mod.number, data) {
...
## for f(age_last)
predict_f.age_last <- function(X, mod.number, data) {
...
### define a few things 
ecoprov = 211
mod <- get(paste("nls_", ecoprov, ".",  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == paste(ecoprov),]$Best.Mod, sep =""))
mod.num <-  Best_mods.P_bal[Best_mods.P_bal$Ecoprovince == paste(ecoprov),]$Best.Mod
data <- get(paste("DeltaB_df_", ecoprov, sep = ""))
vcov_mat <- vcov_list[[paste(ecoprov)]]
...
parameters[,"alpha"] <- rep(nls.param.df_bal[nls.param.df_bal$Ecoprovince == ecoprov,]$alpha, nsamp)
parameters[,"tau"] <- rep(nls.param.df_bal[nls.param.df_bal$Ecoprovince == ecoprov,]$tau, nsamp)

### solve f(age) function at age_first and age_last
sim_f_age_first <- apply(parameters, 1, predict_f.age_first, data = data, mod.number = mod.num,
                         simplify = T)
sim_f_age_last <- apply(parameters, 1, predict_f.age_last, data = data, mod.number = mod.num,
                        simplify = T)
...
### take means and store in list 
B_sim_pred_f_age_bar[[paste(ecoprov)]] <- rowMeans(sim_DeltaB, na.rm = T)
...
predict_f.year_first <- function(X, mod.number, data) {
...
## for f(year_last)
predict_f.year_last <- function(X, mod.number, data, nls.pars) {
...
parameters <- if(mod.num == 2){
  matrix(data = c(rnorm(nsamp, summary(mod)$param["tau","Estimate"], vcov_mat["tau","tau"]), ### sample parameters from Random Normal dist
...
sim_f_year_first <- apply(parameters, 1, predict_f.year_first, data = data, mod.number = mod.num, 
                          simplify = TRUE)
sim_f_year_last <- apply(parameters, 1, predict_f.year_last, data = data, mod.number = mod.num,
                        simplify = TRUE)
...
### take means and store in list 
B_sim_pred_f_year_bar[[paste(ecoprov)]] <- rowMeans(sim_DeltaB, na.rm = T)
...
predict_f.disturbance_first <- function(X, mod.number, data) {
...
## for f(B_L_prop_last)
predict_f.disturbance_last <- function(X, mod.number, data, nls.pars) {
...
parameters <- if(mod.num == 2){
  matrix(data = c(rep(summary(mod)$param["tau","Estimate"], 9999), 
                  rnorm(nsamp, summary(mod)$param["alpha","Estimate"], vcov_mat["alpha","alpha"]), ### sample parameters from Random Normal distrep
                  rep(summary(mod)$param["A","Estimate"], 9999),
                  rep(summary(mod)$param["k","Estimate"], 9999)), nrow = 9999, ncol =4,
         dimnames = list(NULL, c("tau", "alpha", "A", "k")))
} else if(mod.num == 5){
...
sim_f_disturbance_first <- apply(parameters, 1, predict_f.disturbance_first, data = data, mod.number = mod.num,
                                 simplify = TRUE)
sim_f_disturbance_last <- apply(parameters, 1, predict_f.disturbance_last, data = data, mod.number = mod.num,
                                simplify = TRUE)
...
### take means and store in list 
B_sim_pred_f_disturbance_bar[[paste(ecoprov)]] <- rowMeans(sim_DeltaB, na.rm = T)
...
gg_BK2b <- ggplot(aes(x=Ecoprovince, y = value, group = variable), 
          ...
          facet_wrap(vars(variable), ncol =1, nrow =4) +
...
gg_BK2b + coord_cartesian(ylim=c(-1,2.5))
# dev.off()
```
...
gg_fig3.Xb <- cowplot::plot_grid(gg_BK2b + coord_cartesian(ylim=c(-2, 2.5)), #+
                                 #theme(plot.margin = margin(t = 0, r = 12.5, b = 0, l = 0, unit = "pt")) ,
...
## make the graphic
tiff(filename = "Fig3_07.17.2023_3_stacker_v2.tiff", width = 13.5, height = 20, units = "cm", res = 600, compression = "lzw")
gg_fig3.Xb
dev.off()
```
...
gg.DELTAB_prod_corr1 <- ggplot(aes(y= value_total, x = value_year, col = Ecoprovince),
                       ...
                       ggrepel::geom_text_repel(aes(label = Ecoprovince), 
                                          ...
                                          min.segment.length = 10) + 
                       xlab(expression(Delta*B[productivity~trend]~"(Mg ha"^-1*" yr"^-1*")")) + 
                       ylab(expression(Delta*B[observed]~"(Mg ha"^-1*" yr"^-1*")")) 
...
gg.DELTAB_prod_corr2 <- ggplot(aes(y= value_total, x = value_age, col = Ecoprovince),
                       ...
                       ggrepel::geom_text_repel(aes(label = Ecoprovince), 
                                          size = 3, direction = "both", 
                                          min.segment.length = 10) + 
                       xlab(expression(Delta*B[age]~"(Mg ha"^-1*" yr"^-1*")")) + 
                       ylab(expression(Delta*B[observed]~"(Mg ha"^-1*" yr"^-1*")")) 
...
gg.DELTAB_prod_corr3 <- ggplot(aes(y= value_total, x = value_disturbance, col = Ecoprovince),
                       ...
                       ggrepel::geom_text_repel(aes(label = Ecoprovince), 
                                          size = 3, direction = "both", 
                                          min.segment.length = 10) + 
                       xlab(expression(Delta*B[disturbance]~"(Mg ha"^-1*" yr"^-1*")")) + 
                       ylab(expression(Delta*B[observed]~"(Mg ha"^-1*" yr"^-1*")")) 

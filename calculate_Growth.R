### Calculate G - FIA Stand biomass Growth dataset -- 
...
## STEP 2 - TREE BIOMASS G
  ...
  for(i in 1:nrow(calc_df)) {   
    ...
    calc_df$B_L_prop[i] <- ifelse(calc_df$B_plt_t1_MgHa[i] == 0, 0, ((calc_df$B_dead_MgHa[i] + calc_df$B_cut_MgHa[i]) / calc_df$B_plt_t1_MgHa[i]))
...
### first  fix the issue when MEASTIME_t1 is zero -- in that case, we take MEASTIME_t2 - REMPER to fill in. 
G[G$MEASTIME_t1 == 0,]$MEASTIME_t1 <- G[G$MEASTIME_t1 == 0,]$MEASTIME_t2 -   G[G$MEASTIME_t1 == 0,]$REMPER
...
colnames(G)[48] <-  "STDAGE_t2" # make the STDAGE col from the condition table be "STDAGE_t2"
...
save(G, file = "G_clean&subsetted_07.10.2023.Rdata")
